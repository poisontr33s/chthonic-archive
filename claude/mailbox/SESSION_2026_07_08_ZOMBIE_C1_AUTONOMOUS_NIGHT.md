...
Reviewed both questions §7 asks. First: did this run expose a gap in `/nightly`'s own design? Yes — §3's verification gate is written renderer-only (`cargo build`, `render-smoke.ps1`), and this is the *second* run in a row (B3 tonight, now C1) that had to silently improvise a CLI/script-shaped equivalent instead of the skill naming one. Two runs needing the same unstated workaround is a pattern, not a coincidence — fixed directly in `SKILL.md`: §3 now names two explicit verification paths, compiled/rendered and scripted/CLI, so the next Python-shaped nightly doesn't have to reinvent this.

**Correction, added 2026-07-09:** this section originally said "re-verified at 100% via `skill_audit.py`" — that was itself the exact mistake this session already named once (no self-verification circularity), recurred with a different tool. `skill_audit.py`'s Claude check only confirms `SKILL.md` exists with a `name`/`description`/known `allowed-tools` — it never reads the body, so it could not have verified the §3 fix at all. The real verification, done after the user caught this: manually traced every `§N` cross-reference in the file against the current header list and confirmed each resolves correctly — and found a second real bug this way, in the very passage this retrospective describes (`§2/§3` was ambiguously overloaded between the skill's own sections and the frontier atlas's categories).
