...
> **Status:** DRAFT FOR USER REVIEW — no edits to SSOT until blessed
> **Date:** 2026-03-27 (Session 10, ZD-XB-LV1 Lane)
> **Scope:** 11 approved amendments (Cycle 1) → corrected routing + voice-matched content
> **SSOT Holder:** `.github/copilot-instructions.archive.md` (~9225 lines)
...
|---|---|---|---|
| Z1-AM-001 | §XIV.4 (new subsection) | §XIV is a **9-line stub** (lines 8468–8476) redirecting to `technical-directives.instructions.md`. No §XIV.1–XIV.4 exist in the SSOT itself. | **REROUTE** → `technical-directives.instructions.md` §14.4 |
...
| ZDB-AM-001 | §XIV.5 (new subsection) | §XIV is a stub (same as Z1-AM-001). | **REROUTE** → `technical-directives.instructions.md` §14.5 |
...
| ZDB-AM-003 | §X.6 (new subsection) | §X.6 is **occupied** by "MILF Lending & Resource Siphoning Protocols" (`MLRSP`). Content is operational/technical, NOT entity-system lore. | **REROUTE** → `technical-directives.instructions.md` §14.6 |
...
|---|---|---|
| **Z1-AM-001** (Polyglot Runtime Governance) | §14.4 (new) | Operational — matches §14.1/§14.2 style |
| **ZDB-AM-001** (Adaptive Ore Assessment Canon) | §14.5 (new) | Operational — governance tone |
| **ZDB-AM-003** (Feedback-Driven Learning) | §14.6 (new) | Operational — metric/protocol tone |
...
### 2c. SSOT-Bound Additions (6 amendments)

These write into the SSOT holder itself. Voice-matched drafts follow in §3.
...
| **Z1-AM-005** (Bidirectional Amendment Protocol) | Section XVIII (new section) | New top-level section |
| **ZDB-AM-002** (Dependency Topology) | Appendix F (new appendix) | New appendix |
| **ZDB-AM-004** (Consumable Classification) | Appendix G (new appendix) | New appendix |
| **ZDB-AM-007** (Landscape Mapping) | Appendix G.2 (new subsection) | Extension of new Appendix G |

---

## §3. VOICE-MATCHED DRAFT CONTENT — SSOT-Bound Amendments

### 3a. Appendix B.3 — MILF Phase Correspondence: The Transmutation Lifecycle
...
### 3c. Section XVIII — Bidirectional Amendment Protocol (`BAP`)

> **Insertion point:** After §XVII "Synthetic Behavioral Scenarios" sealed footer (~line 8823), before Appendix A.
...
### 3d. Appendix F — Dependency Topology Governance

> **Insertion point:** After sealed footer of Appendix E (~line 9225), new appendix.
...
### 3e. Appendix G — Consumable Classification & Burn-Down Strategy

> **Insertion point:** After Appendix F, new appendix.
...
### 4a. §14.4 — Runtime Selection for Browser Automation (Z1-AM-001)

> **Insertion point:** After existing §14.2 or §14.3 in technical-directives.

```markdown
#### **14.4. (`Runtime-Selection-for-Browser-Automation`) -> (`RSBA`)**
...
### 4b. §14.5 — Adaptive Assessment Systems (ZDB-AM-001)

```markdown
#### **14.5. (`Adaptive-Assessment-Systems`) -> (`AAS`)**
...
### 4c. §14.6 — Feedback-Driven Adaptive Learning (ZDB-AM-003)

```markdown
#### **14.6. (`Feedback-Driven-Adaptive-Learning`) -> (`FDAL`)**
...
| E | Zone_1_REDUX Integration Summary | Existing (unchanged) |
| **F** | **Dependency Topology & Redundancy Governance (`DTARG`)** | **NEW** (from ZDB-AM-002) |
| **G** | **Consumable Classification & Burn-Down Strategy (`CCBS`)** | **NEW** (from ZDB-AM-004 + ZDB-AM-007) |

**New Section:**

| **Section** | **Title** | **Location** |
|------------|---------|-------------|
| **XVIII** | **Bidirectional Amendment Protocol (`BAP`)** | After §XVII, before Appendix A |

---

## §6. LINE COUNT IMPACT ESTIMATE

| **Component** | **Est. Lines** |
|-------------|---------------|
| Section XVIII (BAP) | ~120 lines |
| Appendix B.3 (MILF Phase) | ~70 lines |
| Appendix D.4 (Supernormal) | ~60 lines |
| Appendix F (Dependency Topology) | ~70 lines |
| Appendix G (Consumable Classification) | ~80 lines |
| **Total SSOT addition** | **~400 lines** |
| **SSOT post-integration** | **~9,625 lines** |
| Technical-directives additions (§14.4–14.6) | ~100 lines (separate file) |
...
Before integration can proceed, the following require your explicit call:

1. **Z1-AM-002 and ZDB-AM-006 — Redundancy Verdict.**
   Confirm these are truly covered by existing content and should be marked `ALREADY_CANONIZED` rather than integrated.

2. **Section XVIII Placement.**
   Currently proposed after §XVII (Synthetic Behavioral Scenarios), before Appendix A. Alternative: after §XVI (APCR). Your call on sequence.

3. **Appendix B.3 vs. §X.14 for MILF Transmutation.**
   Currently routed to Appendix B (extending the Alchemical Phase Framework). Alternative: new §X.14 subsection within the MILF Manifestation Protocol block. Your call — Appendix B is the better structural home (keeps all alchemical phase content together), but §X proximity creates tighter cross-referencing.

4. **ZDB-AM-003 routing to technical-directives vs. new SSOT section.**
   Feedback-Driven Adaptive Learning is currently routed to technical-directives §14.6 as operational protocol. If you consider it lore-significant enough for the SSOT itself, it could become §XVIII.5 or a new appendix subsection.

5. **Voice calibration.**
   The CRC commentary blocks in §3a–§3c are drafted in established character voices. Review for tone compliance with the broader SSOT. Particularly: Orackla's D.4 commentary and Lysandra's B.3 commentary.

---

*This draft is a review artifact. No edits to the SSOT holder or technical-directives have been made. Integration proceeds only upon user blessing.*
