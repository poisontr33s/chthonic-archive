...
```mermaid
graph TD
    A[ML Candidate Generated] --> B{Passes Checkpoints 1-9?}
    B -->|No| C[REJECT - Document Violations]
    B -->|Yes| D{Passes Checkpoints 10-13?}
    D -->|No| E[REMEDIATE - Adjust Parameters]
    D -->|Yes| F{Requires SSOT Amendment?}
    F -->|No| G[INTEGRATE - Add to JSON, Document in SSOT Section]
    F -->|Yes| H{Amendment Type?}
    H -->|Minor| I[Section Expansion - Add subsection]
    H -->|Major| J[Structural Change - New tier/chain]
    J --> K[Creator Approval Required]
    K -->|Approved| L[AMEND SSOT - Update metadata, sections]
    K -->|Rejected| C
    I --> L
    L --> M[Update All Artifacts]
    M --> N[.ankhrc Registration]
