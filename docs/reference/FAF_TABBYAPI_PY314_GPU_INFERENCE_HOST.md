...
| **GPU inference (tabby-modern, exllamav2)** | ✅ | ✅ | ✅ | ❌ | ❌ | **`admitted_model_load_gate` L2** — P-10 2026-04-26: `load_time_s=18.2`; exllamav2 EXL2 8bpw; CDI GPU |
...
**Status:** `admitted_import_gate` — P-09 executed 2026-04-25T19:57:28Z inside `chthonic-tabby-modern-gpu:latest` (`e5b8a003e9d1`); 9/9 module imports PASS; settings instantiation OK; app factory OK (9 routes). Containerfile fix: dropped `--no-build-isolation` so uv resolves hatchling+editables via isolated build env.  
**Proof:** `manifest/tabby_modern_import_gate.json` — emitted 2026-04-25T19:57:28Z; `python_version=3.14.4`, `substrate=linux`, `status=admitted_import_gate`, `level=L1`.
...
**Status:** `admitted_model_load_gate` — **G10 ADMITTED 2026-04-26** — P-10 executed inside `chthonic-tabby-modern-gpu:latest` (`0a625eec29f6`); `state.loaded=True`; `model_name=test-model`; `load_time_s=18.2`. `manifest/tabby_model_load_gate.json` emitted.  
**Proof:** `manifest/tabby_model_load_gate.json` — `level=L2`, `status=admitted_model_load_gate`, `verdict=ADMITTED`, `model_dir=/models/test-model`, `load_time_s=18.2`  
**Backend pivot:** `state.py` switched exllamav3→exllamav2 (commit `49e6e33f`). Root: exllamav3 does not support EXL2 quantization format — `No tensors found for model.layers.0.self_attn.q_proj matching supported quantization format`. exllamav2 natively supports EXL2 (G4/G8 Linux admitted). API: `ExLlamaV2Config(str(model_path))` / `ExLlamaV2(cfg)` / `model.load()` / `ExLlamaV2Tokenizer(cfg)`.  
**GPU passthrough:** CDI device `nvidia.com/gpu=all` via `/etc/cdi/nvidia.json`. Hooks path (`/usr/share/containers/oci/hooks.d/`) absent in NVIDIA-Workbench WSL2 distro — CDI is the correct mechanism. Driver: 596.21. `--security-opt=label=disable` not required with CDI.  
**Deviation log (Claudine directive — do not let this calcify):**  
- flash_attn CUDA kernel (`flash_attn_2_cuda`) is NOT compiled — Python-only mode via patch `build/tabby-modern-gpu/patches/patch-flash-attn-cuda-optional.py` (commit `db0f1807`/`e71e032e`). G10 model load does not call attention functions. G11 generation will hit flash_attn kernel or fall through to torch SDPA — path unproven, requires its own gate.  
- Containerfile CMD still shows P-09 in image `0a625eec29f6` (CMD was edited but build was cached through that layer). Runtime override (`python /ext-probes/P-10.py`) is gate-validated. CMD must be corrected before G12 (server smoke) so production launch path matches gate-validated path.
...
| **`tabby_modern` import gate admitted** | **P-09 2026-04-25T19:57:28Z — 9/9 modules PASS, `settings_check=OK host=127.0.0.1 port=5000`, `app_factory_check=OK 9 routes`; `manifest/tabby_modern_import_gate.json` emitted; image `e5b8a003e9d1`** |
| **GPU inference (tabby-modern, exllamav2 model load)** | **P-10 2026-04-26 — `state.loaded=True`, `model_name=test-model`, `load_time_s=18.2`; `manifest/tabby_model_load_gate.json` emitted; image `0a625eec29f6`; CDI `nvidia.com/gpu=all`; exllamav2 EXL2 8bpw Llama-3.1-8B-Instruct** |
...
| GPU inference (tabby-modern, exllamav2 backend) | model load + inference smoke test | G11: single generation + TTFT | ~~`package_forged`~~ → ~~`admitted_import_gate`~~ → ~~`probe_pending_G10`~~ → **`admitted_model_load_gate` L2** — P-10 2026-04-26: `state.loaded=True`, `load_time_s=18.2`; backend pivot exllamav3→exllamav2 (EXL2 format); CDI GPU passthrough; `manifest/tabby_model_load_gate.json` emitted. |
...
| DEV-G10-03 | Backend identity resolved — path A confirmed: corpus is EXL2 (Llama-3.1-8B-Instruct-exl2-8.0bpw + turboderp-released EXL2 models). exllamav2 is the production backend. tabby-modern "exllamav3-native" framing deprecated. `state.py` module docstring updated at commit `49e6e33f` — now reads: "exllamav2 container lifecycle / Backend: exllamav2 (EXL2 quantized models)" with backend history v0.1→v0.2 documented inline. exllamav3 0.0.30 does not support EXL2 format (confirmed: `No tensors found for model.layers.0.self_attn.q_proj matching supported quantization format`). exllamav3 path remains dormant until EXL3-format corpus is available. | **resolved — 2026-05-11** | — | `state.py` docstring revised + ticket c9d0e1f2 closed. G11 unblocked. |

**Punch list last updated:** 2026-05-11 (DEV-G10-03 resolved — path A, exllamav2 production backend confirmed). Items close when their owner gate is admitted with the stated evidence. CI does not currently read this table — it reads `manifest/*.json`. Gate-level evidence is the canonical closure artifact; this table is the human/agent-facing state summary.
