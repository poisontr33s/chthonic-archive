...
class GUIllaume
	...
	def loadConfigs()
		...
		_script_dir = File.dirname(File.expand_path(__FILE__))
		configFileName = File.join(_script_dir, "FAYDEconfig.txt")
		if File.exist?(configFileName)
			configs = File.read(configFileName).split(/\r?\n/)
			configs.map! { |e| e.strip.split(":",2) }

			configs.each do |config|
				if config[1].length > 0
					case config[0]
					when 'database'
						@dbFname = File.expand_path(config[1], _script_dir)
					when 'font'
