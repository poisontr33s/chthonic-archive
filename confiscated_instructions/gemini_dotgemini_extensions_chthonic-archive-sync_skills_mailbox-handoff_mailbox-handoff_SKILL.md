...
## Conventions
- Use clear handoff headers with frontmatter (`type`, `from`, `to`, `created`, `priority`).
- Keep messages short, factual, and actionable.
- Cross-reference only the necessary files.
...
## Command (continuation assistant)

This repo includes a continuation-first inbox reader to prevent "mailbox spam" workflows:
...
Prefer the `SESSION_HANDOFF_*.md` files over chronicles/payload docs when continuing.

## Addendum: Mailbox Scribe
Use the mailbox scribe when you want the mailbox to maintain a single, up-to-date session packet automatically.

Purpose:
- Regenerate a single packet markdown from current mailbox artifacts.
- Update mailbox manifest/current-state files.
- Do not delete historical content.
...
`mailbox_check.py` now orchestrates related mailbox tools directly so mailbox flow is one command surface, not fragmented manual steps.
...
Link canon guard for markdown handoffs (duplicate filename disambiguation):

Dry-run check (fails on findings):

```powershell
uv run .codex/skills/mailbox-handoff/scripts/mailbox_check.py --mode link-canon --link-canon-file claude/mailbox/SESSION_HANDOFF_2026_02_27_PATH_LINK_DISAMBIGUATION_CANON.md
```

Apply fixes in place (repairs common malformed links and disambiguates duplicate basenames):

```powershell
uv run .codex/skills/mailbox-handoff/scripts/mailbox_check.py --mode link-canon --link-canon-file claude/mailbox/SESSION_HANDOFF_2026_02_27_PATH_LINK_DISAMBIGUATION_CANON.md --link-canon-apply
```
...
uv run scripts/handoff_loop.py ack <file> --reader codex

# Session-start sweep: stale + pending obligations
uv run scripts/handoff_loop.py sweep
```

## Link Audit Standalone (post-create guard)
