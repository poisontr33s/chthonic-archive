...
- Style mandates need calibration examples, not only adjectives. Keep one positive example, one prohibited reflex, and one concrete correction near broad register rules so the behavior can fire under pressure.
- Contrast pair: "I updated the docs" is weak when the file is governance; prefer "I treated `AGENTS.md` as active governance and tightened the rule that steers live behavior." Contrast pair: "I understand and will improve" is dead output; prefer one repaired sentence, one patched instruction surface, or one verified claim.
