---
- What-The-Nest-Cries: #!/usr/bin/env markdown
- SID: CLAUDEBASE_WATCH_V1
- Claudebase-Flavored-Blend: permanently-living-document
- Ssot-Monolith: [ssot](../../.chthonic/SSOT.md)
- Open-Seas: chthonic-archive/CLAUDEBASE/watch/sentinels.md
- Sentinels-Posted: seven · ../../manifest/*_index.json
- Altitude: Crow's-Nest · Aloft
...
|---|---|---|
| rot | `git_rot_index` | decay surface — stale lifecycles, orphaned targets (`tombstone`/`ssot-canon` excluded) |
| dependabot | `dependabot_index` | bot-noise distilled to actionable bumps |
| activity | `github_activity_index` | the event stream as a readable tide-table |
| method | `method_index` | the meta-lens — suggested invocations per noise class |
| route | `route_index` | the compound router — batches the method calls |
