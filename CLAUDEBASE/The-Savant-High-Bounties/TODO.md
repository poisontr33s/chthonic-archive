...
**Every task in this file has evidence.** The evidence lives in [GRILLING.md](GRILLING.md).
Before executing a `[DECISION]` task: read the corresponding GRILLING section.  
Before disputing an acceptance criterion: the grilling observation is the reason it exists.
...
**All probe outputs go to `manifest/`.** Never to terminal. The CI membrane reads manifests — not scrollback.
...
## GATE -1 · MCP ECOSYSTEM MODERNIZATION (Intelligence Layer)
