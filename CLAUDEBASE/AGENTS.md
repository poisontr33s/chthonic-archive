...
- *— Keep the working setting local to —* **(`CLAUDEBASE`)** *— unless tasking, explicitly requires a wider entourage as per, 'repositoire' changes, off-island.*
  ...
  - *— Preserve existing tribal markdown flavours in — **(`ANKHOLOGY`/`DSL`)** — **{`ext.-kind`}**`.*
    ...
    - *— Prefer concise, real instructions over meta-documentation.*
    
      - *— Do not invent new token sources or alternate MCP wiring.*

        - *— Do not ask the user to refresh or rotate tokens unless the pool verifier proves the existing token is expired or revoked.*

          - *— For token and MCP work, use the repo scripts and the API pool as the source of truth:*
...
## (`Natural-Adaptive-Tribunal`/`Boxing-Ring-Pivot`/`On-Multi`/`Cross-Laning`)

- *— Make smaller changes resolving i.g.*
