...
- **TAA ghosting under camera motion — investigated, one fix attempt reverted, correctly re-scoped.** First hypothesis (motion vectors were jitter-only) was checked against the real code and found WRONG — `taa_resolve.frag` already has a proper 3×3 neighbourhood clamp, and the real per-pixel motion vectors do correctly include camera motion. Actual mechanism: `blend: 0.1` (`renderer.rs:2725`) is a hardcoded constant, never motion-adaptive — a small clamp box is fine for the sub-pixel jitter that was the only motion in this engine's history until tonight, but admits wrong-but-plausible history under real multi-pixel motion. Attempted the standard fix (motion-adaptive blend weight) — pixel-diff and visual comparison caught a real regression (new streaking/aliasing on wave crests) before it shipped, root-caused to the fix being unable to tell "camera moved" apart from "the water surface itself is animating" (the one motion-vector channel conflates both), so ordinary wave-crest motion alone tripped the adaptive threshold. Reverted cleanly, verified byte-identical to pre-attempt on both the static and pan diagnostics. Real fix needs a way to separate camera-motion from surface-animation motion, which doesn't exist yet — candidate directions (a separate camera-only motion pass, widening the clamp instead of the blend, empirical multi-frame tuning instead of a single-jump test) are written up but not decided. Ref: `[[reference_taa_ghosting_investigation]]`.
