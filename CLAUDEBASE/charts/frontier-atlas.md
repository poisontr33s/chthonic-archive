...
- **Fork-V — Beer-Lambert hit-shading for RT reflections.** The RT-reflections closest-hit shader (`assets/shaders/water_reflection.rchit`) currently does flat vertex-color × basic N·L. The hero water shader's full depth-driven Beer-Lambert + sand-bottom + Fresnel model exists already in `water.frag` and just needs duplicating into the hit shader. Gate condition (does the ray hit the right geometry, in the right place) was proven by Phase 5's pixel-diff this session. Readiest of the three Rung 2.5 forks. Ref: `CLAUDEBASE/charts/north-star-constellations.md` §4.5, `[[project_rung_25_rt_reflections_checkpoint]]`.
