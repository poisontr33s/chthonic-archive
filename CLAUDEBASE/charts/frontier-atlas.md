...
- **Camera input wiring — render-math proven, winit-event-delivery still open.** Surfaced by a digression into 5 cloned reference repos (hsimpson's webgpu/GLSL demos) — confirmed via grep that `main.rs`'s event loop handled zero keyboard/mouse events, and `camera.rs`'s `set_distance`/`set_target` sat unused. Wired WASD (continuous pan, screen-relative via a new `ground_axes()` method) and mouse wheel (zoom via a new `zoom()` method — caught mid-implementation that `set_distance` alone is a near-no-op under orthographic projection; `ortho_size` is the real zoom control). Pushed back on when asked to actually ascertain execution rather than re-assert it: added a temporary `CHTHONIC_TEST_CAMERA` env-var diagnostic calling the exact same methods the real handlers use, pixel-diffed the result against an untouched baseline — pan moved 60,605/230,400 sampled pixels, zoom (2x) moved 145,936/230,400, both frame-wide and visually confirmable. Build clean, 55/55 tests, no-input path still byte-identical. **What's still genuinely open**: whether winit's own `KeyboardInput`/`MouseWheel` events actually reach the handlers from a real keyboard/mouse — mature-library behavior, lower realistic risk than the render math was, but not something an agent can drive or confirm. Ref: `[[reference_camera_input_wiring]]`.
