...
- **New follow-on, surfaced by that same hands-on test: TAA/DLAA ghosting under camera motion.** Root-caused (read `temporal.rs` directly, not guessed): the standalone motion-vector field is jitter-only by construction (sub-pixel Halton sequence); the water shader's real per-pixel motion vectors DO include camera motion correctly, but until tonight nothing had ever moved except that sub-pixel jitter, so the TAA/DLAA resolve's disocclusion/history-rejection handling has never been exercised by real motion — camera input is the first thing in this engine's history able to expose it. Not a bug in what shipped; a genuine, structural, user-named next task ("structurized into a responsible framework") — needs its own session: open the actual TAA resolve pass, check whether DLAA exhibits the same symptom as the TAA fallback, before shaping a fix. Ref: `[[reference_camera_input_wiring]]`.
