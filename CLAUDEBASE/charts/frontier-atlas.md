...
- **Camera input wiring — implemented, NOT hands-on verified.** Surfaced by a digression into 5 cloned reference repos (hsimpson's webgpu/GLSL demos) — confirmed via grep that `main.rs`'s event loop handled zero keyboard/mouse events, and `camera.rs`'s `set_distance`/`set_target` sat unused. Wired WASD (continuous pan, screen-relative via a new `ground_axes()` method) and mouse wheel (zoom via a new `zoom()` method — caught mid-implementation that `set_distance` alone is a near-no-op under orthographic projection; `ortho_size` is the real zoom control). Build clean, 55/55 tests (4 new, pure-math only), render-smoke byte-identical (152021 bytes, proving the no-input default path is untouched). **What's NOT verified**: actual interactive feel — does WASD really pan, does the direction feel right, does scroll actually zoom, is the sensitivity reasonable. No agent can drive a live window's keyboard/mouse; this needs the user's own hands-on test before it's called done. Ref: `[[reference_camera_input_wiring]]`.
