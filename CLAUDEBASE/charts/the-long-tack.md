...
- **(`Arc-II`)**: ***COMPLETE*** *— **(`L−5`)** … **(`L+1`)** all PROVEN on the 4090. The ladder runs end to end: a hash-seeded field became a live-weather GPU advection-diffusion sim that interpolates, evolves, breathes the real sky, moves with the wind, composes into one gated machine, converges to a verified steady state in* `~5 ms`*, and renders to a self-contained browser artifact. Only optional tails remain (L0 hot-reload via Arc-I's* `--watch`*).*
- **(`Arc-III`)**: *at **(`D0`)** — observing. The design law is settled.*

*Next move: **(`Arc-III`)** (D0 — the DSL, observing; the kill-switch holds) or **(`B9`/`B10`)** compound the barometer into the Library Study. Arc-II is closed: the seam ran, breathed, moved, settled, rendered, and proved itself fast — seven rungs from a claim to a `~5 ms` verified sim.*
