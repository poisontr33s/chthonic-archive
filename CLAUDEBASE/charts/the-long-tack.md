...
- **(`Arc-II`)**: *climbing. **(`L−5`)** … **(`L−1`)** + **(`L0`)** render PROVEN on the 4090 — the field interpolates, evolves, breathes the real sky, moves with the wind, settles to a verified steady state, AND renders to a self-contained browser artifact. Last rung: **(`L+1`)** — perform: measure step latency; heavy verified as fast, not just big. (The one optional tail left is L0 hot-reload, via Arc-I's* `--watch`*.)*
- **(`Arc-III`)**: *at **(`D0`)** — observing. The design law is settled.*

*Next move: **(`L+1`)** (measure latency — heavy verified as fast, not just big), or **(`B9`/`B10`)** compound into the Library Study. The seam ran, it breathes, it moves, it settles to a fixpoint, and now it renders — six rungs from claim to artifact.*
