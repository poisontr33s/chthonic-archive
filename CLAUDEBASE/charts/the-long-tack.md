...
> *— The sim ran on a tea-spoon: two Open-Meteo fields (apparent-temp + wind), self-sourced, no rigor. This arc feeds it the REAL multi-disciplinary data-ocean of the archipelago — each layer a field or boundary the proven engine already accepts. Signal varies by discipline; rigor means naming the empty layers too.*
  ...
  - *— **(`[x]`/`M-1`/`·`/`GEBCO-Bathymetry`/`THE-MEDIUM`)** —* **(`PROVEN`)**: `barometer.ts --bathymetry` *pulls real GEBCO 2020 seafloor depth (Open Topo Data, free/no-key) over the sim's exact 56×22 grid →* `charts/bathymetry.json` *(stable, committed). The sim marks land (elev ≥ 0) as a no-flux barrier; the diffuse shader flows heat only through sea — around the real islands, along the channels. 172 land / 1,060 sea; Cuba renders at the SW corner, Great-Inagua ringed by its own footprint. The sim stopped being flat — it runs on the real Bahama Bank.*
    ...
    - *— **(`[x]`/`M-2`/`·`/`Marine`/`SST`)** —* **(`PROVEN`)** —* `barometer --boundary` *— batches the Open-Meteo Marine API → real —* `sea_surface_temperature` *— + ocean-current per island. The sim sources the sea from SST (prefer sst → air → elevation) — a real correction, not cosmetic: SST differs from air by up to 2.3 °C (San-Salvador air 25.5 vs sea 27.8). Currents are recorded (oceanographic flows-to, m/s) but NOT the advection driver — measured 0.06–0.33 m/s, ~20× weaker than wind, too gentle to visibly transport (the calm-banks truth); advection stays wind-driven surface drift. — **(`M-2b`)** (pure-current, near-static) deferred.*
      ...
      - *— **(`[x]`/`M-3`/`·`/`Hurricanes`/`Episodic-Forcing`)** —* **(`PROVEN`)** *: `barometer.ts --hurricane=dorian` writes HURDAT2 Dorian 2019 Cat5 landfall (2019-09-01T18:00Z · 26.5°N 77.8°W · 165 kt · CP 910 hPa) as cyclonic wind field over all 8 islands (haversine distance + exponential decay r_scale=100 km + NH counterclockwise inflow). Sim converged at outer 22 — Grand-Bahama 90 km from center at 34.6 m/s, Great-Inagua 755 km at ~0 m/s, SST-cooled near-storm, climatological far-field. `--hurricane=dorian_stall` renders the catastrophic stall. SVG render → `charts/dorian_cat5_landfall.html` (live-derived, ephemeral). HURDAT2 track hardcoded (14 fixes genesis→dissipation); snapshot registry extensible. The engine is episodic: steady-state AND storm-pass run the same GPU kernels, different boundary conditions — no Rust changes needed.*

        - *— **(`[x]`/`M-4`/`·`/`Weather-Depth`/`Mist-+-Storm`)** — **(`PROVEN`)**: `wxState()` in `barometer.ts` — derived, not fetched; no new API call. WMO code + RH + dew-spread + wind_speed → six classes: `storm / squall / rain / mist / overcast / clear`. Wired into `writeBoundary()` as `wx_state` field in the boundary JSON contract; the Vulkan sim and HTML render read it per island. Live: Grand-Bahama=storm, Bimini=storm today (95→WMO 80+). The field is a tag not a scalar — the engine accepts it without a sim kernel change.*
...
- *— **(`Arc-I`)** — complete through; — **(`B-9`/`+`/`B-11`)** — the barometer is proven as data + notation (the histogram showed the generator fair). — **(`B-10`)** KILLED —* `Index.html slop`. *— The barometer is now also the data-archaeology instrument; —* `--histogram` *— · —* `--bathymetry` *— · —* `--boundary`*.*
  ...
  - *— **(`Arc-II`/`COMPLETE`)** — L−5…L+1 all PROVEN on the 4090, AND now fed real GEBCO bathymetry **(`M-1`)** — the* `~5 ms` *advection-diffusion sim runs over the real Bahama Bank, flowing around actual island geometry. Only optional tail: L0 hot-reload.*
    ...
    - *— **(`Arc-IV`/`COMPLETE`)** — M-1 (GEBCO bathymetry) + M-2 (marine SST) + M-3 (hurricane episodic forcing) + M-4 (weather-depth classification) all PROVEN. Four real geophysical data layers; the sim runs on the real Bahama Bank under the real sky, under a real storm, with per-island atmospheric classification. The data plane is closed — the named void (seismic) confirmed empty. Three fronts compound forward: RENDER (north-star renderer consuming this plane) · ENGINE (device-local memory for 8.4M-cell scale) · any new data source that earns a signal verdict.*

      - *— **(`End-Goal-Proof`/`It-Compounds`)** — the sim is resolution-scalable now —* `--grid=WxH`*, default 56×22 (The same Vulkan/Rust kernels ran); —* `1,232 → 8.4M cells (6800×` *— on the 4090 — the nascent plan compounds to high-end SCALE. The sweep mapped the next ENGINE frontier; —* `HOST_COHERENT` *— buffers cap throughput at ~12 GB/s vs the 4090's ~1 TB/s —* **(`Device-Local`/`Memory`/`+`/`Staging`)** *— is the next hardening. Three fronts run in parallel now: DATA (M-4…), ENGINE (device-local → CUDA/TensorRT later), & —* **(`RENDER`)** *— the real-time visual front-end charted in —* [`north-star-constellations.md`](north-star-constellations.md) *—which — **(`Consumes-Not-Rebuilds`)** — this sim's proven data plane —* `charts/bathymetry.json` *— (→ the hero shallow-water shader · the single-wind spine → its §2.3): the sim computes the field, the renderer draws the turquoise. **(`The-Seam`/`=`/`The-Data-Plane`)**.*
        ...
        - *— **(`Arc-III`/`PARKED`)** — the DSL stays at D0, observed, kill-switch intact. Deliberately NOT a gate — we do not stalemate on it. It earns continuation only by executing, later.*
          ...
          - *— Next move —* **(`M-4`)** *— weather depth: mist + storm conditions derived from Open-Meteo fields already fetched (CAPE, humidity, dew-point, visibility, pressure). Or pivot to ENGINE hardening: device-local memory + staging buffer replaces HOST_COHERENT, unlocking ~1 TB/s GPU throughput for the large-grid (8.4M cell) frontier. Both compound onto proven sim; no DSL, no slop.*
