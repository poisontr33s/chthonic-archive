...
- **(`Arc-II`)**: *climbing. **(`L−5`)** + **(`L−4`)** + **(`L−3`)** + **(`L−2`)** + **(`L−1`)** PROVEN on the 4090 — the field interpolates, evolves, breathes the real sky, moves with the real wind, AND runs as one gated machine to a verified steady state. The four physics rungs are now a single composed sim. Next rung: **(`L0`)** — render: the steady field over the sea-chart, hot-reloadable like **(`Arc-I`)**.*
- **(`Arc-III`)**: *at **(`D0`)** — observing. The design law is settled.*

*Next move: **(`L0`)** (render the steady field over the sea-chart), or **(`B9`/`B10`)** compound into the Library Study. The seam is no longer a claim — it ran, it breathes, it moves, and now it settles to a fixpoint on its own.*
