...
**(`Fork-V`/`Hit-Shading-Fidelity`)** flat vertex-color × basic N·L (standing, shipped) ⟷ the hero shader's full depth-driven Beer–Lambert + sand-bottom + Fresnel model, duplicated into `water_reflection.rchit`. **(`Gate-Now-MET`)** — the same shape as Fork-I's gate: this fork stays shut only while the pipeline's basic correctness (does the ray hit the right geometry, in the right place) is unproven. Phase 5's pixel-diff (13,594/921,600 differing pixels, bounded exactly to the shallow-water region, zero elsewhere) proves it. This is, as of today, the **(`readiest`)** fork of the three — open it whenever the reflected seabed's flatness relative to looking at the seabed directly becomes the thing worth spending a session on.
