...
**(`Remaining-Open`)**: Copernicus SDB end-to-end fetch (needs `CMEMS_USER`/`CMEMS_PASS` — free registration at marine.copernicus.eu; run `uv run --with copernicusmarine python -m copernicusmarine login`). §2.6 astrology half (owner-defined, never invented). Rung 2.5 RT reflections (optional). Site 6 deferred (ELLIPSOID-RETROFIT tags planted).
