...
- **(`Honest-Next-Compounding-Work`/`Ellipsoid-Sites-1-5-Complete`/`2026-06-26`)** Fork-I ENU pipeline sealed: bathymetry in WGS84 metres, ocean surface at ±200 km ENU, Y_SCALE=1.0, HORIZON_EYE ENU-local (beach level at Nassau), depth_m = −v_world_pos.y direct. SIGMA calibrated from measured Jerlov IB IOPs (Williamson & Hollins 2022, `38283a7c`). Shared-Sky invariant §2.3 complete (Hillaire + clouds, sealed 2026-06-25). **(`Remaining`/`Rung-5-Polish`)**: seagrass bottom-reflectance correction (SAND constant ~30% lower than bare sand), WATER in-scatter blue-shift for open Nassau deep water, Secchi-depth-aligned mix() scale (IB Secchi ≈25–30 m → mix/30 more physical than /20). §2.6 astrology half (owner-defined, never invented). Rung 2.5 RT reflections (optional). Site 6 deferred (ELLIPSOID-RETROFIT tags planted).
