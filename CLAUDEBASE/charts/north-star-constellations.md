...
- **(`Fork-I-World`/`Ellipsoid-Retrofit-Underway`/`Third-2026-06-25-Amendment`)** GPU profiling gate: cloud=0.28ms, frame=0.82ms → full-res sealed, checkerboard not needed. WGS84 substrate `src/render/geodesy.rs` built + 7 tests. Camera `GeoAnchor` bridge + `project_ecef_to_render` built + 4 tests (48 total). **(`Site-1`/`Wired`)** (`renderer.rs:351`): camera now holds Nassau ENU tangent-plane anchor. Sites 2–5 pending: bathymetry geodetic→ENU mesh projection, ocean ENU extents, lens HORIZON_EYE rebase, Y_SCALE tag for ellipsoidal normal distance.
...
- **(`Honest-Next-Compounding-Work`/`Rungs`/`6`/`+`/`7`/`Sealed-2026-06-25`/`Shared-Sky-Invariant`/`§2.3`/`Complete`/`Fork-I-Ellipsoid-Retrofit-Underway`/`Third-2026-06-25-Amendment`)** Profiling gate answered: cloud 0.28ms → full-res sealed. **(`Ellipsoid-Sites-2–5`/`Are-The`/`Current`/`Compound-Frontier`)** (bathymetry geodetic→ENU projection, ocean ENU extents, lens rebase, Y_SCALE tag — dependency-ordered in `~/.claude/plans/nifty-coalescing-lamport.md`). Remaining non-Ellipsoid work: Rung 5 polish (Beer–Lambert tightening against spectral data), §2.6 astrology half (owner-defined meanings, never invented), Rung 2.5 RT reflections (deferred optional).
