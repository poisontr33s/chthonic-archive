...
- **Rung ledger — ANNO 2026-06-10, amended 2026-06-17** (we leapt the §5.2 ladder to prove the gate early — §5.1; the 2026-06-17 amendment submerges the temporal scaffold + the full celestial field into the chart — see [`../logbook/03-celestial-field.md`](../logbook/03-celestial-field.md) and [`../logbook/04-structurize-submerge.md`](../logbook/04-structurize-submerge.md)):
  ...
  - **4 · Ocean surface — `[4.2a seam closed · 4.2b/c pending]`.** The compute pipeline (`ocean_compute.rs`) writes a 256×256 rgba16f displacement field each frame; **`water.vert` now samples it** (set 0, binding 0 combined-image-sampler) rather than evaluating Gerstner inline — the data path from compute→storage-image→vertex-stage is live and self-verified (`renders/4-2a-seam.png`, commit `86f06375`). The compute body is still Gerstner (4.2a); **4.2b** replaces it with a Phillips spectrum sum (wind-directed, spectral character, pure shader swap). **4.2c** adds the butterfly IFFT for the full Tessendorf path.
...
- **Honest next compounding work:** rung 2's remaining **DLAA** consumer (the jitter + motion-vector scaffold is now built) and rung 4 (Tessendorf FFT, hardening from the 4.2a foundation) — alongside the §2.7 celestial / perspective lens-set.
