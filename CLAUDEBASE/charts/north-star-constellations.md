...
- *Astronomy half — `[built]`.* Real solar position (and a clear path to lunar and stellar) over the island's coordinates, Horizons-verified. This is what `cosmos.rs` already is.
...
- Render increments self-verify via `scripts/render-smoke.ps1` (build + bounded run + PASS/FAIL on VUIDs/panics, validation layers on). That covers compile- and runtime-correctness; the *visual* stays a human screenshot — the agent has no display.
...
- **Rung ledger — ANNO 2026-06-10** (we leapt the §5.2 ladder to prove the gate early — §5.1):
  - **1 · Foundation — `[built]`.**
  - **2 · Temporal-resolve — `[absent]`.** No jitter / motion-vectors / Streamline / DLAA. Load-bearing, not polish: the hero shader's sun-glint is the textbook sub-pixel TAA-crawl the 240 Hz OLED exposes (§2.4 / KCD1). One of the two honest next steps.
  - **3 · Terrain + data plane — `[partial]`.** Real GEBCO renders, depth-correct, self-verified by framebuffer→PNG; but bathymetry loads *synchronously* at init (violates the §5.4 IO-off-render-thread guardrail) and the mesh is static full-res (no CDLOD/clipmap).
  - **4 · Ocean surface — `[partial]`.** A Gerstner prototype draws as a second pass over the seabed, and the **4.2 compute foundation is begun** (`src/render/ocean_compute.rs` — a compute pipeline writing an rgba16f displacement storage image through its own descriptor sets). The literal §5.2 target — cascaded Tessendorf FFT — is **4.2b/c, pending**. The other honest next step.
  ...
  - **6 · Atmosphere — `[partial]`.** The real **sun position** is grounded (`src/render/cosmos.rs`; NOAA/Meeus; Horizons-verified) and drives the shader; **Hillaire scattering LUTs are not built** — a true sun, not yet a scattered sky.
...
- **Cosmos dimension (§2.6):** astronomy half built; astrology half `[absent]` and the owner's to define.
- **Honest next compounding work:** rung 2 (temporal/DLAA scaffold) and rung 4 (Tessendorf FFT, hardening from the 4.2a foundation) — alongside the identity increment in flight (name + the §2.7 lens-set).
