...
*A navigation document for the world renderer. Written for Claudine to orient by at the start of each session — and for the human who owns the work.*
...
The **'Ethnic'** — non-negotiable, because it is the reason the project is worth doing at all:
...
**'2.2/The-HeroSubsystem/Ocean'** Two parts.
...
## (`4.5`/`·`/`The-Rung-2.5-Forks`/`Where-The-Water's-Next-Decisions-Live`)
 
*Written 2026-07-07, the day Rung 2.5 went live. Not a roadmap for these — a fork map, in the same spirit as §4: each is a gate, not a task. Do not open one before its gate is met; do not let its existence pressure a premature choice. This section is deliberately not a sequence — the forks below are lateral to each other, not staged, and several can sit gated indefinitely without that being failure.*
 
**(`Why-This-Belongs-Here`)** Rung 2.5 is not a detour from the A-C-A engine philosophy (`project_aca_engine_philosophy.md`) — it is the philosophy's clearest expression yet in the renderer. A-C-A reads *Astrology → Cosmology → Astronomy, inside-out*: meaning is owner-authored, but everything beneath meaning is *forced by the substrate*, never a free knob. A traced ray either finds the seabed or it doesn't; the engine cannot fake that the way a raster trick can. Rung 2.5 is the hero shader's own logic — *let the world's actual geometry answer, don't paint an answer over it* — applied one layer deeper, to reflections instead of to Beer–Lambert absorption. The forks below inherit that discipline: each is a place where the substrate, not convenience, gets to decide when it opens.
 
**(`Fork-III`/`Reflection-Geometry-Fidelity`)** flat-rest-state ocean BLAS (standing, shipped) ⟷ wave-displaced ocean BLAS (a compute pre-pass writes displaced positions each frame; the BLAS is rebuilt, not refit, from that buffer). *The tell is ocean self-reflection* — currently cull-masked out entirely (mask 0x02 excluded from the ray-gen's trace) rather than rendered wrong; bathymetry reflection is already full-fidelity and unaffected either way. **(`Gate`)** judged on the OLED, in motion, once there's been enough sailing time on Lagoon's water to know whether the absence of self-reflection actually reads as a gap — a screenshot cannot answer this, only living with it can. **(`Repo-Signature`)** `ray_tracing.rs`'s instance masks (0x01 bathymetry / 0x02 ocean) and `water_reflection.rgen`'s `cullMask` argument.
 
**(`Fork-IV`/`Reflection-Denoising`)** raw single deterministic mirror-ray (standing, shipped) ⟷ a DLSS Ray Reconstruction / DLSS-D FFI bridge (Part B, following `streamline_ffi.rs`'s existing DLAA-bridge shape). **(`Dormant-By-Design`), 'Not-Merely-Deferred'** — verified against 2026 DLSS 4.5 sources that a near-perfect mirror surface genuinely does not need a denoiser; the need only exists if the water ever samples multiple rough/glossy directions per pixel instead of one. **(`Gate`)** roughness gets added to the reflection model at all — until then this fork may never open, and that is a correct outcome, not an incomplete one. **(`Repo-Signature`)** `streamline_ffi.rs` exposes only DLAA bindings; `sl_dlss_d.h` sits vendored and unbound in the Streamline SDK.
 
**(`Fork-V`/`Hit-Shading-Fidelity`/`Opened`/`2026-07-07`)** flat vertex-color × basic N·L ⟷ the hero shader's full depth-driven Beer–Lambert + sand-bottom model, ported into `water_reflection.rchit`. Deliberately NOT the hero shader's final Fresnel/sky-reflection blend — that term is the water surface's own appearance from above; a single-bounce hit shader answers "what does the ray see having hit the seabed," not a second reflection layered on the first. Exposed-cay pixels keep the real per-vertex bathymetry color (matching water.frag's own asymmetry); underwater pixels use the fixed `SAND` reflectance attenuated by Beer–Lambert plus the water column's in-scatter glow — same constants, same citations (Williamson & Hollins 2022, Jerlov IB), hand-duplicated since this codebase has no shader `#include`. Sun intensity reaches the hit shader via a widened RT push-constant range (16→20 bytes) rather than resizing the widely-shared `FrameUniform` UBO — the smaller, local blast radius given this session's own earlier lesson about UBO-offset fragility. `cargo test` 51/51, `render-smoke` clean, real pixel delta this time (152021 vs 163308 bytes — expected, this is the one Rung 2.5 fork that changes what the reflection actually looks like rather than just its correctness). Readiest-of-three claim cashed in.
 
**(`Adjacent`/`Not-A-Fork`/`Resolved`/`2026-07-07`)** the atmosphere UBO offset bug turned out to be two bugs: the offset itself (`skyview.comp`/`multiscatter.comp` missing the two leading mat4s that `transmittance.comp` correctly reads), plus the real cause of the near-total-black regression the first fix attempt hit — `rayIntersectSphere` (all three shaders) always returned the far root of the ray-sphere quadratic, which is only correct for origin-inside-sphere queries; for the earth/`bottom_radius` clip it needed the near root, and the far root let the raymarch tunnel through solid earth into exploding negative-height density terms. Root-caused via a faithful numeric Python reproduction of the real shader math with the real runtime constants (not another blind edit-and-screenshot pass), fixed in all three shaders, and render-verified with `CHTHONIC_LENS=perspective` (the default isometric smoke camera never draws open sky, its background is a hardcoded clear color). See `reference_atmosphere_ubo_offset_bug.md` for the full diagnostic trail.
...
| ~~Should~~ **Done** *(2026-07-07)* | Atmosphere UBO offset bug | Was two bugs, not one — offset fix plus a `rayIntersectSphere` far/near-root bug in all 3 atmosphere shaders (the real cause of the earlier near-total-black regression). Root-caused via numeric reproduction, fixed, render-verified via perspective lens. See `reference_atmosphere_ubo_offset_bug.md`. |
...
- **(`Honest-Next-Compounding-Work`/`Ellipsoid-Sites-1-5-Complete`/`2026-06-26`)** Fork-I ENU pipeline sealed: bathymetry in WGS84 metres, ocean surface at ±200 km ENU, Y_SCALE=1.0, HORIZON_EYE ENU-local (beach level at Nassau), depth_m = −v_world_pos.y direct. SIGMA calibrated from measured Jerlov IB IOPs (Williamson & Hollins 2022, `38283a7c`). Shared-Sky invariant §2.3 complete (Hillaire + clouds, sealed 2026-06-25). **(`DR-Calibrated-2026-06-27`/`b8b7f60a`)**: SAND → vec3(0.150, 0.172, 0.097) (70% T.testudinum canopy + 30% carbonate sand; CoBOP/Voss 2003 Lee Stocking Island, Hill 2014, Mobley 2005 LUT, cover from Moritsch 2025 PMC12084626). WATER → vec3(0.006, 0.060, 0.185) (Jerlov IB R_inf ∝ b_b/a midpoint; Williamson-Hollins 2022 IOPs + Morel & Maritorena 2001 Case 1; G/B=0.32, R/B=0.032 — red near-zero at depth). **(`Shader-Source-Ledger`/`2026-06-27`/`af6838d5`)** Every numeric constant in the water rendering pipeline cited: `charts/shader-source-ledger.md` (SIGMA/SAND/WATER/Fresnel/lambert + WGS84 + Nassau anchor + lens + cascade patch sizes; atmospheric shader constants cited inline in the compute shaders separately). **(`Bathymetry-Gap3`/`merged-main`/`af6838d5`+`8369e8aa`)** NOAA NCEI ArcGIS ImageServer (DEM_all mosaic, ETOPO 2022 + GEBCO 2024 ~450 m native) via F32 GeoTIFF at 400×300=120,000 cells (~2 km/cell), 97× resolution increase over the OpenTopoData 56×22 sparse-sampling path. **(`Bathymetry-Composite`/`2026-06-27`)** NOAA base + GMRT topo-mask overlay (`--bathymetry-source=composite`): both fetched in parallel; 42.7% Bahamas cells (~51k/120k) replaced with GMRT ~100 m multibeam where valid; smoke PASS (screenshot 142 KB, richer depth variance). Production `charts/bathymetry.json` now composite source. **(`Copernicus-SDB-Plumbing`/`2026-06-27`)** `--bathymetry-source=copernicus` wired via copernicusmarine toolbox v2.4.1 (`uv run --with copernicusmarine`). Product `BATHYMETRY_GLO_PHY_COASTAL_L4_MY_016_001`, variable `height` (units: m, positive-up, matches NOAA/GMRT convention). `scripts/cm_sdb_fetch.py` handles credential resolution (env vars: `CMEMS_USER`/`CMEMS_PASS` → toolbox → stored login) + `open_dataset()` lazy subset + bilinear resample to W×H + JSON output. Missing credentials → exit 3 + clear diagnostic. Candidate outputs: `charts/bathymetry-copernicus.json` + `charts/bathymetry-copernicus-composite.json` + `charts/bathymetry-copernicus-report.md`. Production `bathymetry.json` NOT touched. **(`Direct-Zarr-Probe`/`2026-06-27`)** `--probe-direct-zarr` added to `cm_sdb_fetch.py`. Probe confirmed: static-ARCO Zarr URI `https://s3.waw3-1.cloudferro.com/mdl-arco-time-003/arco/.../static.zarr`. Metadata plane = public (`.zmetadata` HTTP 200, `height` var present, lat/lon coords visible). Data plane = auth-gated (`height/0.0` chunk → HTTP 403). Verdict: **direct Zarr is metadata-only without credentials; data always requires auth via the copernicusmarine toolbox path.** **(`Secchi-Depth-Verified`/`2026-07-05`)** Checked numerically rather than tuned: `floor_vis` (the luma-weighted round-trip transmittance driving the floor/water-glow mix in `water.frag`) computed across depth using the live SIGMA constants — 100% at 0m, 4.35% at 20m, 2.25% at 25m, 1.63% at 27.5m, 1.18% at 30m. The floor reads as effectively invisible across the entire Jerlov IB Secchi range (~25–30m) with no separate threshold constant needed — the existing DR-calibrated SIGMA already produces correct Secchi behavior as an emergent property. Comment + the table now live at `assets/shaders/water.frag` next to the `floor_vis` computation. This closes the item — no shader change was needed, only the verification.

**(`Copernicus-SDB-E2E-Verified`/`2026-07-05`)** The credential gate is closed: owner registered at marine.copernicus.eu, ran `scripts/setup-copernicus-creds.ps1` (masked prompt, persists `CMEMS_USER`/`CMEMS_PASS` to the Windows user profile — also mirrored into `~/.chthonic/api_pool.json`), and the full chain ran end-to-end via `bun run CLAUDEBASE/quarterdeck/barometer.ts --bathymetry --bathymetry-source=copernicus`: auth → dataset open (`cmems_obs-sdb_glo_phy_comp_my_100m-l4-s2_static`, shape 6261×6851) → resample to 400×300 → fused candidate. One real bug hit and fixed along the way: `cm_sdb_fetch.py`'s final print crashed on a `→` character under `uv run`'s default cp1252 stdout on Windows — fixed by setting `PYTHONUTF8=1` (same class of issue as `reference_python_encoding_windows` memory). Result: 26,508/120,000 cells valid (22.1%) — **not** lower-quality than the NOAA+GMRT composite's 100%, but a different kind of source: SDB is optically-shallow-only (26,504 of those cells are in the 0–30m band), so it's precision-shallow, not full-depth. Candidate outputs at `charts/bathymetry-copernicus.json` / `charts/bathymetry-copernicus-composite.json` / `charts/bathymetry-copernicus-report.md`; production `bathymetry.json` untouched by design.

**(`Bathymetry-Path-Override`/`2026-07-05`)** `main.rs` now reads `CHTHONIC_BATHYMETRY_PATH` (falls back to `DEFAULT_PATH`) so a candidate dataset can be smoke-tested without touching production — added specifically to test the Copernicus composite safely.

**(`Visual-Gate-Result`/`2026-07-05`)** Ran `render-smoke.ps1` against both production and the Copernicus-fused composite, then pixel-diffed the two screenshots (not just eyeballed): 4,765/921,600 pixels differ (0.52%), concentrated entirely in the shallow-water region Copernicus actually covers, tracing coastline/reef-like shapes — a real, spatially-coherent signal, not noise. **'Also-Surfaced/A-Pre-Existing/Unrelated Bug'** both renders show an identical hard checkerboard/grid artifact in the CDLOD terrain — confirmed pre-existing (present with zero diff between the two conditions), **not** caused by the Copernicus work. Not yet root-caused (candidate: CDLOD clipmap seam/moiré, unconfirmed). **'Promotion-To-Poduction-Is-Deliberately-Deferred'** — not because Copernicus looks wrong, but because the checkerboard bug makes a clean visual read of the water surface unreliable, and the report's own gate ("promote only after visual confirmation") wasn't cleanly met. Re-attempt promotion once the checkerboard is root-caused.

**(`Checkerboard-Root-Cause-And-Fix`/`2026-07-06`)** Root-caused and fixed. Falsifier first: forced `clipmap_grid(1, 32)` (single LOD level, zero ring boundaries) — identical artifact persisted, ruling out CDLOD seams entirely. Traced instead to the ocean surface's 2-cascade Tessendorf FFT pipeline (`ocean_h0.comp` → `ocean_evolve.comp` → `ocean_fft.comp` → `water.vert`), two independent bugs: (1) **'Dominant-Cause'** — `water.vert` never referenced each cascade's `patch_size` (C0=5m, C1=60m) in its UV computation, sampling both displacement textures over the full 200km world extent instead of tiling every `patch_size` metres (sampler was already `REPEAT`-wrapped, just never given a UV that exceeds `[0,1]`) — the raw 256×256 FFT texture's own low-frequency structure was directly visible, magnified ~40,000× for C0 alone; fixed by computing `uv_cN = world_xz / patch_size_cN` per cascade. (2) A secondary, independently-real bug: `ocean_fft.comp`'s butterfly runs forward-indexed (0..N-1) but `ocean_evolve.comp` synthesizes a centered spectrum (DC at texel N/2), an indexing mismatch every Tessendorf-FFT reference corrects with a `(-1)^(x+z)` sign flip on the final pass — added, verified as a real (non-zero) effect independent of fix (1). Verified via pixel-diff at each step; single-LOD test isolated (1), then (1) vs (1)+(2) isolated via `git stash` to confirm both are genuine, not redundant. Files: `assets/shaders/water.vert`, `assets/shaders/ocean_fft.comp`.

**(`Copernicus-Production-Promotion`/`2026-07-06`)** Re-ran the 2026-07-05 visual gate with the checkerboard confound gone: production vs. Copernicus-fused composite now diff at 2,586/921,600 pixels (0.28%, down from the confounded 0.52%), bounding box `x:419–1201, y:272–586` — bounded to the shallow-water region Copernicus covers, no checkerboard noise mixed in. Clean. Promoted: `CLAUDEBASE/charts/bathymetry-copernicus-composite.json` → `CLAUDEBASE/charts/bathymetry.json` (pre-promotion production backed up to `bathymetry-pre-copernicus-backup.json`). Verified: rebuilt with no `CHTHONIC_BATHYMETRY_PATH` override, screenshot byte-identical to the Copernicus-candidate render (145,460 bytes) — production now serves the fused dataset by default.

**(`Rung-2.5-RT-Reflections-Live`/`2026-07-07`)** Hardware ray-traced water reflections landed — `VK_KHR_ray_tracing_pipeline`/`acceleration_structure`/`deferred_host_operations` (Khronos-standard, cross-vendor), replacing the purely analytic sky-only reflection with real bathymetry visibility. Six phases (BLAS/TLAS → shaders → pipeline+SBT → render-loop dispatch), each verified via `render-smoke.ps1`, plus an independent adversarial review before the dispatch phase that caught and fixed four real issues (SBT alignment, a resize-safety gap, ocean self-hit speckle, a wrong claim in an earlier comment) — see `project_rung_25_rt_reflections_checkpoint.md` in Claude's memory for the full account. First live pixel-diff: 13,594/921,600 pixels differ (1.48%), bounded almost exactly to the shallow-water/bathymetry region, **'Zero-Diff-Pixels/In-Deep-Water'** (explicitly checked) — a spatially coherent, geometry-correlated signal, not noise. Deliberately deferred: DLSS Ray Reconstruction bridge (not needed — single deterministic mirror-ray, no denoiser required per 2026 DLSS 4.5 sources), displaced-ocean-BLAS (ocean self-reflection currently cull-masked out), full Beer-Lambert hit-shading (flat albedo × N·L for now).

**(`Remaining-Open`)**: §2.6 astrology half (owner-defined, never invented). Site 6 deferred (ELLIPSOID-RETROFIT tags planted). A pre-existing, unrelated bug surfaced during the Rung 2.5 review: `skyview.comp`/`multiscatter.comp` read the shared frame UBO at the wrong byte offset (missing two leading mat4s `transmittance.comp` correctly includes) — sky/atmosphere rendering may have run on garbage sun-direction/scattering constants; not yet fixed.
...
| Shader validation | `glslangValidator` (Vulkan SDK), `--target-env vulkan1.3` — wired into `ci/checks/glsl-lint.ts` |
| Shader code sharing | `glslc`'s native `#include` (same-dir relative, verified needs no flags); `shaderc`-crate fallback via `set_include_callback` in `build.rs`, for backend parity |
