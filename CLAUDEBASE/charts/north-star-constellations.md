...
- **(`Honest-Next-Compounding-Work`/`Ellipsoid-Sites-1-5-Complete`/`2026-06-26`)** Fork-I ENU pipeline sealed: bathymetry in WGS84 metres, ocean surface at ±200 km ENU, Y_SCALE=1.0, HORIZON_EYE ENU-local (beach level at Nassau), depth_m = −v_world_pos.y direct. SIGMA calibrated from measured Jerlov IB IOPs (Williamson & Hollins 2022, `38283a7c`). Shared-Sky invariant §2.3 complete (Hillaire + clouds, sealed 2026-06-25). **(`DR-Calibrated-2026-06-27`/`b8b7f60a`)**: SAND → vec3(0.150, 0.172, 0.097) (70% T.testudinum canopy + 30% carbonate sand; CoBOP/Voss 2003 Lee Stocking Island, Hill 2014, Mobley 2005 LUT, cover from Moritsch 2025 PMC12084626). WATER → vec3(0.006, 0.060, 0.185) (Jerlov IB R_inf ∝ b_b/a midpoint; Williamson-Hollins 2022 IOPs + Morel & Maritorena 2001 Case 1; G/B=0.32, R/B=0.032 — red near-zero at depth). **(`Remaining-Open`)**: Secchi-depth mix() scale (IB Secchi ≈25–30 m → floor_vis threshold tuning; open). Bathymetry resolution: current pipeline sparse-samples GEBCO at 0.3° (~67× decimation of native 15" source); upgrade tiers — (1) GEBCO OPeNDAP at native 15"=450 m (free, immediate); (2) GMRT GridServer topo-mask bbox query up to ~100 m/node (no-key, test first); (3) Copernicus Marine `cmems_obs-sdb_glo_phy_comp_my_100m-l4-s2_static` Sentinel-2 SDB (100 m, free+login) or TCarta ArcGIS SDB (ArcGIS item f1880f788e384d55af2eaf353d154c08). §2.6 astrology half (owner-defined, never invented). Rung 2.5 RT reflections (optional). Site 6 deferred (ELLIPSOID-RETROFIT tags planted).
