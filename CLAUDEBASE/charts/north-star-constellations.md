...
- **Rung ledger — ANNO 2026-06-10, amended 2026-06-17** (we leapt the §5.2 ladder to prove the gate early — §5.1; the 2026-06-17 amendment submerges the temporal scaffold + the full celestial field into the chart — see [`../logbook/03-celestial-field.md`](../logbook/03-celestial-field.md) and [`../logbook/04-structurize-submerge.md`](../logbook/04-structurize-submerge.md)):
  ...
  - **3 · Terrain + data plane — `[IO fixed · no CDLOD]`.** Real GEBCO renders, depth-correct, self-verified by framebuffer→PNG. **IO guardrail met** (`std::thread::spawn` + join in `main.rs` pre-loads the mesh before `Renderer::new` — the render thread receives finished vertices, never touches disk). Remaining: CDLOD/clipmap LOD (mesh is static full-res).
  - **4 · Ocean surface — `[4.2c IFFT live · single-cascade]`.** The compute pipeline (`ocean_compute.rs`) drives the full Tessendorf chain per frame: **h0(k)** initial Phillips spectrum generated once at init (`ocean_h0.comp`, Box-Muller Gaussian + wind-directed `P(k)`); **evolve** `h(k,t) = h0·e^(iωt) + h0(-k)*·e^(-iωt)` per frame (`ocean_evolve.comp`); **16-pass butterfly IFFT** (8 horizontal + 8 vertical Cooley-Tukey radix-2 DIT, `ocean_fft.comp`) ping-ponging between two intermediate images; final stage writes into `disp_image` which `water.vert` samples as before. Self-verified: `renders/4-2c-fft.png` — spectral energy concentration and wind-directional lateral displacement visible. Remaining: CDLOD mesh (static full-res still), second cascade (swell vs ripple), GPU allocation via `gpu-allocator` (currently hand-rolled).
...
- **Honest next compounding work:** rung 2's remaining **DLAA** consumer (the jitter + motion-vector scaffold is now built); rung 4 second cascade + `gpu-allocator` migration; rung 3 CDLOD mesh; the §2.7 perspective lens — alongside the §2.6 astrology half (owner-defined, attach-points named in `celestial-field.md`).
