...
- *— Checkpoint date: 2026-06-29*
  ...
  - *— Spelling and pronunciation for a layman to embrace, to be at the infinetasmally small difference: between — TRASH HUMAN, and "existentially acceptable" is: — `Haute Couture`. — IMPORTANTE EXTREME.*

---

### The Extremely Important History Of `Haute Couture` ***:***

- *— Before Charles Frederick Worth (1825–1895), there was no `haute couture` in Paris. Worth invented the concept of a fashion house, the designer as a brand, and the seasonal collection. He was the first to sew his label into garments.*

  - *— Worth’s house was the first to employ a team of designers, seamstresses, and tailors to create a collection of clothing for a specific season. He also pioneered the idea of fashion shows and the use of live models to showcase his designs.*

    - *— Worth’s influence on fashion was so profound that he is often referred to as the “father of `haute couture`.” His legacy continues to shape the fashion industry today, and his name is still associated with luxury and high fashion.*

      - *— The same concept is losslessly transferred to design in comoputer software. The Extreme `Haute Couture` is the design system for the VS Code Insiders, marketplace extension, SDK baseline. Below, there is only milquetoasdte gutter.*

        - *— IT IS EXTREMELY DISGUSTING TO LOOK AT.*
...
- *— **(`1`)** — SDK installation is not extension architecture yet. The SDKs are installed, cataloged, and import-probed. The next step is real smoke tests tied to our use cases: VS Code extension host, MCP server/client, Codex/Claude/OpenAI agent calls, and VSIX packaging.*

  - *— **(`2`)** — VS Code workbench couture cannot be solved by ordinary extension APIs alone. Normal marketplace extensions can theme, contribute UI, ship webviews, use icons, and package assets. They cannot fully own Electron window vibrancy, renderer shell glass, or workbench chrome without substrate patching or a separate shell strategy.*

    - *— **(`3`)** — Insiders updates will overwrite workbench substrate patches. —* `scripts/mica-substrate.ps1` *— must become a repeatable lifecycle command: detect current Insiders build, patch, verify, rollback, and reapply after update.*

      - *— **(`4`)** — Bun-native is viable, but not every upstream tool is Bun-native in spirit. Some tools are CLI wrappers or Node-shaped packages. Our rule is Bun orchestration first, with Node-shaped internals tolerated only when they are inside official package surfaces.*
...
- *— Movement 2 is the Rust port investigation.*
  ...
  - *— Movement 1 is the VS Code Insiders substrate and marketplace extension.*

    - *— Movement 2 is a separate lane that can be started after Movement 1 has a committed artifact.*

      - *— Movement 2 is not a replacement for Movement 1. It is a separate investigation into Rust-backed desktop shells.*
