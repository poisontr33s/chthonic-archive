...
</article>`}).join("")}function c(j,Q){let J=q.workspace.workspaceFolders?.[0];if(!J)return'<html><body style="font-family: sans-serif; padding: 20px;"><h1>No workspace folder found</h1></body></html>';let K=E.join(J.uri.fsPath,"dependency_graph.json");if(!z.existsSync(K))return`<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
<body>
    <div class="shell">
        ...
        <section class="grid">
            ${Z.map(([X,$])=>{let C=$.slice(0,4).map((M)=>`<li>${G(M)}</li>`).join(""),L=$.length>4?`<li>+ ${$.length-4} more</li>`:"";return`<article class="node">
...
</html>`}function i(){if(!q.workspace.workspaceFolders?.[0])return'<html><body style="font-family: sans-serif; padding: 20px;"><h1>No workspace folder found</h1></body></html>';return`<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
