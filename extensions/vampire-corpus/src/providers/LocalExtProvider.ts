...
// Tree item kinds
// ---------------------------------------------------------------------------
type ExtItemKind = 'localExt.installed' | 'localExt.notInstalled' | 'localExt.section' | 'localExt.error';

class LocalExtItem extends vscode.TreeItem {
    readonly extInfo: LocalExtInfo | null;

    constructor(
        label: string,
        collapsible: vscode.TreeItemCollapsibleState,
        kind: ExtItemKind,
        info: LocalExtInfo | null = null,
        opts: { description?: string; tooltip?: string | vscode.MarkdownString; icon?: string } = {},
    ) {
...
// Provider
// ---------------------------------------------------------------------------
export class LocalExtProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
    private _onDidChange = new vscode.EventEmitter<vscode.TreeItem | undefined>();
    readonly onDidChangeTreeData = this._onDidChange.event;

    private extensions: LocalExtInfo[] = [];
    ...
    private scan(): void {
        const extsDirRel = vscode.workspace.getConfiguration('vampireCorpus').get<string>('extensionsDir') ?? 'extensions';
        ...
        try {
            ...
            for (const entry of entries) {
                ...
                try {
                    manifest = JSON.parse(fs.readFileSync(pkgPath, 'utf8')) as ExtManifest;
                } catch {
                    continue;
                }
    ...
    private makeItem(info: LocalExtInfo): LocalExtItem {
        ...
        const kind: ExtItemKind = info.isInstalled ? 'localExt.installed' : 'localExt.notInstalled';
    ...
    async installExt(item: unknown): Promise<void> {
    ...
    async uninstallExt(item: unknown): Promise<void> {
        const info = this.extractInfo(item);
        if (!info) {
        ...
        const confirmed = await vscode.window.showWarningMessage(
            `Uninstall "${label}" (${info.extensionId})?`,
            { modal: false },
            'Uninstall',
        );
    ...
    openFolder(item: unknown): void {
        const info = this.extractInfo(item);
        if (!info) return;
        void vscode.commands.executeCommand('revealFileInOS', vscode.Uri.file(info.folderPath));
    }

    private extractInfo(item: unknown): LocalExtInfo | null {
        if (item instanceof LocalExtItem) return item.extInfo;
        return null;
    }
}
...
function resolveCodeCli(): string {
