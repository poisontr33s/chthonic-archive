...
// ---------------------------------------------------------------------------
// Types matching manifest/corpus-state.json
// ---------------------------------------------------------------------------
interface CorpusState {
    ...
    sessions?: number;
    entities?: number;
    entity_occurrences?: number;
    gate_ladder?: Record<string, boolean>;
    vec_count?: number;
    vec_model?: string;
    vec_dims?: number;
    satellites?: string[];
...
interface SatelliteJson {
    ...
    version?: string;
    description?: string;
    drain_format?: { type?: string; path?: string; schema_version?: number };
    tables?: string[];
    federation_keys?: string[];
}

// ---------------------------------------------------------------------------
// Item kinds for contextValue (used in package.json menus)
// ---------------------------------------------------------------------------
type ItemKind =
    | 'section'
    | 'corpus.stat'
...
class CorpusItem extends vscode.TreeItem {
    constructor(
        ...
        kind: ItemKind,
        opts: { description?: string; tooltip?: string; icon?: string; command?: vscode.Command } = {},
    ) {
        super(label, collapsible);
        this.contextValue = kind;
        if (opts.description !== undefined) this.description = opts.description;
        if (opts.tooltip !== undefined) this.tooltip = opts.tooltip;
...
// ---------------------------------------------------------------------------
// Section root items (collapsible)
// ---------------------------------------------------------------------------
type SectionId = 'corpus' | 'gates' | 'satellites';

class SectionItem extends vscode.TreeItem {
    readonly sectionId: SectionId;

    constructor(label: string, sectionId: SectionId, description?: string) {
        super(label, vscode.TreeItemCollapsibleState.Expanded);
        this.sectionId = sectionId;
        this.contextValue = 'section';
        if (description) this.description = description;
        switch (sectionId) {
            case 'corpus': this.iconPath = new vscode.ThemeIcon('database'); break;
            case 'gates': this.iconPath = new vscode.ThemeIcon('symbol-boolean'); break;
            case 'satellites': this.iconPath = new vscode.ThemeIcon('remote-explorer'); break;
        }
    }
}
...
// Provider
// ---------------------------------------------------------------------------
export class CorpusTreeProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
    ...
    private load(): void {
        try {
            ...
            this.state = JSON.parse(fs.readFileSync(this.statePath, 'utf8')) as CorpusState;
            this.loadError = null;
        } catch (e) {
            this.state = null;
    ...
    private getRoots(): vscode.TreeItem[] {
        if (this.loadError || !this.state) {
            const err = new CorpusItem(
                'corpus-state.json not found',
                vscode.TreeItemCollapsibleState.None,
                'error',
                { icon: 'warning', description: 'Run vampire:drain to initialize' },
            );
            return [err];
        }
    ...
    private getSectionChildren(section: SectionId): vscode.TreeItem[] {
    ...
    private getCorpusStats(): vscode.TreeItem[] {
        const s = this.state!;
        const items: vscode.TreeItem[] = [];

        const addStat = (label: string, value: string | number | undefined, icon: string) => {
            const item = new CorpusItem(
                label,
                vscode.TreeItemCollapsibleState.None,
                'corpus.stat',
                { description: String(value ?? '—'), icon },
            );
            items.push(item);
        };
    ...
    private getGates(): vscode.TreeItem[] {
        ...
        return allGates.map(key => {
            ...
            const kind: ItemKind = admitted ? 'gate.admitted' : 'gate.failed';
    ...
    private getSatellites(): vscode.TreeItem[] {
        ...
        return satellitePaths.map(relPath => {
            ...
            try {
                const sat = JSON.parse(fs.readFileSync(fullPath, 'utf8')) as SatelliteJson;
