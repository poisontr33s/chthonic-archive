...
</html>`}}function qe(){let e="";for(let i=0;i<32;i+=1)e+="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(Math.random()*62));return e}var ht=h(require("fs/promises")),L=h(require("path")),pt=h(require("vscode"));var Ot=h(require("crypto"));class nt{leaves=new Map;dirty=!1;upsert(t){let e=Ue(t.path),i=He(e,t);if(this.leaves.get(e)!==i)return this.leaves.set(e,i),this.dirty=!0,!0;return!1}hasDirty(){return this.dirty}settle(t){if(!this.dirty||this.leaves.size===0)return null;let e=Oe(Array.from(this.leaves.entries()).sort((i,o)=>i[0].localeCompare(o[0])));return this.dirty=!1,{reason:t,rootHash:e,leafCount:this.leaves.size,generatedAt:Date.now()}}}function He(t,e){let i=[t,e.entropy.toFixed(6),e.complexity,e.debt,e.freshness.toFixed(6),e.ruffViolations,e.updatedAt].join("|");return z(i)}function Oe(t){if(t.length===0)return z("EMPTY");let e=t.map(([i,o])=>z(`${i}:${o}`));while(e.length>1){let i=[];for(let o=0;o<e.length;o+=2){let s=e[o],a=e[o+1]??e[o];i.push(z(`${s}${a}`))}e=i}return e[0]}function z(t){return Ot.createHash("sha256").update(t,"utf8").digest("hex")}function Ue(t){return t.replace(/\\/g,"/")}var M=h(require("fs")),S=h(require("path")),rt=require("child_process"),W=h(require("vscode"));function D(t,e){let i="",o=(s)=>{let a=s.trim();if(!a)return;try{let l=JSON.parse(a);if(!l||typeof l!=="object"||Array.isArray(l))throw Error("JSONL payload must be an object");t(l)}catch(l){e(l instanceof Error?l:Error(String(l)))}};return{push(s){i+=s.toString();while(!0){let a=i.indexOf(`
`);if(a<0)return;let l=i.slice(0,a);i=i.slice(a+1),o(l)}},flush(){if(!i.trim()){i="";return}o(i),i=""}}}var __dirname="C:\\Users\\erdno\\chthonic-archive\\extensions\\chthonic-archive\\src\\polyglot";class at{output;rootPath=null;sidecarRootPath=null;pythonSidecar=null;rubySidecar=null;onDidReceiveRuffEmitter=new W.EventEmitter;onDidReceiveRuff=this.onDidReceiveRuffEmitter.event;onDidReceiveLoreEmitter=new W.EventEmitter;onDidReceiveLore=this.onDidReceiveLoreEmitter.event;onDidReceiveSidecarErrorEmitter=new W.EventEmitter;onDidReceiveSidecarError=this.onDidReceiveSidecarErrorEmitter.event;constructor(t){this.output=t}start(t){if(this.rootPath=t,this.sidecarRootPath=Ne(t),!this.sidecarRootPath){let i=`sidecar scripts not found. checked: ${Ut(t).join(", ")}`;this.output.appendLine(`[polyglot] ${i}`),this.fireSidecarError("python",i),this.fireSidecarError("ruby",i);return}this.output.appendLine(`[polyglot] sidecar root: ${this.sidecarRootPath}`),this.startPythonSidecar(),this.startRubySidecar()}requestScan(t){if(!this.pythonSidecar||this.pythonSidecar.killed)this.startPythonSidecar();this.writeJson(this.pythonSidecar,t)}requestLore(t){if(!this.rubySidecar||this.rubySidecar.killed)this.startRubySidecar();this.writeJson(this.rubySidecar,t)}dispose(){this.pythonSidecar?.kill(),this.rubySidecar?.kill(),this.pythonSidecar=null,this.rubySidecar=null,this.sidecarRootPath=null,this.onDidReceiveRuffEmitter.dispose(),this.onDidReceiveLoreEmitter.dispose(),this.onDidReceiveSidecarErrorEmitter.dispose()}startPythonSidecar(){if(!this.rootPath||this.pythonSidecar)return;if(!this.sidecarRootPath){this.fireSidecarError("python","sidecar root unresolved; python scan unavailable.");return}let t=S.join(this.sidecarRootPath,"python","entropy_scan.py");if(!M.existsSync(t)){this.fireSidecarError("python",`python sidecar missing: ${t}`);return}let e=S.join(this.sidecarRootPath,"venv",process.platform==="win32"?"Scripts":"bin"),i=S.join(e,process.platform==="win32"?"python.exe":"python"),o=M.existsSync(i),s=o?i:"uv",a=o?[t,"--stdio"]:["run",t,"--stdio"];if(!o)this.output.appendLine("[polyglot:python] .chthonic/venv not found, falling back to uv run.");this.pythonSidecar=rt.spawn(s,a,{cwd:this.rootPath,stdio:["pipe","pipe","pipe"],env:Ie(process.env,e)});let l=D((c)=>this.handlePythonPayload(c),(c)=>this.output.appendLine(`[polyglot:python] invalid JSONL payload: ${c.message}`));this.pythonSidecar.stdout?.on("data",(c)=>l.push(c)),this.pythonSidecar.stderr?.on("data",(c)=>{this.output.appendLine(`[polyglot:python] ${c.toString().trimEnd()}`)}),this.pythonSidecar.on("error",(c)=>{this.fireSidecarError("python",`failed to spawn: ${c.message}`),this.pythonSidecar=null}),this.pythonSidecar.on("exit",(c)=>{l.flush(),this.output.appendLine(`[polyglot:python] exited with code ${c??-1}`),this.pythonSidecar=null})}startRubySidecar(){if(!this.rootPath||this.rubySidecar)return;if(!this.sidecarRootPath){this.fireSidecarError("ruby","sidecar root unresolved; lore unavailable.");return}let t=S.join(this.sidecarRootPath,"ruby","lore.rb");if(!M.existsSync(t)){this.fireSidecarError("ruby",`ruby sidecar missing: ${t}`);return}this.rubySidecar=rt.spawn("ruby",[t],{cwd:this.rootPath,stdio:["pipe","pipe","pipe"]});let e=D((i)=>this.handleRubyPayload(i),(i)=>this.output.appendLine(`[polyglot:ruby] invalid JSONL payload: ${i.message}`));this.rubySidecar.stdout?.on("data",(i)=>e.push(i)),this.rubySidecar.stderr?.on("data",(i)=>{this.output.appendLine(`[polyglot:ruby] ${i.toString().trimEnd()}`)}),this.rubySidecar.on("error",(i)=>{this.fireSidecarError("ruby",`failed to spawn: ${i.message}`),this.rubySidecar=null}),this.rubySidecar.on("exit",(i)=>{e.flush(),this.output.appendLine(`[polyglot:ruby] exited with code ${i??-1}`),this.rubySidecar=null})}writeJson(t,e){if(!t?.stdin||t.killed)return;try{t.stdin.write(`${JSON.stringify(e)}
`)}catch(i){this.output.appendLine(`[polyglot] sidecar write failed: ${Je(i)}`)}}fireSidecarError(t,e){this.onDidReceiveSidecarErrorEmitter.fire({type:"error",source:t,message:e}),this.output.appendLine(`[polyglot:${t}] ${e}`)}handlePythonPayload(t){if(t.type==="ruff-summary"){let e=t;this.onDidReceiveRuffEmitter.fire(e);return}if(t.type==="error"){let e=String(t.message??"python sidecar error");this.output.appendLine(`[polyglot:python] ${e}`),this.onDidReceiveSidecarErrorEmitter.fire({type:"error",source:"python",message:e})}}handleRubyPayload(t){if(t.type==="lore"){this.onDidReceiveLoreEmitter.fire(t);return}if(t.type==="error"){let e=String(t.message??"ruby sidecar error");this.output.appendLine(`[polyglot:ruby] ${e}`),this.onDidReceiveSidecarErrorEmitter.fire({type:"error",source:"ruby",message:e})}}}function Je(t){if(t instanceof Error)return`${t.name}: ${t.message}`;return String(t)}function Ne(t){for(let e of Ut(t))if(M.existsSync(S.join(e,"python","entropy_scan.py"))&&M.existsSync(S.join(e,"ruby","lore.rb")))return e;return null}function Ut(t){let e=[S.join(t,".chthonic"),S.join(t,"extensions","chthonic-archive",".chthonic"),S.resolve(__dirname,"..",".chthonic")];return Array.from(new Set(e))}function Ie(t,e){if(!M.existsSync(e))return t;let i=process.platform==="win32"?"Path":"PATH",o=t[i]??process.env[i]??"";return{...t,[i]:o?`${e}${S.delimiter}${o}`:e}}var Jt=h(require("fs")),Y=h(require("fs/promises")),$=h(require("path")),lt=require("child_process");class ct{output;rootPath=null;options={rpcUrl:"http://127.0.0.1:8899",autostartValidator:!1};ledgerFilePath=null;validatorProcess=null;hostProcess=null;requestCounter=1;pending=new Map;constructor(t){this.output=t}async start(t,e){this.rootPath=t,this.options=e;let i=$.join(t,".chthonic","ledger");if(await Y.mkdir(i,{recursive:!0}),this.ledgerFilePath=$.join(i,"entropy-settlements.jsonl"),e.autostartValidator)this.startValidator();this.startHostProcess()}async commitEntropy(t){if(!this.rootPath||!this.ledgerFilePath)return{mode:"offline",detail:"Ledger host not started"};let e={...t,rpcUrl:this.options.rpcUrl,recordedAt:Date.now()},i={mode:"offline",detail:"Rust ledger host unavailable; persisted settlement locally."};try{i=await this.submitToHost(t)}catch(o){this.output.appendLine(`[ledger-rust] submit failed: ${ze(o)}`)}return await Y.appendFile(this.ledgerFilePath,`${JSON.stringify({...e,receipt:i})}
`,"utf8"),i}dispose(){for(let[t,e]of this.pending)e.reject(Error(`request ${t} cancelled`));this.pending.clear(),this.hostProcess?.kill(),this.hostProcess=null,this.validatorProcess?.kill(),this.validatorProcess=null}startHostProcess(){if(!this.rootPath||this.hostProcess)return;let t=Ve(this.rootPath,this.options.hostBinaryPath),e=this.options.walletPath??$.join(this.rootPath,".chthonic","wallets","payer.json"),i=this.options.idlPath??$.join(this.rootPath,".chthonic","wallets","entropy_ledger.json");this.hostProcess=lt.spawn(t,["--wallet",e,"--idl",i,"--rpc-url",this.options.rpcUrl],{cwd:this.rootPath,stdio:["pipe","pipe","pipe"]});let o=D((s)=>this.handleHostPayload(s),(s)=>this.output.appendLine(`[ledger-rust] invalid JSON payload: ${s.message}`));this.hostProcess.stdout?.on("data",(s)=>o.push(s)),this.hostProcess.stderr?.on("data",(s)=>{this.output.appendLine(`[ledger-rust] ${s.toString().trimEnd()}`)}),this.hostProcess.on("error",(s)=>{this.output.appendLine(`[ledger-rust] failed to spawn host: ${s.message}`),this.rejectAllPending(Error(`host spawn failed: ${s.message}`)),this.hostProcess=null}),this.hostProcess.on("exit",(s)=>{o.flush(),this.output.appendLine(`[ledger-rust] host exited with code ${s??-1}`),this.rejectAllPending(Error("ledger host exited")),this.hostProcess=null})}async submitToHost(t){if(!this.hostProcess||this.hostProcess.killed)this.startHostProcess();if(!this.hostProcess?.stdin||this.hostProcess.killed)return{mode:"offline",detail:"Rust host binary is missing or not executable."};let e=this.requestCounter++,i={jsonrpc:"2.0",id:e,method:"submit_entropy",params:{entropy_score:Math.max(0,Math.round(t.leafCount*17+13)),merkle_root:t.rootHash,leaf_count:t.leafCount,reason:t.reason}};return await new Promise((s,a)=>{this.pending.set(e,{resolve:s,reject:a}),this.hostProcess?.stdin?.write(`${JSON.stringify(i)}
`,(l)=>{if(!l)return;this.pending.delete(e),a(l)})})}handleHostPayload(t){let e=t.id;if(typeof e!=="number")return;let i=this.pending.get(e);if(!i)return;if(this.pending.delete(e),t.error&&typeof t.error==="object"){let s=t;i.resolve({mode:"offline",detail:`Rust ledger error ${s.error.code}: ${s.error.message}`});return}let o=t;i.resolve({mode:"validator-rust",txSignature:o.result.signature,detail:`Rust anchor-client submission accepted${o.result.slot?` (slot ${o.result.slot})`:""}.`})}rejectAllPending(t){for(let[e,i]of this.pending)this.pending.delete(e),i.reject(t)}startValidator(){if(!this.rootPath||this.validatorProcess)return;let t=Ge(this.rootPath),e=Fe(this.options.rpcUrl)??8899,i=$.join(this.rootPath,".chthonic","solana-ledger");this.validatorProcess=lt.spawn(t,["--ledger",i,"--rpc-port",String(e),"--reset","--quiet"],{cwd:this.rootPath,stdio:["ignore","pipe","pipe"]}),this.validatorProcess.stdout?.on("data",(o)=>{this.output.appendLine(`[solana] ${o.toString().trimEnd()}`)}),this.validatorProcess.stderr?.on("data",(o)=>{this.output.appendLine(`[solana] ${o.toString().trimEnd()}`)}),this.validatorProcess.on("error",(o)=>{this.output.appendLine(`[solana] validator spawn error: ${o.message}`),this.validatorProcess=null}),this.validatorProcess.on("exit",(o)=>{this.output.appendLine(`[solana] validator exited with code ${o??-1}`),this.validatorProcess=null})}}function Ve(t,e){if(e)return e;let i=process.platform==="win32"?"entropy-ledger-host.exe":"entropy-ledger-host";return $.join(t,"native","target","release",i)}function Ge(t){let e=process.platform==="win32"?"solana-test-validator.exe":"solana-test-validator",i=$.join(t,".chthonic","bin",e);if(Jt.existsSync(i))return i;return process.platform==="win32"?"solana-test-validator":e}function Fe(t){try{let e=new URL(t);if(!e.port)return null;let i=Number(e.port);return Number.isFinite(i)?i:null}catch{return null}}function ze(t){if(t instanceof Error)return`${t.name}: ${t.message}`;return String(t)}class dt{output;options;backend=null;constructor(t,e){this.output=t;this.options=e}async start(t){this.backend=this.options.mode==="bankrun"?new Nt(this.output):new It(this.output,this.options),await this.backend.start(t)}async commitEntropy(t){if(!this.backend)return{mode:"offline",detail:"LedgerBroker backend not initialized."};return this.backend.commitEntropy(t)}dispose(){this.backend?.dispose(),this.backend=null}}class Nt{output;sequence=0;constructor(t){this.output=t}async start(t){this.output.appendLine("[ledger] phantom mode active (bankrun simulation).")}async commitEntropy(t){return this.sequence+=1,{mode:"bankrun",txSignature:`bankrun-${t.rootHash.slice(0,20)}-${this.sequence}`,detail:"In-memory bankrun simulation accepted the entropy settlement."}}dispose(){}}class It{options;client;constructor(t,e){this.options=e;this.client=new ct(t)}async start(t){await this.client.start(t,{rpcUrl:this.options.rpcUrl,autostartValidator:this.options.autostartValidator,hostBinaryPath:this.options.hostBinaryPath,walletPath:this.options.walletPath,idlPath:this.options.idlPath})}async commitEntropy(t){return this.client.commitEntropy(t)}dispose(){this.client.dispose()}}class ut{output;workerClient;options;requestDecorationRefresh;broker;merkle=new nt;ledger;tooltipAugments=new Map;rootPath=null;scanTimer=null;settleTimer=null;gitPollTimer=null;gitHeadSnapshot=null;pendingSettleReason=null;disposables=[];constructor(t,e,i,o){this.output=t;this.workerClient=e;this.options=i;this.requestDecorationRefresh=o;this.broker=new at(t),this.ledger=new dt(t,{mode:this.options.ledgerMode,rpcUrl:this.options.solanaRpcUrl,autostartValidator:this.options.solanaAutostartValidator,hostBinaryPath:this.options.solanaLedgerHostBinaryPath,walletPath:this.options.solanaWalletPath,idlPath:this.options.solanaIdlPath}),this.disposables.push(this.broker.onDidReceiveRuff((s)=>this.applyRuffSummary(s)),this.broker.onDidReceiveLore((s)=>this.applyLore(s)),this.workerClient.onDidUpdateRecords((s)=>this.captureEntropyLeaves(s)))}async start(t){if(this.rootPath=t,!this.options.enabled)return;this.broker.start(t),await this.ledger.start(t),this.broker.requestScan({type:"scan",reason:"manual",root:t}),this.startScanLoop(),this.startGitWatcher()}onDidSaveDocument(t){if(!this.rootPath||!this.options.enabled||t.uri.scheme!=="file")return;let e=Vt(this.rootPath,t.uri.fsPath);if(!e)return;this.broker.requestScan({type:"scan",reason:"save",root:this.rootPath,files:[e]}),this.scheduleSettlement("save")}requestManualScan(){if(!this.rootPath||!this.options.enabled)return;this.broker.requestScan({type:"scan",reason:"manual",root:this.rootPath})}getTooltipFragments(t){if(!this.rootPath||t.scheme!=="file")return[];let e=Vt(this.rootPath,t.fsPath);if(!e)return[];let i=this.tooltipAugments.get(e);if(!i)return[];let o=[];if(i.ruffViolations>0){if(o.push(`Ruff ${i.ruffViolations} violation${i.ruffViolations===1?"":"s"}`),i.ruffCodeSummary)o.push(`Codes ${i.ruffCodeSummary}`)}if(i.loreLine)o.push(i.loreLine);return o}dispose(){if(this.scanTimer)clearInterval(this.scanTimer),this.scanTimer=null;if(this.settleTimer)clearTimeout(this.settleTimer),this.settleTimer=null;if(this.gitPollTimer)clearInterval(this.gitPollTimer),this.gitPollTimer=null;this.broker.dispose(),this.ledger.dispose(),this.disposables.forEach((t)=>t.dispose()),this.disposables.length=0}captureEntropyLeaves(t){if(!this.options.enabled)return;let e=0;for(let i of t){let o=this.workerClient.getRecord(i);if(!o)continue;let s=this.tooltipAugments.get(o.path);if(this.merkle.upsert({path:o.path,entropy:o.entropy,complexity:o.complexity,debt:o.debt,freshness:o.freshness,ruffViolations:s?.ruffViolations??0,updatedAt:Date.now()}))e+=1}if(e>0)this.scheduleSettlement("interval")}applyRuffSummary(t){if(!this.rootPath||!this.options.enabled)return;let e=t.files,i=[],o=0;for(let s of e){let a=this.tooltipAugments.get(s.path),l={ruffViolations:s.violations,ruffCodeSummary:Ye(s.byCode),loreLine:a?.loreLine};this.tooltipAugments.set(s.path,l);let c=pt.Uri.file(L.join(this.rootPath,s.path));i.push(c);let u=this.workerClient.getRecord(c);if(u){if(this.merkle.upsert({path:u.path,entropy:u.entropy,complexity:u.complexity,debt:u.debt,freshness:u.freshness,ruffViolations:s.violations,updatedAt:Date.now()}))o+=1;if(u.entropy>=0.45||s.violations>0)this.broker.requestLore({type:"lore-request",root:this.rootPath,path:s.path,entropy:u.entropy,violations:s.violations})}}if(i.length>0)this.requestDecorationRefresh(i);if(o>0||this.merkle.hasDirty())this.scheduleSettlement(t.reason)}applyLore(t){if(!this.rootPath||!this.options.enabled)return;if(t.root!==this.rootPath)return;let e=this.tooltipAugments.get(t.path);this.tooltipAugments.set(t.path,{ruffViolations:e?.ruffViolations??t.violations,ruffCodeSummary:e?.ruffCodeSummary,loreLine:t.line}),this.requestDecorationRefresh([pt.Uri.file(L.join(this.rootPath,t.path))])}startScanLoop(){if(this.scanTimer||!this.rootPath)return;let t=Math.max(this.options.pythonScanIntervalMs,1e4);this.scanTimer=setInterval(()=>{if(!this.rootPath)return;this.broker.requestScan({type:"scan",reason:"interval",root:this.rootPath})},t)}startGitWatcher(){if(!this.rootPath||this.gitPollTimer)return;this.gitHeadSnapshot=null,this.gitPollTimer=setInterval(async()=>{if(!this.rootPath)return;let t=await We(this.rootPath);if(!t)return;if(!this.gitHeadSnapshot){this.gitHeadSnapshot=t;return}if(t!==this.gitHeadSnapshot){if(this.gitHeadSnapshot=t,this.output.appendLine("[polyglot] git HEAD changed, scheduling Merkle settlement."),this.rootPath)this.broker.requestScan({type:"scan",reason:"commit",root:this.rootPath});this.scheduleSettlement("commit")}},6000)}scheduleSettlement(t){if(!this.options.enabled)return;if(this.pendingSettleReason=Ke(this.pendingSettleReason,t),this.settleTimer)clearTimeout(this.settleTimer);this.settleTimer=setTimeout(()=>{this.settleTimer=null,this.flushSettlement()},Math.max(this.options.settleDebounceMs,300))}async flushSettlement(){let t=this.pendingSettleReason??"manual";this.pendingSettleReason=null;let e=this.merkle.settle(t);if(!e)return;let i=await this.ledger.commitEntropy(e),o=[`[polyglot] settled Merkle root ${e.rootHash.slice(0,16)}...`,`leaves=${e.leafCount}`,`mode=${i.mode}`];if(i.txSignature)o.push(`tx=${i.txSignature}`);o.push(`detail=${i.detail}`),this.output.appendLine(o.join(" "))}}async function We(t){let e=L.join(t,".git"),i=L.join(e,"HEAD"),o="";try{o=await ht.readFile(i,"utf8")}catch{return null}let s=o.trim();if(!s)return null;if(!s.startsWith("ref:"))return s;let a=s.slice(4).trim(),l=L.join(e,a);try{let c=await ht.readFile(l,"utf8");return`ref:${a}:${c.trim()}`}catch{return`ref:${a}:missing`}}function Vt(t,e){let i=L.relative(t,e);if(!i||i.startsWith("..")||L.isAbsolute(i))return null;return i.replace(/\\/g,"/")}function Ye(t){let e=Object.entries(t).filter((i)=>i[1]>0).sort((i,o)=>o[1]-i[1]||i[0].localeCompare(o[0])).slice(0,3).map(([i,o])=>`${i}×${o}`);return e.length>0?e.join(", "):void 0}function Ke(t,e){if(!t)return e;return Gt(e)>=Gt(t)?e:t}function Gt(t){switch(t){case"commit":return 4;case"save":return 3;case"manual":return 2;case"interval":default:return 1}}var Ft=h(require("path")),zt=require("child_process"),k=h(require("vscode"));class ft{output;headlessVulkan;daemonBinaryOverride;eolApiBase;entropyMonitorIntervalMs;rootPath=null;daemonProcess=null;requestCounter=1;pending=new Map;onDidReceiveManifestEmitter=new k.EventEmitter;onDidReceiveManifest=this.onDidReceiveManifestEmitter.event;onDidReceiveEnvEmitter=new k.EventEmitter;onDidReceiveEnv=this.onDidReceiveEnvEmitter.event;onDidReceiveSedimentEmitter=new k.EventEmitter;onDidReceiveSediment=this.onDidReceiveSedimentEmitter.event;onDidReceiveSedimentChunkEmitter=new k.EventEmitter;onDidReceiveSedimentChunk=this.onDidReceiveSedimentChunkEmitter.event;onDidReceiveSynapseEmitter=new k.EventEmitter;onDidReceiveSynapse=this.onDidReceiveSynapseEmitter.event;onDidReceiveEntropyStateEmitter=new k.EventEmitter;onDidReceiveEntropyState=this.onDidReceiveEntropyStateEmitter.event;onDidReceiveFiredancerSurgeEmitter=new k.EventEmitter;onDidReceiveFiredancerSurge=this.onDidReceiveFiredancerSurgeEmitter.event;constructor(t,e,i,o="https://endoflife.date/api/v1",s=21600000){this.output=t;this.headlessVulkan=e;this.daemonBinaryOverride=i;this.eolApiBase=o;this.entropyMonitorIntervalMs=s}start(t){this.rootPath=t,this.startDaemon()}async requestSediment(t,e){return this.submitRequest("reactor/sediment",{max_layers:t,max_files:e})}async requestSedimentStream(t,e,i=220){return this.submitRequest("reactor/sediment_stream",{max_layers:t,max_files:e,chunk_size:i})}async requestSedimentSynapse(t,e,i=220){return this.submitRequest("reactor/sediment_synapse",{max_layers:t,max_files:e,chunk_size:i})}async requestDetect(){return this.submitRequest("anno/detect",{})}async requestProvision(){return this.submitRequest("anno/provision",{})}async requestEntropyState(){return this.submitRequest("reactor/entropy_state",{})}dispose(){for(let[,t]of this.pending)t.reject(Error("AnnoClient disposed"));this.pending.clear(),this.daemonProcess?.kill(),this.daemonProcess=null,this.onDidReceiveManifestEmitter.dispose(),this.onDidReceiveEnvEmitter.dispose(),this.onDidReceiveSedimentEmitter.dispose(),this.onDidReceiveSedimentChunkEmitter.dispose(),this.onDidReceiveSynapseEmitter.dispose(),this.onDidReceiveEntropyStateEmitter.dispose(),this.onDidReceiveFiredancerSurgeEmitter.dispose()}startDaemon(){if(!this.rootPath||this.daemonProcess)return;let t=this.daemonBinaryOverride??Xe(this.rootPath),e=["--workspace",this.rootPath];if(this.headlessVulkan)e.push("--headless-vulkan");this.daemonProcess=zt.spawn(t,e,{cwd:this.rootPath,env:{...process.env,CHTHONIC_EOL_API_BASE:this.eolApiBase,CHTHONIC_ENTROPY_MONITOR_INTERVAL_MS:String(Math.max(60000,this.entropyMonitorIntervalMs))},stdio:["pipe","pipe","pipe"]});let i=D((o)=>this.handlePayload(o),(o)=>this.output.appendLine(`[daemon] invalid JSONL: ${o.message}`));this.daemonProcess.stdout?.on("data",(o)=>i.push(o)),this.daemonProcess.stderr?.on("data",(o)=>{this.output.appendLine(`[daemon] ${o.toString().trimEnd()}`)}),this.daemonProcess.on("error",(o)=>{this.output.appendLine(`[daemon] spawn failed: ${o.message}`),this.rejectAllPending(Error(`daemon spawn failed: ${o.message}`)),this.daemonProcess=null}),this.daemonProcess.on("exit",(o)=>{i.flush(),this.output.appendLine(`[daemon] exited with code ${o??-1}`),this.rejectAllPending(Error("daemon exited")),this.daemonProcess=null})}handlePayload(t){if("method"in t&&!("id"in t)){this.handleNotification(t);return}if("id"in t&&typeof t.id==="number")this.handleResponse(t)}handleNotification(t){let{method:e,params:i}=t;switch(e){case"anno/manifest":this.onDidReceiveManifestEmitter.fire(i),this.output.appendLine(`[daemon] ANNO manifest received (${i.languages?.length??0} languages)`);break;case"anno/env":this.onDidReceiveEnvEmitter.fire(i),this.output.appendLine("[daemon] env report received");break;case"reactor/status":{let o=i.status??"unknown";this.output.appendLine(`[daemon] reactor status: ${o}`);break}case"reactor/sedimentChunk":this.onDidReceiveSedimentChunkEmitter.fire(i);break;case"reactor/synapse":this.onDidReceiveSynapseEmitter.fire(i),this.output.appendLine(`[daemon] synapse status: ${i.status} (${i.mode})`);break;case"reactor/entropyState":this.onDidReceiveEntropyStateEmitter.fire(i),this.output.appendLine(`[daemon] entropy state: ${i.status} (${Math.round((i.decay_score??0)*100)}%)`);break;case"reactor/firedancerSurge":this.onDidReceiveFiredancerSurgeEmitter.fire(i),this.output.appendLine(`[daemon] firedancer slot ${i.slot} tps ${i.simulated_tps} (${i.surge?"surge":"flow"})`);break;default:this.output.appendLine(`[daemon] unknown notification: ${e}`)}}handleResponse(t){let e=t.id,i=this.pending.get(e);if(!i)return;if(this.pending.delete(e),t.error&&typeof t.error==="object"){let o=t.error;i.reject(Error(o.message));return}i.resolve(t.result)}submitRequest(t,e){if(!this.daemonProcess?.stdin||this.daemonProcess.killed)this.startDaemon();if(!this.daemonProcess?.stdin)return Promise.reject(Error("daemon not available"));let i=this.requestCounter++,o={jsonrpc:"2.0",id:i,method:t,params:e};return new Promise((s,a)=>{this.pending.set(i,{resolve:s,reject:a}),this.daemonProcess?.stdin?.write(`${JSON.stringify(o)}
`,(l)=>{if(l)this.pending.delete(i),a(l)})})}rejectAllPending(t){for(let[e,i]of this.pending)this.pending.delete(e),i.reject(t)}}function Xe(t){let e=process.platform==="win32"?"chthonic-daemon.exe":"chthonic-daemon";return Ft.join(t,"native","target","release",e)}var C=h(require("vscode"));class mt{output;envCollection;disposed=!1;constructor(t,e){this.output=t;this.envCollection=e}async activate(){if(this.disposed)return;try{await C.commands.executeCommand("workbench.action.closeSidebar"),await C.commands.executeCommand("workbench.action.terminal.moveToSidePanel"),await C.commands.executeCommand("workbench.action.toggleAuxiliaryBar"),await C.commands.executeCommand("workbench.action.toggleMaximizedPanel"),await C.commands.executeCommand("workbench.action.focusActiveEditorGroup"),this.output.appendLine("[cockpit] layout activated: sidebar=closed, terminal=AuxBar, panel=maximized, editor=Center")}catch(t){this.output.appendLine(`[cockpit] layout activation failed: ${Ze(t)}`)}}applyTerminalEnv(t){if(this.disposed)return;if(!t.path_mutations.length)return;let e=t.path_mutations.sort((o,s)=>o.priority-s.priority).map((o)=>o.path),i=process.platform==="win32"?";":":";for(let o of e)this.envCollection.prepend("PATH",`${o}${i}`);for(let o of C.window.terminals)if(process.platform==="win32")for(let s of e)o.sendText(`$env:PATH = "${s};$env:PATH"`,!0);else for(let s of e)o.sendText(`export PATH="${s}:$PATH"`,!0);if(this.output.appendLine(`[cockpit] terminal env updated: ${e.length} PATH mutations applied`),t.warnings.length>0)for(let o of t.warnings)this.output.appendLine(`[cockpit] warning: ${o}`)}dispose(){this.disposed=!0}}function Ze(t){if(t instanceof Error)return`${t.name}: ${t.message}`;return String(t)}var Yt=h(require("fs")),yt=h(require("path"));class SynapseBridge{output;extensionRoot;binding=null;reader=null;descriptor=null;transportMode;constructor(t,e,i){this.output=t;this.extensionRoot=e;this.transportMode=Qe(i)}updateDescriptor(t){if(this.descriptor=t,this.transportMode==="jsonl"){this.output.appendLine("[synapse] disabled by transport=jsonl");return}if(t.status!=="ready"||t.mode!=="shared_memory"||!t.shm_id){this.output.appendLine(`[synapse] unavailable: ${t.reason??t.status}`);return}try{let e=this.ensureBindingLoaded();this.reader=new e.SynapseReader(t.shm_id,t.event_name??void 0),this.output.appendLine(`[synapse] connected (shm=${t.shm_id})`)}catch(e){this.reader=null,this.output.appendLine(`[synapse] init failed, falling back to JSONL: ${ti(e)}`)}}isReady(){if(this.transportMode==="jsonl")return!1;return this.reader!==null&&this.descriptor?.status==="ready"&&this.descriptor.mode==="shared_memory"}async drain(t,e){if(!this.reader||t.transport!=="shared_memory")return 0;let i=Math.max(0,t.chunks_written??t.total_chunks??0);if(i===0)return 0;let o=0,s=Date.now();while(o<i&&Date.now()-s<4000){if(!this.reader.wait_for_signal(120)){await Wt();continue}while(!0){let l=this.reader.read_chunk();if(!l||l.byteLength===0)break;let c=new Uint8Array(l.buffer,l.byteOffset,l.byteLength),u=new Uint8Array(c.byteLength);if(u.set(c),e(u),o+=1,o>=i)break}await Wt()}if(o<i)this.output.appendLine(`[synapse] partial drain: ${o}/${i}`);return o}dispose(){this.reader=null}ensureBindingLoaded(){if(this.binding)return this.binding;let candidates=[yt.join(this.extensionRoot,"src","reactor","synapse.node"),yt.join(this.extensionRoot,"native","target","release","synapse.node")],existing=candidates.find((t)=>Yt.existsSync(t));if(!existing)throw Error(`synapse.node not found in ${candidates.join(", ")}`);let req=eval("require");return this.binding=req(existing),this.binding}}function Qe(t){let e=t.trim().toLowerCase();if(e==="shared_memory")return"shared_memory";if(e==="jsonl")return"jsonl";return"auto"}function ti(t){if(t instanceof Error)return`${t.name}: ${t.message}`;return String(t)}async function Wt(){await new Promise((t)=>setTimeout(t,0))}var x=h(require("vscode"));var Kt=h(require("fs")),Xt=h(require("path")),ei=[{file:"mise.toml",weight:16},{file:".mise.toml",weight:6},{file:"Cargo.toml",weight:20},{file:"native/Cargo.toml",weight:8},{file:"anno-manifest.toml",weight:8},{file:"uv.lock",weight:10},{file:".python-version",weight:5},{file:".ruby-version",weight:10},{file:"Gemfile",weight:6},{file:"go.mod",weight:10},{file:".node-version",weight:4},{file:".nvmrc",weight:4},{file:"package.json",weight:5},{file:"bun.lock",weight:4}];async function Zt(t){let e=ei.map((s)=>{let a=Xt.join(t,s.file),l=Kt.existsSync(a);return{...s,present:l}}),i=Math.min(100,e.reduce((s,a)=>a.present?s+a.weight:s,0)),o=ii(i);return{score:i,tier:o,markers:e,present:e.filter((s)=>s.present).map((s)=>s.file),missing:e.filter((s)=>!s.present).map((s)=>s.file)}}function Qt(t){switch(t){case"loom":return"loom.svg";case"lens":return"lens.svg";default:return"gate.svg"}}function ii(t){if(t>=80)return"loom";if(t>=45)return"lens";return"gate"}class vt{extensionUri;output;containerId;fallbackItem;fallbackNoticeEmitted=!1;latestRustification=null;latestEntropy=null;constructor(t,e,i="chthonic-archive"){this.extensionUri=t;this.output=e;this.containerId=i;this.fallbackItem=x.window.createStatusBarItem(x.StatusBarAlignment.Left,47),this.fallbackItem.command="chthonic.refreshRustification",this.fallbackItem.tooltip="Rustification score and activity bar icon fallback",this.fallbackItem.show()}async update(t){this.latestRustification=t,await x.commands.executeCommand("setContext","chthonic.rustificationTier",t.tier),await x.commands.executeCommand("setContext","chthonic.rustificationScore",t.score),await this.apply()}async updateEntropy(t){this.latestEntropy=t,await x.commands.executeCommand("setContext","chthonic.decayStatus",t.status),await x.commands.executeCommand("setContext","chthonic.decayScore",Math.round(t.decay_score*100)),await this.apply()}dispose(){this.fallbackItem.dispose()}async apply(){if(!this.fallbackNoticeEmitted)this.output.appendLine("[morph] dynamic Activity Bar icon updates are unavailable on stable API; using status lane fallback"),this.fallbackNoticeEmitted=!0;this.updateFallbackStatus()}resolveIconFile(){let t=this.latestEntropy;if(t){if(t.decay_score>0.5||t.status==="critical")return"hazard.svg";if(t.decay_score<0.2&&t.status==="pristine")return"gate.svg";return"lens.svg"}let e=this.latestRustification?.tier??"gate";return Qt(e)}updateFallbackStatus(){let t=this.latestRustification,e=this.latestEntropy;if(!t){this.fallbackItem.text="$(pulse) Slab boot",this.fallbackItem.tooltip="Rustification score pending";return}let i=oi(t.tier),o=e?Math.round(e.decay_score*100):null,s=e?`${e.status} ${o}%`:"unknown";this.fallbackItem.text=`$(pulse) ${i} ${t.score}% · Decay ${o??"?"}%`,this.fallbackItem.tooltip=[`Rustification ${t.score}% (${t.tier})`,`Decay ${s}`,`Glyph: ${this.resolveIconFile()}`,`Present: ${t.present.join(", ")||"none"}`,`Missing: ${t.missing.join(", ")||"none"}`,e&&e.critical_tools.length>0?`Critical tools: ${e.critical_tools.join(", ")}`:void 0].filter(Boolean).join(`
`)}}function oi(t){switch(t){case"loom":return"Loom";case"lens":return"Lens";default:return"Gate"}}var q=h(require("vscode"));class gt{output;constructor(t){this.output=t}async activate(){await this.applyCommandLayout(),await q.commands.executeCommand("workbench.view.extension.chthonic-archive"),await q.commands.executeCommand("chthonic.loomView.focus"),this.output.appendLine("[deep-focus] activated via stable command lane")}async applyCommandLayout(){try{await q.commands.executeCommand("workbench.action.terminal.moveToSidePanel"),await q.commands.executeCommand("workbench.action.toggleAuxiliaryBar"),await q.commands.executeCommand("workbench.action.focusActiveEditorGroup")}catch(t){this.output.appendLine(`[deep-focus] command layout failed: ${si(t)}`)}}}function si(t){if(t instanceof Error)return`${t.name}: ${t.message}`;return String(t)}var St=h(require("vscode"));class bt{output;constructor(t){this.output=t}async activate(){await this.applyCommandLayout(),await St.commands.executeCommand("workbench.view.extension.chthonic-archive"),this.output.appendLine("[restore-order] completed via stable command lane")}async applyCommandLayout(){await ni(this.output,[["workbench.view.extension.chthonic-archive"],["chthonic.statusView.focus"],["workbench.action.moveFocusedViewToSecondarySidebar"],["chthonic.loomView.focus"],["workbench.action.moveFocusedViewToPanel"],["workbench.action.positionPanelBottom"],["workbench.action.focusActiveEditorGroup"]])}}async function ni(t,e){for(let i of e){let[o,...s]=i;try{await St.commands.executeCommand(o,...s)}catch(a){t.appendLine(`[restore-order] command failed (${o}): ${ri(a)}`)}}}function ri(t){if(t instanceof Error)return`${t.name}: ${t.message}`;return String(t)}var H=h(require("vscode"));class K{static viewType="chthonic.loomView";view=null;report=null;snapshot=null;resolveWebviewView(t,e,i){this.view=t,t.webview.options={enableScripts:!0},t.webview.html=this.buildHtml(),t.webview.onDidReceiveMessage((o)=>{if(!o||typeof o!=="object")return;let s=o;if(s.type==="refresh")H.commands.executeCommand("chthonic.refreshRustification");if(s.type==="rescan")H.commands.executeCommand("chthonic.entropyRefresh");if(s.type==="heal")H.commands.executeCommand("chthonic.slabHeal");if(s.type==="deepFocus")H.commands.executeCommand("chthonic.deepFocus");if(s.type==="restoreOrder")H.commands.executeCommand("chthonic.restoreOrder")}),this.postState()}update(t){this.report=t,this.postState()}updateWorkspaceHealth(t){this.snapshot=t,this.postState()}dispose(){this.view=null}postState(){if(!this.view)return;this.view.webview.postMessage({type:"state",report:this.report,snapshot:this.snapshot})}buildHtml(){let t=ai();return`<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
<body>
    ...
    <script nonce="${t}">
        ...
        window.addEventListener('message', (event) => {
...
</html>`}}function ai(){let e="";for(let i=0;i<24;i+=1)e+="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(Math.random()*62));return e}var f=h(require("fs")),m=h(require("path")),X=h(require("child_process")),ie=h(require("vscode"));class Et{output;envCollection;workspaceRoot;timer=null;running=!1;options={intervalMs:21600000,eolApiBase:"https://endoflife.date/api"};constructor(t,e,i){this.output=t;this.envCollection=e;this.workspaceRoot=i}start(t){this.options=t,this.stopTimer(),this.timer=setInterval(()=>{this.runNow("interval")},Math.max(60000,t.intervalMs))}async runNow(t="manual"){if(this.running){this.output.appendLine("[slab-heal] skipped; already running");return}this.running=!0;try{let e=await this.auditVsToolchain();if(e&&(!e.allRequiredComponentsPresent||!e.allCriticalBinariesPresent))this.output.appendLine(`[slab-heal] vs2026 audit drift detected: components=${e.missingComponentCount}, binaries=${e.missingBinaryCount}`);let i=await this.collectRuntimeStates();if(i.length===0){this.output.appendLine("[slab-heal] no runtime states detected; skipping");return}let o=[];for(let s of i)if(await this.isEol(s.language,s.version))o.push(s);if(o.length===0){this.output.appendLine(`[slab-heal] ${t}: all slab runtimes are supported`);return}this.output.appendLine(`[slab-heal] ${t}: stale runtimes detected: ${o.map((s)=>`${s.language}@${s.version}`).join(", ")}`),await this.repair(o)}catch(e){this.output.appendLine(`[slab-heal] run failed: ${I(e)}`)}finally{this.running=!1}}dispose(){this.stopTimer()}stopTimer(){if(this.timer)clearInterval(this.timer),this.timer=null}async collectRuntimeStates(){let t=[{language:"python",command:"python",args:["-c",'import sys; print(f"{sys.version_info[0]}.{sys.version_info[1]}")']},{language:"ruby",command:"ruby",args:["-e","print RUBY_VERSION"]},{language:"go",command:"go",args:["env","GOVERSION"]},{language:"rust",command:"rustc",args:["--version"]},{language:"solana",command:"solana",args:["--version"]}],e=[];for(let o of t)try{let s=await O(o.command,o.args,this.workspaceRoot),a=hi(s,o.language);if(!a)continue;e.push({language:o.language,version:a})}catch(s){this.output.appendLine(`[slab-heal] probe ${o.language} failed: ${I(s)}`)}let i=await ci(this.workspaceRoot);if(i)e.push({language:"visual-studio",version:i});else this.output.appendLine("[slab-heal] probe visual-studio failed: VS 2026 Insiders not detected");return e}async isEol(t,e){let i=await this.fetchCycles(t);if(!i||!Array.isArray(i))return!1;let o=pi(t,e),s=i.find((l)=>{let c=l;return c.cycle===o||c.cycle===o.split(".").slice(0,1).join(".")});if(!s||s.eol==null)return!1;if(typeof s.eol==="boolean")return s.eol;let a=Date.parse(s.eol);if(Number.isNaN(a))return!1;return a<=Date.now()}async fetchCycles(t){let e=ui(t);for(let i of e){let o=await fetch(`${this.options.eolApiBase}/${i}.json`,{headers:{accept:"application/json"}});if(!o.ok)continue;let s=await o.json();if(Array.isArray(s))return s}return this.output.appendLine(`[slab-heal] endoflife feed unavailable for ${t}; skipping lifecycle gate`),null}async repair(t){if(await di("mise",this.workspaceRoot))await Pt("mise",["upgrade"],this.workspaceRoot,this.output),await Pt("mise",["reshim"],this.workspaceRoot,this.output);else this.output.appendLine("[slab-heal] mise not found on PATH; skipped upgrade/reshim");await this.refreshToolpoolSnapshot();let i=await this.relinkVsHeaders();if(i)this.output.appendLine(`[slab-heal] relinked MSVC headers at ${i}`);else this.output.appendLine("[slab-heal] VS include relink skipped (MSVC path not found)");ie.window.showInformationMessage(`Chthonic self-heal complete: ${t.map((o)=>`${o.language}@${o.version}`).join(", ")}`)}async relinkVsHeaders(){let t=await li(this.workspaceRoot);if(!t||!this.workspaceRoot)return null;let e=m.join(this.workspaceRoot,".chthonic","native","msvc"),i=m.join(e,"include");f.mkdirSync(e,{recursive:!0});try{f.rmSync(i,{recursive:!0,force:!0})}catch{}try{f.symlinkSync(t,i,"junction")}catch(o){let s=m.join(e,"include.path.txt");f.writeFileSync(s,t,"utf8"),this.output.appendLine(`[slab-heal] symlink failed; wrote include manifest ${s}: ${I(o)}`)}return this.envCollection.replace("CHTHONIC_VS_CPP_INCLUDE",t),this.envCollection.prepend("INCLUDE",`${t};`),t}async auditVsToolchain(){let t=te(this.workspaceRoot);if(!t)return null;let e=m.join(t,"scripts","vs2026_audit.ps1");if(!f.existsSync(e))return null;try{let i=await O("pwsh",["-NoProfile","-File",e,"-Json"],t);return JSON.parse(i).summary??null}catch(i){return this.output.appendLine(`[slab-heal] vs2026 audit script failed: ${I(i)}`),null}}async refreshToolpoolSnapshot(){let t=te(this.workspaceRoot);if(!t)return;let e=m.join(t,"scripts","toolpool-scan.ts");if(!f.existsSync(e))return;try{await Pt("bun",["run","scripts/toolpool-scan.ts","--write-env","--quiet"],t,this.output),this.output.appendLine("[slab-heal] tool-pool snapshot refreshed")}catch(i){this.output.appendLine(`[slab-heal] tool-pool snapshot failed: ${I(i)}`)}}}async function li(t){let e=m.join(process.env["ProgramFiles(x86)"]||"C:\\Program Files (x86)","Microsoft Visual Studio","Installer","vswhere.exe");if(f.existsSync(e))try{let o=await O(e,["-latest","-products","*","-requires","Microsoft.VisualStudio.Component.VC.Tools.x86.x64","-property","installationPath"],t),s=ee(o.trim());if(s)return s}catch{}let i=["C:\\Program Files\\Microsoft Visual Studio\\18","C:\\Program Files\\Microsoft Visual Studio\\2026","C:\\Program Files\\Microsoft Visual Studio\\2022"];for(let o of i){let s=ee(o);if(s)return s}return null}function te(t){if(!t)return null;let e=m.join(t,"extensions","chthonic-archive");if(f.existsSync(m.join(e,"package.json")))return e;if(f.existsSync(m.join(t,"package.json")))return t;return null}async function ci(t){let e=m.join(process.env["ProgramFiles(x86)"]||"C:\\Program Files (x86)","Microsoft Visual Studio","Installer","vswhere.exe");if(!f.existsSync(e))return null;let i=[["-prerelease","-latest","-products","Microsoft.VisualStudio.Product.Professional","-property","installationVersion"],["-prerelease","-latest","-products","Microsoft.VisualStudio.Product.BuildTools","-property","installationVersion"]];for(let o of i)try{let a=(await O(e,o,t)).trim();if(a.length>0)return a}catch{}return null}function ee(t){if(!f.existsSync(t))return null;let e=[],i=[t];while(i.length>0){let o=i.pop(),s=[];try{s=f.readdirSync(o,{withFileTypes:!0})}catch{continue}for(let a of s){let l=m.join(o,a.name);if(!a.isDirectory())continue;if(a.name==="include"&&l.includes(`${m.sep}VC${m.sep}Tools${m.sep}MSVC${m.sep}`)){e.push(l);continue}i.push(l)}}if(e.length===0)return null;return e.sort((o,s)=>s.localeCompare(o,void 0,{numeric:!0,sensitivity:"base"})),e[0]}async function O(t,e,i){return new Promise((o,s)=>{X.execFile(t,e,{cwd:i||void 0,windowsHide:!0,timeout:15000},(a,l,c)=>{if(a){s(Error(`${t} ${e.join(" ")} failed: ${c||a.message}`));return}o(String(l).trim())})})}async function di(t,e){if(process.platform==="win32")try{return await O("where",[t],e),!0}catch{return!1}try{return await O("which",[t],e),!0}catch{return!1}}async function Pt(t,e,i,o){await new Promise((s,a)=>{let l=X.spawn(t,e,{cwd:i||void 0,windowsHide:!0,stdio:["ignore","pipe","pipe"]});l.stdout.on("data",(c)=>{o.appendLine(`[slab-heal] ${c.toString().trimEnd()}`)}),l.stderr.on("data",(c)=>{o.appendLine(`[slab-heal] ${c.toString().trimEnd()}`)}),l.on("error",a),l.on("exit",(c)=>{if(c===0)s();else a(Error(`${t} ${e.join(" ")} exited with code ${c??-1}`))})})}function hi(t,e){let i=t.trim();if(!i)return"";switch(e){case"go":return i.replace(/^go/i,"");case"rust":return i.replace(/^rustc\s+/i,"").split(/\s+/)[0]??"";case"solana":return i.replace(/^solana-cli\s+/i,"").split(/\s+/)[0]??"";case"visual-studio":return i.split(/\s+/)[0]??"";default:return i}}function pi(t,e){let i=e.split(".");if(t==="go")return i.slice(0,2).join(".");if(t==="solana")return i.slice(0,1).join(".");if(t==="visual-studio")return i.slice(0,2).join(".");return i.slice(0,2).join(".")}function ui(t){switch(t){case"solana":return["solana","agave","solana-cli"];case"visual-studio":return["visual-studio"];default:return[t]}}function I(t){if(t instanceof Error)return`${t.name}: ${t.message}`;return String(t)}function fi(t){console.log("☥ Chthonic Archive extension activated");let e=r.window.createOutputChannel("Chthonic Archive"),i=r.workspace.workspaceFolders?.[0]?.uri.fsPath||null,o=r.workspace.getConfiguration("chthonic");t.subscriptions.push(e,r.window.registerWebviewViewProvider("chthonic.chatView",new ne));let s=new vt(t.extensionUri,e),a=new gt(e),l=new bt(e),c=new K,u=new Et(e,t.environmentVariableCollection,i);t.subscriptions.push(s,c,u,r.window.registerWebviewViewProvider(K.viewType,c));let p=o,Lt=p.get("entropy.enabled",!0),le=p.get("entropy.maxFiles",1e4),ce=p.get("entropy.scanIntervalMs",20000),de=p.get("entropy.decorationDebounceMs",120),he=p.get("entropy.decorationBatchSize",240),pe=p.get("entropy.polyglotEnabled",!0),ue=p.get("entropy.pythonScanIntervalMs",30000),fe=p.get("entropy.ledgerSettleDebounceMs",1400),me=p.get("entropy.ledgerMode","validator"),ye=p.get("entropy.solanaRpcUrl","http://127.0.0.1:8899"),ve=p.get("entropy.solanaAutostartValidator",!1),ge=Z(p.get("entropy.solanaLedgerHostBinaryPath","")),Se=Z(p.get("entropy.solanaWalletPath","")),be=Z(p.get("entropy.solanaIdlPath","")),v=new ot(t,e),V,T=new ut(e,v,{enabled:pe,pythonScanIntervalMs:ue,settleDebounceMs:fe,ledgerMode:me,solanaRpcUrl:ye,solanaAutostartValidator:ve,solanaLedgerHostBinaryPath:ge,solanaWalletPath:Se,solanaIdlPath:be},(n)=>V?.enqueueExternalUpdates(n));V=new st(v,de,he,(n)=>T.getTooltipFragments(n));let Q=new F(t.extensionUri,v);Q.setRootPath(i),c.updateWorkspaceHealth(v.getSnapshot()),t.subscriptions.push(v,V,Q,T,v.onDidUpdateSnapshot((n)=>{c.updateWorkspaceHealth(n)}),r.window.registerFileDecorationProvider(V),r.window.registerWebviewViewProvider(F.viewType,Q));let B=async(n)=>{if(!i)return;let d=await Zt(i);c.update(d),await s.update(d),e.appendLine(`[monolith] toolchain completeness ${d.score}% (${d.tier}) via ${n}`)};if(i){B("startup");let n=r.workspace.createFileSystemWatcher(new r.RelativePattern(i,"{uv.lock,Cargo.toml,native/Cargo.toml,mise.toml,.mise.toml,anno-manifest.toml,go.mod,.ruby-version,Gemfile,.node-version,.nvmrc,package.json,bun.lock,.python-version}"));t.subscriptions.push(n,n.onDidCreate(()=>{B("marker-create")}),n.onDidChange(()=>{B("marker-change")}),n.onDidDelete(()=>{B("marker-delete")}))}if(i&&Lt)v.start(i,le,ce),T.start(i);t.subscriptions.push(r.workspace.onDidSaveTextDocument((n)=>{v.refreshFile(n.uri),T.onDidSaveDocument(n)})),t.subscriptions.push(r.commands.registerCommand("chthonic.entropyRefresh",()=>{if(!i){r.window.showWarningMessage("Workspace root is required to refresh workspace health.");return}if(!Lt){r.window.showWarningMessage("Workspace health lane is disabled (chthonic.entropy.enabled=false).");return}v.rescanNow(),v.requestGraph(260),T.requestManualScan(),r.window.showInformationMessage("Chthonic workspace health scan requested")}));let xt=p.get("reactor.enabled",!0),Pe=p.get("reactor.headlessVulkan",!0),Ee=p.get("reactor.cockpitAutoLayout",!1),Re=p.get("reactor.transport","auto"),$t=Z(p.get("reactor.daemonBinaryPath","")),w=gi(i,xt,$t),Le=p.get("slab.selfHealingEnabled",!0),kt=p.get("slab.selfHealingIntervalMs",21600000),Ct=p.get("slab.eolApiBase","https://endoflife.date/api"),xe=yi(Ct),$e=Math.max(60000,Math.floor(kt/2)),R=new ft(e,Pe,$t,xe,$e),G=new mt(e,t.environmentVariableCollection),wt=new SynapseBridge(e,t.extensionPath,Re),tt=null,et=null,U=null,Dt=(n,d)=>{let g=d?`${n} (${d.toLocaleString()} tps)`:n;e.appendLine(`[loom-reflex] panel lane ${g}`),l.activate()},ke=(n)=>{e.appendLine(`[loom-reflex] primary lane ${n}`),r.commands.executeCommand("workbench.view.extension.chthonic-archive"),r.commands.executeCommand("chthonic.loomView.focus")};if(!w.ready)e.appendLine(`[reactor] lane parked: ${w.reason}`);t.subscriptions.push(R,G,wt);let Mt=(n)=>{if(s.updateEntropy(n),e.appendLine(`[workspace-health] score ${Math.round(n.decay_score*100)}% (${n.status}) from ${n.source_mise??"no-mise"}`),n.validator_active){let y=`${n.validator_process??"validator"}:${n.validator_source_mise}`;if(y!==et)et=y,ke(`validator-online:${y}`)}else et=null;if(n.firedancer_surge&&n.validator_active){let y=`${n.checked_at_epoch_ms}:${n.simulated_tps}`;if(y!==U)U=y,Dt("workspace-health-surge",n.simulated_tps)}else U=null;if(!n.critical||!n.auto_update_enabled){tt=null;return}let d=`${n.status}:${n.auto_update}:${[...n.critical_tools].sort().join(",")}`;if(d===tt)return;tt=d;let g=n.critical_tools.length>0?n.critical_tools.join(", "):"runtime toolchain";r.window.showWarningMessage(`Workspace health is critical (${Math.round(n.decay_score*100)}%). ${g} needs healing.`,"Run mise upgrade","Later").then((y)=>{if(y==="Run mise upgrade")u.runNow("manual")})};if(t.subscriptions.push(R.onDidReceiveEnv((n)=>{G.applyTerminalEnv(n)}),R.onDidReceiveSynapse((n)=>{wt.updateDescriptor(n)}),R.onDidReceiveEntropyState((n)=>{Mt(n)}),R.onDidReceiveFiredancerSurge((n)=>{if(n.surge){let d=`${n.slot}:${n.simulated_tps}`;if(d!==U)U=d,Dt("daemon-surge",n.simulated_tps)}})),w.ready&&i&&xt){if(R.start(i),R.requestEntropyState().then((d)=>{Mt(d)}).catch((d)=>{e.appendLine(`[workspace-health] initial snapshot unavailable: ${d}`)}),Ee)G.activate();let n=E.join(i,".git","HEAD");if(P.existsSync(n)){let d=null,g=P.watch(E.join(i,".git"),{persistent:!1},(y,jt)=>{if(jt==="HEAD"||jt?.startsWith("refs")){if(d)clearTimeout(d);d=setTimeout(()=>{e.appendLine("[reactor] git change detected, recomputing sediment"),R.requestSediment(10,500).catch((Ce)=>{e.appendLine(`[reactor] live-loop sediment failed: ${Ce}`)})},800)}});t.subscriptions.push({dispose:()=>g.close()})}}if(i&&Le)u.start({intervalMs:kt,eolApiBase:Ct}),u.runNow("interval");if(t.subscriptions.push(r.commands.registerCommand("chthonic.activateCockpit",()=>{G.activate()}),r.commands.registerCommand("chthonic.deepFocus",()=>{a.activate()}),r.commands.registerCommand("chthonic.restoreOrder",()=>{l.activate()}),r.commands.registerCommand("chthonic.slabHeal",()=>{u.runNow("manual")}),r.commands.registerCommand("chthonic.refreshRustification",()=>{B("manual-command")}),r.commands.registerCommand("chthonic.annoDetect",()=>{if(!w.ready){r.window.showWarningMessage(`ANNO lane unavailable: ${w.reason}. Using workspace-health fallback.`),r.commands.executeCommand("chthonic.entropyRefresh");return}if(i)R.start(i);r.window.showInformationMessage("ANNO project detection triggered")}),r.commands.registerCommand("chthonic.reactorSediment",async()=>{if(!w.ready){v.rescanNow(),v.requestGraph(260),r.window.showInformationMessage(`Reactor sediment lane unavailable (${w.reason}); refreshed workspace-health graph instead.`);return}try{let n=await R.requestSediment(10,500);r.window.showInformationMessage(`Sediment computed: ${n.file_count} files, ${n.layer_count} layers (${n.backend}, ${n.compute_time_ms}ms)`)}catch(n){r.window.showErrorMessage(`Sediment computation failed: ${n}`)}}),r.commands.registerCommand("chthonic.openWebCockpit",()=>{let n=vi(o),d=r.window.createWebviewPanel("chthonicWebCockpit","Chthonic Web Cockpit",r.ViewColumn.Active,{enableScripts:!0,retainContextWhenHidden:!0});d.webview.html=`
                <!DOCTYPE html>
                <html lang="en">
                <head>
                    <meta charset="UTF-8">
            ...
            `,d.webview.onDidReceiveMessage((g)=>{e.appendLine(`[webview] Received message: ${JSON.stringify(g)}`)},void 0,t.subscriptions),d.onDidDispose(()=>{e.appendLine("[webview] Chthonic Web Cockpit panel disposed")},void 0,t.subscriptions)}),r.commands.registerCommand("chthonic.startWebCockpit",()=>{if(!i){r.window.showWarningMessage("Workspace root is required to start the Bun/Next cockpit.");return}let n=r.window.createTerminal({name:"Chthonic Web Cockpit",cwd:i});n.show(),n.sendText("bun run web:dev"),e.appendLine("[web-cockpit] started via bun run web:dev")}),r.commands.registerCommand("chthonic.openBunTrainingDocs",async()=>{let n=[{label:"Bun React Guide",description:"Build a React app with Bun",url:"https://bun.com/docs/guides/ecosystem/react#build-a-react-app-with-bun"},{label:"Bun Next.js Guide",description:"Build an app with Next.js and Bun",url:"https://bun.com/docs/guides/ecosystem/nextjs#build-an-app-with-next-js-and-bun"},{label:"Bun Tailwind (Fullstack Bundler)",description:"Tailwind plugin lane for Bun fullstack",url:"https://bun.com/docs/bundler/fullstack#tailwindcss-plugin"},{label:"Bun SQLite API",description:"Typed SQL training lane (bun:sqlite)",url:"https://bun.com/docs/api/sqlite"}],d=await r.window.showQuickPick(n,{placeHolder:"Open Bun training docs"});if(!d)return;r.env.openExternal(r.Uri.parse(d.url))}),r.commands.registerCommand("chthonic.postRestartVerify",()=>{if(!i){r.window.showWarningMessage("Workspace root is required to run post-restart verification.");return}let n=r.window.createTerminal({name:"Chthonic Post-Restart Verify",cwd:i});n.show(),n.sendText("bun run --cwd extensions/chthonic-archive insiders:post-restart:verify"),e.appendLine("[insiders] post-restart verification lane started")}),r.commands.registerCommand("chthonic.restartGate",()=>{if(!i){r.window.showWarningMessage("Workspace root is required to run restart gate checks.");return}let n=r.window.createTerminal({name:"Chthonic Restart Gate",cwd:i});n.show(),n.sendText("bun run --cwd extensions/chthonic-archive insiders:restart:gate"),e.appendLine("[insiders] restart gate check lane started")})),i&&o.get("webCockpit.autoStart",!1))r.commands.executeCommand("chthonic.startWebCockpit");let it=new re;r.window.registerTreeDataProvider("chthonic.themeView",it),t.subscriptions.push(r.commands.registerCommand("chthonic.switchTheme",async()=>{let n=[{label:"$(paintcan) Flesh & Earth",description:"Warm earth · WCAG AA · Distribution palette",id:"Chthonic Mandala - Flesh & Earth"},{label:"$(zap) ROGBIV",description:"SSOT spectral · FA¹⁻⁵ canonical hexes",id:"Chthonic Mandala - ROGBIV"}],d=r.workspace.getConfiguration("workbench").get("colorTheme"),g=await r.window.showQuickPick(n.map((y)=>({...y,picked:d===y.id})),{placeHolder:`Current: ${d}`});if(g)await r.workspace.getConfiguration("workbench").update("colorTheme",g.id,r.ConfigurationTarget.Workspace),r.window.showInformationMessage(`Theme: ${g.id}`),it.refresh()}));let Tt=o;if(Tt.get("showSSOTHash",!0)){let n=r.window.createStatusBarItem(r.StatusBarAlignment.Left,50);n.command="chthonic.verifySSOT",n.tooltip="SSOT integrity hash — click to verify",t.subscriptions.push(n),oe(n),t.subscriptions.push(r.workspace.onDidSaveTextDocument((d)=>{if(d.fileName.includes("copilot-instructions"))oe(n)}))}if(Tt.get("showLineage",!0)){let n=r.window.createStatusBarItem(r.StatusBarAlignment.Left,49);n.command="workbench.view.scm";let d=()=>{let y=bi(i);n.text=`$(git-branch) ${y.label}`,n.tooltip=y.tooltip,n.show()};d();let g=setInterval(d,6000);t.subscriptions.push({dispose:()=>clearInterval(g)}),n.show(),t.subscriptions.push(n)}let Bt=new ae;r.window.registerTreeDataProvider("chthonic.statusView",Bt),t.subscriptions.push(r.commands.registerCommand("chthonic.verifySSOT",async()=>{let n=Rt();if(n)r.window.showInformationMessage(`SSOT SHA-256: ${n.substring(0,16)}…`);else r.window.showWarningMessage("SSOT file not found")})),t.subscriptions.push(r.commands.registerCommand("chthonic.refreshStatus",()=>{Bt.refresh(),it.refresh(),v.rescanNow(),v.requestGraph(260),T.requestManualScan(),B("refresh-status")}))}function mi(){}function Rt(){let t=r.workspace.workspaceFolders?.[0];if(!t)return null;let e=r.workspace.getConfiguration("chthonic").get("ssotPath",".github/copilot-instructions.md"),i=E.join(t.uri.fsPath,e);if(!P.existsSync(i))return null;let s=P.readFileSync(i,"utf-8").replace(/\r\n/g,`
`).replace(/\r/g,`
`).split(`
`).map((a)=>a.trimEnd()).join(`
`).trim();return se.createHash("sha256").update(s,"utf-8").digest("hex")}function oe(t){let e=Rt();if(e)t.text=`$(shield) ${e.substring(0,8)}`,t.show();else t.text="$(shield) SSOT ??",t.show()}function Z(t){let e=t.trim();return e.length>0?e:void 0}function yi(t){let e=t.trim().replace(/\/+$/,"");if(e.endsWith("/api"))return`${e}/v1`;return e.length>0?e:"https://endoflife.date/api/v1"}function vi(t){let e=t.get("webCockpit.url","http://127.0.0.1:3000").trim();if(/^https?:\/\//i.test(e))return e;return`http://${e}`}function gi(t,e,i){if(!e)return{ready:!1,reason:"disabled by chthonic.reactor.enabled=false"};if(!t)return{ready:!1,reason:"workspace root is unavailable"};let o=Si(t,i);if(!P.existsSync(o))return{ready:!1,reason:`daemon binary missing (${o})`,daemonPath:o};return{ready:!0,reason:"ready",daemonPath:o}}function Si(t,e){if(e)return E.isAbsolute(e)?e:E.join(t,e);let i=process.platform==="win32"?"chthonic-daemon.exe":"chthonic-daemon";return E.join(t,"native","target","release",i)}function bi(t){if(!t)return{label:"no-workspace",tooltip:"Chthonic lineage unavailable: no workspace root."};let e=E.join(t,".git"),i=E.join(e,"HEAD");if(!P.existsSync(i))return{label:"no-git",tooltip:"Chthonic lineage unavailable: .git/HEAD not found."};try{let o=P.readFileSync(i,"utf8").trim();if(!o)return{label:"detached",tooltip:"Chthonic lineage unavailable: empty git HEAD."};if(!o.startsWith("ref:"))return{label:`detached@${o.slice(0,8)}`,tooltip:`Detached HEAD ${o}`};let s=o.slice(4).trim(),a=s.split("/").pop()||s,l=E.join(e,...s.split("/")),c=P.existsSync(l)?P.readFileSync(l,"utf8").trim().slice(0,8):"unknown";return{label:`${a}@${c}`,tooltip:`Chthonic lineage
${s}
${c}`}}catch(o){return{label:"lineage-error",tooltip:`Chthonic lineage read failed: ${String(o)}`}}}class ne{resolveWebviewView(t,e,i){t.webview.options={enableScripts:!1},t.webview.html=`
            <!DOCTYPE html>
            <html lang="en">
            <head>
                <meta charset="UTF-8" />
