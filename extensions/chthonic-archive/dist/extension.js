...
</html>`}}function Ke(){let e="";for(let s=0;s<24;s+=1)e+="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(Math.random()*62));return e}var m=c(require("fs")),v=c(require("path")),Y=c(require("child_process")),Xt=c(require("vscode"));class kt{output;envCollection;workspaceRoot;timer=null;running=!1;options={intervalMs:21600000,eolApiBase:"https://endoflife.date/api"};constructor(t,e,s){this.output=t;this.envCollection=e;this.workspaceRoot=s}start(t){this.options=t,this.stopTimer(),this.timer=setInterval(()=>{this.runNow("interval")},Math.max(60000,t.intervalMs))}async runNow(t="manual"){if(this.running){this.output.appendLine("[slab-heal] skipped; already running");return}this.running=!0;try{let e=await this.collectRuntimeStates();if(e.length===0){this.output.appendLine("[slab-heal] no runtime states detected; skipping");return}let s=[];for(let i of e)if(await this.isEol(i.language,i.version))s.push(i);if(s.length===0){this.output.appendLine(`[slab-heal] ${t}: all slab runtimes are supported`);return}this.output.appendLine(`[slab-heal] ${t}: stale runtimes detected: ${s.map((i)=>`${i.language}@${i.version}`).join(", ")}`),await this.repair(s)}catch(e){this.output.appendLine(`[slab-heal] run failed: ${Et(e)}`)}finally{this.running=!1}}dispose(){this.stopTimer()}stopTimer(){if(this.timer)clearInterval(this.timer),this.timer=null}async collectRuntimeStates(){let t=[{language:"python",command:"python",args:["-c",'import sys; print(f"{sys.version_info[0]}.{sys.version_info[1]}")']},{language:"ruby",command:"ruby",args:["-e","print RUBY_VERSION"]},{language:"go",command:"go",args:["env","GOVERSION"]}],e=[];for(let s of t)try{let i=await Zt(s.command,s.args,this.workspaceRoot),n=Ye(i,s.language);if(!n)continue;e.push({language:s.language,version:n})}catch(i){this.output.appendLine(`[slab-heal] probe ${s.language} failed: ${Et(i)}`)}return e}async isEol(t,e){let s=await fetch(`${this.options.eolApiBase}/${t}.json`,{headers:{accept:"application/json"}});if(!s.ok)throw Error(`endoflife query failed for ${t}: ${s.status}`);let i=await s.json();if(!Array.isArray(i))return!1;let n=Xe(t,e),o=i.find((l)=>{let u=l;return u.cycle===n||u.cycle===n.split(".").slice(0,1).join(".")});if(!o||o.eol==null)return!1;if(typeof o.eol==="boolean")return o.eol;let r=Date.parse(o.eol);if(Number.isNaN(r))return!1;return r<=Date.now()}async repair(t){await Yt("mise",["upgrade"],this.workspaceRoot,this.output),await Yt("mise",["reshim"],this.workspaceRoot,this.output);let e=await this.relinkVsHeaders();if(e)this.output.appendLine(`[slab-heal] relinked MSVC headers at ${e}`);else this.output.appendLine("[slab-heal] VS include relink skipped (MSVC path not found)");Xt.window.showInformationMessage(`Chthonic self-heal complete: ${t.map((s)=>`${s.language}@${s.version}`).join(", ")}`)}async relinkVsHeaders(){let t=await Ge(this.workspaceRoot);if(!t||!this.workspaceRoot)return null;let e=v.join(this.workspaceRoot,".chthonic","native","msvc"),s=v.join(e,"include");m.mkdirSync(e,{recursive:!0});try{m.rmSync(s,{recursive:!0,force:!0})}catch{}try{m.symlinkSync(t,s,"junction")}catch(i){let n=v.join(e,"include.path.txt");m.writeFileSync(n,t,"utf8"),this.output.appendLine(`[slab-heal] symlink failed; wrote include manifest ${n}: ${Et(i)}`)}return this.envCollection.replace("CHTHONIC_VS_CPP_INCLUDE",t),this.envCollection.prepend("INCLUDE",`${t};`),t}}async function Ge(t){let e=v.join(process.env["ProgramFiles(x86)"]||"C:\\Program Files (x86)","Microsoft Visual Studio","Installer","vswhere.exe");if(m.existsSync(e))try{let i=await Zt(e,["-latest","-products","*","-requires","Microsoft.VisualStudio.Component.VC.Tools.x86.x64","-property","installationPath"],t),n=Gt(i.trim());if(n)return n}catch{}let s=["C:\\Program Files\\Microsoft Visual Studio\\2026","C:\\Program Files\\Microsoft Visual Studio\\2022"];for(let i of s){let n=Gt(i);if(n)return n}return null}function Gt(t){if(!m.existsSync(t))return null;let e=[],s=[t];while(s.length>0){let i=s.pop(),n=[];try{n=m.readdirSync(i,{withFileTypes:!0})}catch{continue}for(let o of n){let r=v.join(i,o.name);if(!o.isDirectory())continue;if(o.name==="include"&&r.includes(`${v.sep}VC${v.sep}Tools${v.sep}MSVC${v.sep}`)){e.push(r);continue}s.push(r)}}if(e.length===0)return null;return e.sort((i,n)=>n.localeCompare(i,void 0,{numeric:!0,sensitivity:"base"})),e[0]}async function Zt(t,e,s){return new Promise((i,n)=>{Y.execFile(t,e,{cwd:s||void 0,windowsHide:!0,timeout:15000},(o,r,l)=>{if(o){n(Error(`${t} ${e.join(" ")} failed: ${l||o.message}`));return}i(String(r).trim())})})}async function Yt(t,e,s,i){await new Promise((n,o)=>{let r=Y.spawn(t,e,{cwd:s||void 0,windowsHide:!0,stdio:["ignore","pipe","pipe"]});r.stdout.on("data",(l)=>{i.appendLine(`[slab-heal] ${l.toString().trimEnd()}`)}),r.stderr.on("data",(l)=>{i.appendLine(`[slab-heal] ${l.toString().trimEnd()}`)}),r.on("error",o),r.on("exit",(l)=>{if(l===0)n();else o(Error(`${t} ${e.join(" ")} exited with code ${l??-1}`))})})}function Ye(t,e){let s=t.trim();if(!s)return"";switch(e){case"go":return s.replace(/^go/i,"");default:return s}}function Xe(t,e){let s=e.split(".");if(t==="go")return s.slice(0,2).join(".");return s.slice(0,2).join(".")}function Et(t){if(t instanceof Error)return`${t.name}: ${t.message}`;return String(t)}function Ze(t){console.log("☥ Chthonic Archive extension activated");let e=a.window.createOutputChannel("Chthonic SDK"),s=a.workspace.workspaceFolders?.[0]?.uri.fsPath||null,i=j.join(s||"","meta-ide","copilot-sdk","harness.ts"),n=new V(t.extensionUri,i,(d)=>e.appendLine(`[${new Date().toISOString()}] ${d}`));t.subscriptions.push(a.window.registerWebviewViewProvider(V.viewType,n));let o=new xt(t.extensionUri,e),r=new St(e),l=new G,u=new kt(e,t.environmentVariableCollection,s);t.subscriptions.push(o,l,u,a.window.registerWebviewViewProvider(G.viewType,l));let p=a.workspace.getConfiguration("chthonic"),I=p.get("entropy.enabled",!0),h=p.get("entropy.maxFiles",1e4),b=p.get("entropy.scanIntervalMs",20000),Z=p.get("entropy.decorationDebounceMs",120),y=p.get("entropy.decorationBatchSize",240),ne=p.get("entropy.polyglotEnabled",!0),oe=p.get("entropy.pythonScanIntervalMs",30000),re=p.get("entropy.ledgerSettleDebounceMs",1400),ae=p.get("entropy.ledgerMode","validator"),de=p.get("entropy.solanaRpcUrl","http://127.0.0.1:8899"),ce=p.get("entropy.solanaAutostartValidator",!1),le=X(p.get("entropy.solanaLedgerHostBinaryPath","")),pe=X(p.get("entropy.solanaWalletPath","")),ue=X(p.get("entropy.solanaIdlPath","")),x=new ot(t,e),F,$=new ft(e,x,{enabled:ne,pythonScanIntervalMs:oe,settleDebounceMs:re,ledgerMode:ae,solanaRpcUrl:de,solanaAutostartValidator:ce,solanaLedgerHostBinaryPath:le,solanaWalletPath:pe,solanaIdlPath:ue},(d)=>F?.enqueueExternalUpdates(d));F=new at(x,Z,y,(d)=>$.getTooltipFragments(d));let R=new N(t.extensionUri,x);R.setRootPath(s),t.subscriptions.push(x,F,R,$,a.window.registerFileDecorationProvider(F),a.window.registerWebviewViewProvider(N.viewType,R));let A=async(d)=>{if(!s)return;let g=await Qt(s);l.update(g),await o.update(g),e.appendLine(`[monolith] rustification ${g.score}% (${g.tier}) via ${d}`)};if(s){A("startup");let d=a.workspace.createFileSystemWatcher(new a.RelativePattern(s,"{uv.lock,Cargo.toml,mise.toml,.mise.toml,go.mod,.ruby-version}"));t.subscriptions.push(d,d.onDidCreate(()=>{A("marker-create")}),d.onDidChange(()=>{A("marker-change")}),d.onDidDelete(()=>{A("marker-delete")}))}if(s&&I)x.start(s,h,b),$.start(s);t.subscriptions.push(a.workspace.onDidSaveTextDocument((d)=>{x.refreshFile(d.uri),$.onDidSaveDocument(d)})),t.subscriptions.push(a.commands.registerCommand("chthonic.entropyRefresh",()=>{x.rescanNow(),x.requestGraph(260),$.requestManualScan(),a.window.showInformationMessage("Chthonic entropy scan requested")}));let he=p.get("reactor.enabled",!0),ge=p.get("reactor.headlessVulkan",!0),me=p.get("reactor.cockpitAutoLayout",!1),fe=p.get("reactor.transport","auto"),ye=X(p.get("reactor.daemonBinaryPath","")),ve=p.get("slab.selfHealingEnabled",!0),be=p.get("slab.selfHealingIntervalMs",21600000),xe=p.get("slab.eolApiBase","https://endoflife.date/api"),S=new yt(e,ge,ye),q=new vt(e,t.environmentVariableCollection),tt=new SynapseBridge(e,t.extensionPath,fe);if(t.subscriptions.push(S,q,tt),t.subscriptions.push(S.onDidReceiveEnv((d)=>{q.applyTerminalEnv(d)}),S.onDidReceiveSediment((d)=>{R.postSedimentData(d)}),S.onDidReceiveSedimentChunk((d)=>{R.postSedimentChunk(d)}),S.onDidReceiveSynapse((d)=>{tt.updateDescriptor(d)})),R.onRequestSediment(()=>{es(S,R,tt,e)}),s&&he){if(S.start(s),me)q.activate();let d=j.join(s,".git","HEAD");if(C.existsSync(d)){let g=null,_=C.watch(j.join(s,".git"),{persistent:!1},(st,Rt)=>{if(Rt==="HEAD"||Rt?.startsWith("refs")){if(g)clearTimeout(g);g=setTimeout(()=>{e.appendLine("[reactor] git change detected, recomputing sediment"),S.requestSediment(10,500).catch((Se)=>{e.appendLine(`[reactor] live-loop sediment failed: ${Se}`)})},800)}});t.subscriptions.push({dispose:()=>_.close()})}}if(s&&ve)u.start({intervalMs:be,eolApiBase:xe}),u.runNow("interval");t.subscriptions.push(a.commands.registerCommand("chthonic.activateCockpit",()=>{q.activate()}),a.commands.registerCommand("chthonic.deepFocus",()=>{r.activate()}),a.commands.registerCommand("chthonic.slabHeal",()=>{u.runNow("manual")}),a.commands.registerCommand("chthonic.refreshRustification",()=>{A("manual-command")}),a.commands.registerCommand("chthonic.annoDetect",()=>{if(s)S.start(s);a.window.showInformationMessage("ANNO project detection triggered")}),a.commands.registerCommand("chthonic.reactorSediment",async()=>{try{let d=await S.requestSediment(10,500);a.window.showInformationMessage(`Sediment computed: ${d.file_count} files, ${d.layer_count} layers (${d.backend}, ${d.compute_time_ms}ms)`)}catch(d){a.window.showErrorMessage(`Sediment computation failed: ${d}`)}}));let et=new se;a.window.registerTreeDataProvider("chthonic.themeView",et),t.subscriptions.push(a.commands.registerCommand("chthonic.switchTheme",async()=>{let d=[{label:"$(paintcan) Flesh & Earth",description:"Warm earth · WCAG AA · Distribution palette",id:"Chthonic Mandala - Flesh & Earth"},{label:"$(zap) ROGBIV",description:"SSOT spectral · FA¹⁻⁵ canonical hexes",id:"Chthonic Mandala - ROGBIV"}],g=a.workspace.getConfiguration("workbench").get("colorTheme"),_=await a.window.showQuickPick(d.map((st)=>({...st,picked:g===st.id})),{placeHolder:`Current: ${g}`});if(_)await a.workspace.getConfiguration("workbench").update("colorTheme",_.id,a.ConfigurationTarget.Workspace),a.window.showInformationMessage(`Theme: ${_.id}`),et.refresh()}));let wt=a.workspace.getConfiguration("chthonic");if(wt.get("showSSOTHash",!0)){let d=a.window.createStatusBarItem(a.StatusBarAlignment.Left,50);d.command="chthonic.verifySSOT",d.tooltip="SSOT integrity hash — click to verify",t.subscriptions.push(d),te(d),t.subscriptions.push(a.workspace.onDidSaveTextDocument((g)=>{if(g.fileName.includes("copilot-instructions"))te(d)}))}if(wt.get("showLineage",!0)){let d=a.window.createStatusBarItem(a.StatusBarAlignment.Left,49);d.text="$(git-branch) ☥ main",d.tooltip="Chthonic lineage",d.show(),t.subscriptions.push(d)}let Lt=new ie;a.window.registerTreeDataProvider("chthonic.statusView",Lt),t.subscriptions.push(a.commands.registerCommand("chthonic.verifySSOT",async()=>{let d=Pt();if(d)a.window.showInformationMessage(`SSOT SHA-256: ${d.substring(0,16)}…`);else a.window.showWarningMessage("SSOT file not found")})),t.subscriptions.push(a.commands.registerCommand("chthonic.refreshStatus",()=>{Lt.refresh(),et.refresh(),x.rescanNow(),x.requestGraph(260),$.requestManualScan(),A("refresh-status")}))}function ts(){}async function es(t,e,s,i){try{if(s.isReady()){let n=await t.requestSedimentSynapse(10,500,220);if(n.transport==="shared_memory"){let o=await s.drain(n,(r)=>{e.postSedimentBinary(r)});i.appendLine(`[reactor] synapse drain ${o}/${n.chunks_written} chunks`);return}}await t.requestSedimentStream(10,500)}catch(n){i.appendLine(`[reactor] sediment request failed: ${n}`)}}function Pt(){let t=a.workspace.workspaceFolders?.[0];if(!t)return null;let e=a.workspace.getConfiguration("chthonic").get("ssotPath",".github/copilot-instructions.md"),s=j.join(t.uri.fsPath,e);if(!C.existsSync(s))return null;let n=C.readFileSync(s,"utf-8").replace(/\r\n/g,`
`).replace(/\r/g,`
`).split(`
`).map((o)=>o.trimEnd()).join(`
`).trim();return ee.createHash("sha256").update(n,"utf-8").digest("hex")}function te(t){let e=Pt();if(e)t.text=`$(shield) ${e.substring(0,8)}`,t.show();else t.text="$(shield) SSOT ??",t.show()}function X(t){let e=t.trim();return e.length>0?e:void 0}class se{_onDidChange=new a.EventEmitter;onDidChangeTreeData=this._onDidChange.event;refresh(){this._onDidChange.fire()}getTreeItem(t){return t}getChildren(){let t=a.workspace.getConfiguration("workbench").get("colorTheme")||"";return[{name:"Chthonic Mandala - Flesh & Earth",short:"Flesh & Earth",icon:"\uD83C\uDF0D",desc:"Warm earth · Distribution"},{name:"Chthonic Mandala - ROGBIV",short:"ROGBIV",icon:"\uD83C\uDF08",desc:"SSOT spectral · Research"}].map((s)=>{let i=t===s.name,n=new a.TreeItem(`${i?"◉":"○"} ${s.icon} ${s.short}`,a.TreeItemCollapsibleState.None);return n.tooltip=`${s.name}
${s.desc}${i?`

✅ ACTIVE`:""}`,n.description=i?"active":"",n.command={command:"chthonic.switchTheme",title:"Switch"},n})}}class ie{_onDidChange=new a.EventEmitter;onDidChangeTreeData=this._onDidChange.event;refresh(){this._onDidChange.fire()}getTreeItem(t){return t}getChildren(){let t=Pt(),e=[],s=new a.TreeItem(`$(shield) SSOT: ${t?t.substring(0,12)+"…":"not found"}`,a.TreeItemCollapsibleState.None);s.command={command:"chthonic.verifySSOT",title:"Verify"},e.push(s);let i=new a.TreeItem(`$(paintcan) Theme: ${(a.workspace.getConfiguration("workbench").get("colorTheme")||"default").replace("Chthonic Mandala - ","")}`,a.TreeItemCollapsibleState.None);return i.command={command:"chthonic.switchTheme",title:"Switch"},e.push(i),e}}
