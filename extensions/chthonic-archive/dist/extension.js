...
</html>`}}var be=c(require("fs")),Ee=require("module"),Ut=c(require("path")),__filename="C:\\Users\\eldno\\chthonic-archive\\extensions\\chthonic-archive\\src\\reactor\\synapseBridge.ts";class Gt{output;extensionRoot;binding=null;reader=null;descriptor=null;transportMode;constructor(t,e,i){this.output=t;this.extensionRoot=e;this.transportMode=bi(i)}updateDescriptor(t){if(this.descriptor=t,this.transportMode==="jsonl"){this.output.appendLine("[synapse] disabled by transport=jsonl");return}if(t.status!=="ready"||t.mode!=="shared_memory"||!t.shm_id){this.output.appendLine(`[synapse] unavailable: ${t.reason??t.status}`);return}try{let e=this.ensureBindingLoaded();this.reader=new e.SynapseReader(t.shm_id,t.event_name??void 0),this.output.appendLine(`[synapse] connected (shm=${t.shm_id})`)}catch(e){this.reader=null,this.output.appendLine(`[synapse] init failed, falling back to JSONL: ${Ei(e)}`)}}isReady(){if(this.transportMode==="jsonl")return!1;return this.reader!==null&&this.descriptor?.status==="ready"&&this.descriptor.mode==="shared_memory"}async drain(t,e){if(!this.reader||t.transport!=="shared_memory")return 0;let i=Math.max(0,t.chunks_written??t.total_chunks??0);if(i===0)return 0;let o=0,s=Date.now();while(o<i&&Date.now()-s<4000){if(!this.reader.wait_for_signal(120)){await Se();continue}while(!0){let l=this.reader.read_chunk();if(!l||l.byteLength===0)break;let h=new Uint8Array(l.buffer,l.byteOffset,l.byteLength),d=new Uint8Array(h.byteLength);if(d.set(h),e(d),o+=1,o>=i)break}await Se()}if(o<i)this.output.appendLine(`[synapse] partial drain: ${o}/${i}`);return o}dispose(){this.reader=null}ensureBindingLoaded(){if(this.binding)return this.binding;let t=[Ut.join(this.extensionRoot,"src","reactor","synapse.node"),Ut.join(this.extensionRoot,"native","target","release","synapse.node")],e=t.find((o)=>be.existsSync(o));if(!e)throw Error(`synapse.node not found in ${t.join(", ")}`);let i=Ee.createRequire(__filename);return this.binding=i(e),this.binding}}function bi(t){let e=t.trim().toLowerCase();if(e==="shared_memory")return"shared_memory";if(e==="jsonl")return"jsonl";return"auto"}function Ei(t){if(t instanceof Error)return`${t.name}: ${t.message}`;return String(t)}async function Se(){await new Promise((t)=>setTimeout(t,0))}var A=c(require("vscode"));var Re=c(require("fs")),Pe=c(require("path")),Ri=[{file:"mise.toml",weight:16},{file:".mise.toml",weight:6},{file:"Cargo.toml",weight:20},{file:"native/Cargo.toml",weight:8},{file:"anno-manifest.toml",weight:8},{file:"uv.lock",weight:10},{file:".python-version",weight:5},{file:".ruby-version",weight:10},{file:"Gemfile",weight:6},{file:"go.mod",weight:10},{file:".node-version",weight:4},{file:".nvmrc",weight:4},{file:"package.json",weight:5},{file:"bun.lock",weight:4}];async function Le(t){let e=Ri.map((s)=>{let r=Pe.join(t,s.file),l=Re.existsSync(r);return{...s,present:l}}),i=Math.min(100,e.reduce((s,r)=>r.present?s+r.weight:s,0)),o=Pi(i);return{score:i,tier:o,markers:e,present:e.filter((s)=>s.present).map((s)=>s.file),missing:e.filter((s)=>!s.present).map((s)=>s.file)}}function xe(t){switch(t){case"loom":return"loom.svg";case"lens":return"lens.svg";default:return"gate.svg"}}function Pi(t){if(t>=80)return"loom";if(t>=45)return"lens";return"gate"}class zt{extensionUri;output;containerId;fallbackItem;fallbackNoticeEmitted=!1;latestRustification=null;latestEntropy=null;constructor(t,e,i="chthonic-archive"){this.extensionUri=t;this.output=e;this.containerId=i;this.fallbackItem=A.window.createStatusBarItem(A.StatusBarAlignment.Left,47),this.fallbackItem.command="chthonic.refreshRustification",this.fallbackItem.tooltip="Rustification score and activity bar icon fallback",this.fallbackItem.show()}async update(t){this.latestRustification=t,await A.commands.executeCommand("setContext","chthonic.rustificationTier",t.tier),await A.commands.executeCommand("setContext","chthonic.rustificationScore",t.score),await this.apply()}async updateEntropy(t){this.latestEntropy=t,await A.commands.executeCommand("setContext","chthonic.decayStatus",t.status),await A.commands.executeCommand("setContext","chthonic.decayScore",Math.round(t.decay_score*100)),await this.apply()}dispose(){this.fallbackItem.dispose()}async apply(){if(!this.fallbackNoticeEmitted)this.output.appendLine("[morph] dynamic Activity Bar icon updates are unavailable on stable API; using status lane fallback"),this.fallbackNoticeEmitted=!0;this.updateFallbackStatus()}resolveIconFile(){let t=this.latestEntropy;if(t){if(t.decay_score>0.5||t.status==="critical")return"hazard.svg";if(t.decay_score<0.2&&t.status==="pristine")return"gate.svg";return"lens.svg"}let e=this.latestRustification?.tier??"gate";return xe(e)}updateFallbackStatus(){let t=this.latestRustification,e=this.latestEntropy;if(!t){this.fallbackItem.text="$(pulse) Slab boot",this.fallbackItem.tooltip="Rustification score pending";return}let i=Li(t.tier),o=e?Math.round(e.decay_score*100):null,s=e?`${e.status} ${o}%`:"unknown",r=o!==null?` · Decay ${o}%`:"";this.fallbackItem.text=`$(pulse) ${i} ${t.score}%${r}`,this.fallbackItem.tooltip=[`Toolchain completeness: ${t.score}% (${t.tier})`,`Present: ${t.present.join(", ")||"none"}`,`Missing: ${t.missing.join(", ")||"none"}`,e?`Workspace health: ${s}`:"Workspace health: entropy disabled",e&&e.critical_tools.length>0?`Critical tools: ${e.critical_tools.join(", ")}`:void 0].filter(Boolean).join(`
`)}}function Li(t){switch(t){case"loom":return"Loom";case"lens":return"Lens";default:return"Gate"}}var tt=c(require("vscode"));class Ft{output;constructor(t){this.output=t}async activate(){await this.applyCommandLayout(),await tt.commands.executeCommand("workbench.view.extension.chthonic-archive"),await tt.commands.executeCommand("chthonic.loomView.focus"),this.output.appendLine("[deep-focus] activated via stable command lane")}async applyCommandLayout(){try{await tt.commands.executeCommand("workbench.action.terminal.moveToSidePanel"),await tt.commands.executeCommand("workbench.action.toggleAuxiliaryBar"),await tt.commands.executeCommand("workbench.action.focusActiveEditorGroup")}catch(t){this.output.appendLine(`[deep-focus] command layout failed: ${xi(t)}`)}}}function xi(t){if(t instanceof Error)return`${t.name}: ${t.message}`;return String(t)}var It=c(require("vscode"));class Vt{output;constructor(t){this.output=t}async activate(){await this.applyCommandLayout(),await It.commands.executeCommand("workbench.view.extension.chthonic-archive"),this.output.appendLine("[restore-order] completed via stable command lane")}async applyCommandLayout(){await $i(this.output,[["workbench.view.extension.chthonic-archive"],["chthonic.statusView.focus"],["workbench.action.moveFocusedViewToSecondarySidebar"],["chthonic.loomView.focus"],["workbench.action.moveFocusedViewToPanel"],["workbench.action.positionPanelBottom"],["workbench.action.focusActiveEditorGroup"]])}}async function $i(t,e){for(let i of e){let[o,...s]=i;try{await It.commands.executeCommand(o,...s)}catch(r){t.appendLine(`[restore-order] command failed (${o}): ${ki(r)}`)}}}function ki(t){if(t instanceof Error)return`${t.name}: ${t.message}`;return String(t)}var et=c(require("vscode"));class yt{static viewType="chthonic.loomView";view=null;report=null;snapshot=null;resolveWebviewView(t,e,i){this.view=t,t.webview.options={enableScripts:!0},t.webview.html=this.buildHtml(),t.webview.onDidReceiveMessage((o)=>{if(!o||typeof o!=="object")return;let s=o;if(s.type==="refresh")et.commands.executeCommand("chthonic.refreshRustification");if(s.type==="rescan")et.commands.executeCommand("chthonic.entropyRefresh");if(s.type==="heal")et.commands.executeCommand("chthonic.slabHeal");if(s.type==="deepFocus")et.commands.executeCommand("chthonic.deepFocus");if(s.type==="restoreOrder")et.commands.executeCommand("chthonic.restoreOrder")}),this.postState()}update(t){this.report=t,this.postState()}updateWorkspaceHealth(t){this.snapshot=t,this.postState()}dispose(){this.view=null}postState(){if(!this.view)return;this.view.webview.postMessage({type:"state",report:this.report,snapshot:this.snapshot})}buildHtml(){let t=Ci();return`<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</html>`}}function Ci(){let e="";for(let i=0;i<24;i+=1)e+="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(Math.random()*62));return e}var v=c(require("fs")),S=c(require("path")),vt=c(require("child_process")),Ce=c(require("vscode"));class Yt{output;envCollection;workspaceRoot;timer=null;running=!1;options={intervalMs:21600000,eolApiBase:"https://endoflife.date/api"};constructor(t,e,i){this.output=t;this.envCollection=e;this.workspaceRoot=i}start(t){this.options=t,this.stopTimer(),this.timer=setInterval(()=>{this.runNow("interval")},Math.max(60000,t.intervalMs))}async runNow(t="manual"){if(this.running){this.output.appendLine("[slab-heal] skipped; already running");return}this.running=!0;try{let e=await this.auditVsToolchain();if(e&&(!e.allRequiredComponentsPresent||!e.allCriticalBinariesPresent))this.output.appendLine(`[slab-heal] vs2026 audit drift detected: components=${e.missingComponentCount}, binaries=${e.missingBinaryCount}`);let i=await this.collectRuntimeStates();if(i.length===0){this.output.appendLine("[slab-heal] no runtime states detected; skipping");return}let o=[];for(let s of i)if(await this.isEol(s.language,s.version))o.push(s);if(o.length===0){this.output.appendLine(`[slab-heal] ${t}: all slab runtimes are supported`);return}this.output.appendLine(`[slab-heal] ${t}: stale runtimes detected: ${o.map((s)=>`${s.language}@${s.version}`).join(", ")}`),await this.repair(o)}catch(e){this.output.appendLine(`[slab-heal] run failed: ${lt(e)}`)}finally{this.running=!1}}dispose(){this.stopTimer()}stopTimer(){if(this.timer)clearInterval(this.timer),this.timer=null}async collectRuntimeStates(){let t=[{language:"python",command:process.platform==="win32"?"python3":"python",args:["-c",'import sys; print(f"{sys.version_info[0]}.{sys.version_info[1]}")']},{language:"ruby",command:"ruby",args:["-e","print RUBY_VERSION"]},{language:"go",command:"go",args:["env","GOVERSION"]},{language:"rust",command:"rustc",args:["--version"]},{language:"solana",command:"solana",args:["--version"]}],e=[];for(let o of t){let s=Di(o),r=await Bi(s,this.workspaceRoot);if(!r.version){this.output.appendLine(`[slab-heal] probe ${o.language} failed (${Ti(s)}): ${lt(r.error)}`);continue}let l=Mi(r.version,o.language);if(!l)continue;e.push({language:o.language,version:l})}let i=await ji(this.workspaceRoot);if(i)e.push({language:"visual-studio",version:i});else this.output.appendLine("[slab-heal] probe visual-studio failed: VS 2026 Insiders not detected");return e}async isEol(t,e){let i=await this.fetchCycles(t);if(!i||!Array.isArray(i))return!1;let o=qi(t,e),s=i.find((l)=>{let h=l;return h.cycle===o||h.cycle===o.split(".").slice(0,1).join(".")});if(!s||s.eol==null)return!1;if(typeof s.eol==="boolean")return s.eol;let r=Date.parse(s.eol);if(Number.isNaN(r))return!1;return r<=Date.now()}async fetchCycles(t){let e=Hi(t);for(let i of e){let o=await fetch(`${this.options.eolApiBase}/${i}.json`,{headers:{accept:"application/json"}});if(!o.ok)continue;let s=await o.json();if(Array.isArray(s))return s}return this.output.appendLine(`[slab-heal] endoflife feed unavailable for ${t}; skipping lifecycle gate`),null}async repair(t){if(await Ai("mise",this.workspaceRoot))await Wt("mise",["upgrade"],this.workspaceRoot,this.output),await Wt("mise",["reshim"],this.workspaceRoot,this.output);else this.output.appendLine("[slab-heal] mise not found on PATH; skipped upgrade/reshim");await this.refreshToolpoolSnapshot();let i=await this.relinkVsHeaders();if(i)this.output.appendLine(`[slab-heal] relinked MSVC headers at ${i}`);else this.output.appendLine("[slab-heal] VS include relink skipped (MSVC path not found)");Ce.window.showInformationMessage(`Chthonic self-heal complete: ${t.map((o)=>`${o.language}@${o.version}`).join(", ")}`)}async relinkVsHeaders(){let t=await _i(this.workspaceRoot);if(!t||!this.workspaceRoot)return null;let e=S.join(this.workspaceRoot,".chthonic","native","msvc"),i=S.join(e,"include");v.mkdirSync(e,{recursive:!0});try{v.rmSync(i,{recursive:!0,force:!0})}catch{}try{v.symlinkSync(t,i,"junction")}catch(o){let s=S.join(e,"include.path.txt");v.writeFileSync(s,t,"utf8"),this.output.appendLine(`[slab-heal] symlink failed; wrote include manifest ${s}: ${lt(o)}`)}return this.envCollection.replace("CHTHONIC_VS_CPP_INCLUDE",t),this.envCollection.prepend("INCLUDE",`${t};`),t}async auditVsToolchain(){let t=$e(this.workspaceRoot);if(!t)return null;let e=S.join(t,"scripts","vs2026_audit.ps1");if(!v.existsSync(e))return null;try{let i=await it("pwsh",["-NoProfile","-File",e,"-Json"],t);return JSON.parse(i).summary??null}catch(i){return this.output.appendLine(`[slab-heal] vs2026 audit script failed: ${lt(i)}`),null}}async refreshToolpoolSnapshot(){let t=$e(this.workspaceRoot);if(!t)return;let e=S.join(t,"scripts","toolpool-scan.ts");if(!v.existsSync(e))return;try{await Wt("bun",["run","scripts/toolpool-scan.ts","--write-env","--quiet"],t,this.output),this.output.appendLine("[slab-heal] tool-pool snapshot refreshed")}catch(i){this.output.appendLine(`[slab-heal] tool-pool snapshot failed: ${lt(i)}`)}}}function Di(t){if(t.language==="python"&&process.platform==="win32")return[{command:"python3",args:t.args},{command:"py",args:["-3",...t.args]},{command:"python",args:t.args}];if(t.language==="go"&&process.platform==="win32")return[{command:"go",args:t.args},{command:"goup",args:["go",...t.args]},{command:"goup",args:["current"]}];return[{command:t.command,args:t.args}]}function Ti(t){return t.map((e)=>`${e.command} ${e.args.join(" ")}`.trim()).join(" | ")}async function Bi(t,e){let i=Error("no probe candidates defined");for(let o of t)try{let s=await it(o.command,o.args,e);if(s.trim().length>0)return{version:s,error:null};i=Error(`${o.command} returned empty output`)}catch(s){i=s}return{version:null,error:i}}async function _i(t){let e=S.join(process.env["ProgramFiles(x86)"]||"C:\\Program Files (x86)","Microsoft Visual Studio","Installer","vswhere.exe");if(v.existsSync(e))try{let o=await it(e,["-latest","-products","*","-requires","Microsoft.VisualStudio.Component.VC.Tools.x86.x64","-property","installationPath"],t),s=ke(o.trim());if(s)return s}catch{}let i=["C:\\Program Files\\Microsoft Visual Studio\\18","C:\\Program Files\\Microsoft Visual Studio\\2026","C:\\Program Files\\Microsoft Visual Studio\\2022"];for(let o of i){let s=ke(o);if(s)return s}return null}function $e(t){if(!t)return null;let e=S.join(t,"extensions","chthonic-archive");if(v.existsSync(S.join(e,"package.json")))return e;if(v.existsSync(S.join(t,"package.json")))return t;return null}async function ji(t){let e=S.join(process.env["ProgramFiles(x86)"]||"C:\\Program Files (x86)","Microsoft Visual Studio","Installer","vswhere.exe");if(!v.existsSync(e))return null;let i=[["-prerelease","-latest","-products","Microsoft.VisualStudio.Product.Professional","-property","installationVersion"],["-prerelease","-latest","-products","Microsoft.VisualStudio.Product.BuildTools","-property","installationVersion"]];for(let o of i)try{let r=(await it(e,o,t)).trim();if(r.length>0)return r}catch{}return null}function ke(t){if(!v.existsSync(t))return null;let e=[],i=[t];while(i.length>0){let o=i.pop(),s=[];try{s=v.readdirSync(o,{withFileTypes:!0})}catch{continue}for(let r of s){let l=S.join(o,r.name);if(!r.isDirectory())continue;if(r.name==="include"&&l.includes(`${S.sep}VC${S.sep}Tools${S.sep}MSVC${S.sep}`)){e.push(l);continue}i.push(l)}}if(e.length===0)return null;return e.sort((o,s)=>s.localeCompare(o,void 0,{numeric:!0,sensitivity:"base"})),e[0]}async function it(t,e,i){return new Promise((o,s)=>{vt.execFile(t,e,{cwd:i||void 0,windowsHide:!0,timeout:15000},(r,l,h)=>{if(r){s(Error(`${t} ${e.join(" ")} failed: ${h||r.message}`));return}o(String(l).trim())})})}async function Ai(t,e){if(process.platform==="win32")try{return await it("where",[t],e),!0}catch{return!1}try{return await it("which",[t],e),!0}catch{return!1}}async function Wt(t,e,i,o){await new Promise((s,r)=>{let l=vt.spawn(t,e,{cwd:i||void 0,windowsHide:!0,stdio:["ignore","pipe","pipe"]});l.stdout.on("data",(h)=>{o.appendLine(`[slab-heal] ${h.toString().trimEnd()}`)}),l.stderr.on("data",(h)=>{o.appendLine(`[slab-heal] ${h.toString().trimEnd()}`)}),l.on("error",r),l.on("exit",(h)=>{if(h===0)s();else r(Error(`${t} ${e.join(" ")} exited with code ${h??-1}`))})})}function Mi(t,e){let i=t.trim();if(!i)return"";switch(e){case"go":return i.replace(/^go/i,"");case"rust":return i.replace(/^rustc\s+/i,"").split(/\s+/)[0]??"";case"solana":return i.replace(/^solana-cli\s+/i,"").split(/\s+/)[0]??"";case"visual-studio":return i.split(/\s+/)[0]??"";default:return i}}function qi(t,e){let i=e.split(".");if(t==="go")return i.slice(0,2).join(".");if(t==="solana")return i.slice(0,1).join(".");if(t==="visual-studio")return i.slice(0,2).join(".");return i.slice(0,2).join(".")}function Hi(t){switch(t){case"solana":return["solana","agave","solana-cli"];case"visual-studio":return["visual-studio"];default:return[t]}}function lt(t){if(t instanceof Error)return`${t.name}: ${t.message}`;return String(t)}function Ji(t){console.log("☥ Chthonic Archive extension activated");let e=n.window.createOutputChannel("Chthonic Archive"),i=n.workspace.workspaceFolders?.[0]?.uri.fsPath||null,o=n.workspace.getConfiguration("chthonic");t.subscriptions.push(e,n.window.registerWebviewViewProvider("chthonic.chatView",new Nt(i)));let s=new zt(t.extensionUri,e),r=new Ft(e),l=new Vt(e),h=new yt,d=new Yt(e,t.environmentVariableCollection,i);t.subscriptions.push(s,h,d,n.window.registerWebviewViewProvider(yt.viewType,h));let p=o,g=p.get("security.allowNativeSidecars",!1),$=p.get("entropy.enabled",!1),f=p.get("entropy.maxFiles",3000),O=p.get("entropy.scanIntervalMs",60000),D=p.get("entropy.decorationDebounceMs",120),V=p.get("entropy.decorationBatchSize",240),P=p.get("entropy.polyglotEnabled",!1)&&g,ot=p.get("entropy.pythonScanIntervalMs",30000),W=p.get("entropy.ledgerSettleDebounceMs",1400),U=p.get("entropy.ledgerMode","bankrun"),st=p.get("entropy.solanaRpcUrl","http://127.0.0.1:8899"),ht=p.get("entropy.solanaAutostartValidator",!1),bt=gt(p.get("entropy.solanaLedgerHostBinaryPath","")),Me=gt(p.get("entropy.solanaWalletPath","")),qe=gt(p.get("entropy.solanaIdlPath","")),b=new kt(t,e),dt,Y=new Ht(e,b,{enabled:P,pythonScanIntervalMs:ot,settleDebounceMs:W,ledgerMode:U,solanaRpcUrl:st,solanaAutostartValidator:ht,solanaLedgerHostBinaryPath:bt,solanaWalletPath:Me,solanaIdlPath:qe},(a)=>dt?.enqueueExternalUpdates(a));dt=new Ct(b,D,V,(a)=>Y.getTooltipFragments(a));let Et=new pt(t.extensionUri,b);Et.setRootPath(i),h.updateWorkspaceHealth(b.getSnapshot()),t.subscriptions.push(b,dt,Et,Y,b.onDidUpdateSnapshot((a)=>{h.updateWorkspaceHealth(a)}),n.window.registerFileDecorationProvider(dt),n.window.registerWebviewViewProvider(pt.viewType,Et));let w=async(a)=>{if(!i)return;let u=await Le(i);h.update(u),await s.update(u),e.appendLine(`[monolith] toolchain completeness ${u.score}% (${u.tier}) via ${a}`)};if(i){w("startup");let a=n.workspace.createFileSystemWatcher(new n.RelativePattern(i,"{uv.lock,Cargo.toml,native/Cargo.toml,mise.toml,.mise.toml,anno-manifest.toml,go.mod,.ruby-version,Gemfile,.node-version,.nvmrc,package.json,bun.lock,.python-version}"));t.subscriptions.push(a,a.onDidCreate(()=>{w("marker-create")}),a.onDidChange(()=>{w("marker-change")}),a.onDidDelete(()=>{w("marker-delete")}))}if(i&&$){if(b.start(i,f,O),P)Y.start(i)}t.subscriptions.push(n.workspace.onDidSaveTextDocument((a)=>{b.refreshFile(a.uri),Y.onDidSaveDocument(a)})),t.subscriptions.push(n.commands.registerCommand("chthonic.entropyRefresh",()=>{if(!i){n.window.showWarningMessage("Workspace root is required to refresh workspace health.");return}if(!$){n.window.showWarningMessage("Workspace health lane is disabled (chthonic.entropy.enabled=false).");return}b.rescanNow(),b.requestGraph(260),Y.requestManualScan(),n.window.showInformationMessage("Chthonic workspace health scan requested")}));let Kt=p.get("reactor.enabled",!1),Xt=Kt&&g,He=p.get("reactor.headlessVulkan",!0),Je=p.get("reactor.cockpitAutoLayout",!1),Oe=p.get("reactor.transport","auto"),Zt=gt(p.get("reactor.daemonBinaryPath","")),T=Wi(i,Xt,Zt,g,Kt),Rt=p.get("slab.selfHealingEnabled",!1)&&g,Qt=p.get("slab.selfHealingIntervalMs",21600000),te=p.get("slab.eolApiBase","https://endoflife.date/api"),Ne=Ui(te),Ue=Math.max(60000,Math.floor(Qt/2)),k=new Jt(e,He,Zt,Ne,Ue),ct=new Ot(e,t.environmentVariableCollection),ee=new Gt(e,t.extensionPath,Oe),Pt=null,Lt=null,nt=null,ie=(a,u)=>{let m=u?`${a} (${u.toLocaleString()} tps)`:a;e.appendLine(`[loom-reflex] panel lane ${m}`),l.activate()},Ge=(a)=>{e.appendLine(`[loom-reflex] primary lane ${a}`),n.commands.executeCommand("workbench.view.extension.chthonic-archive"),n.commands.executeCommand("chthonic.loomView.focus")};if(!T.ready)e.appendLine(`[reactor] lane parked: ${T.reason}`);t.subscriptions.push(k,ct,ee);let oe=(a)=>{if(s.updateEntropy(a),e.appendLine(`[workspace-health] score ${Math.round(a.decay_score*100)}% (${a.status}) from ${a.source_mise??"no-mise"}`),a.validator_active){let y=`${a.validator_process??"validator"}:${a.validator_source_mise}`;if(y!==Lt)Lt=y,Ge(`validator-online:${y}`)}else Lt=null;if(a.firedancer_surge&&a.validator_active){let y=`${a.checked_at_epoch_ms}:${a.simulated_tps}`;if(y!==nt)nt=y,ie("workspace-health-surge",a.simulated_tps)}else nt=null;if(!a.critical||!a.auto_update_enabled){Pt=null;return}let u=`${a.status}:${a.auto_update}:${[...a.critical_tools].sort().join(",")}`;if(u===Pt)return;Pt=u;let m=a.critical_tools.length>0?a.critical_tools.join(", "):"runtime toolchain";n.window.showWarningMessage(`Workspace health is critical (${Math.round(a.decay_score*100)}%). ${m} needs healing.`,"Run mise upgrade","Later").then((y)=>{if(y==="Run mise upgrade")d.runNow("manual")})};if(t.subscriptions.push(k.onDidReceiveEnv((a)=>{ct.applyTerminalEnv(a)}),k.onDidReceiveSynapse((a)=>{ee.updateDescriptor(a)}),k.onDidReceiveEntropyState((a)=>{oe(a)}),k.onDidReceiveFiredancerSurge((a)=>{if(a.surge){let u=`${a.slot}:${a.simulated_tps}`;if(u!==nt)nt=u,ie("daemon-surge",a.simulated_tps)}})),T.ready&&i&&Xt){if(k.start(i),k.requestEntropyState().then((u)=>{oe(u)}).catch((u)=>{e.appendLine(`[workspace-health] initial snapshot unavailable: ${u}`)}),Je)ct.activate();let a=x.join(i,".git","HEAD");if(R.existsSync(a)){let u=null,m=R.watch(x.join(i,".git"),{persistent:!1},(y,M)=>{if(M==="HEAD"||M?.startsWith("refs")){if(u)clearTimeout(u);u=setTimeout(()=>{e.appendLine("[reactor] git change detected, recomputing sediment"),k.requestSediment(10,500).catch((G)=>{e.appendLine(`[reactor] live-loop sediment failed: ${G}`)})},800)}});t.subscriptions.push({dispose:()=>m.close()})}}if(i&&Rt)d.start({intervalMs:Qt,eolApiBase:te}),d.runNow("interval");if(t.subscriptions.push(n.commands.registerCommand("chthonic.activateCockpit",()=>{ct.activate()}),n.commands.registerCommand("chthonic.deepFocus",()=>{r.activate()}),n.commands.registerCommand("chthonic.restoreOrder",()=>{l.activate()}),n.commands.registerCommand("chthonic.slabHeal",()=>{if(!g){n.window.showWarningMessage("Native sidecars are disabled (chthonic.security.allowNativeSidecars=false).");return}d.runNow("manual")}),n.commands.registerCommand("chthonic.refreshRustification",()=>{w("manual-command")}),n.commands.registerCommand("chthonic.annoDetect",()=>{if(!g){n.window.showWarningMessage("ANNO lane is disabled by chthonic.security.allowNativeSidecars=false."),n.commands.executeCommand("chthonic.entropyRefresh");return}if(!T.ready){n.window.showWarningMessage(`ANNO lane unavailable: ${T.reason}. Using workspace-health fallback.`),n.commands.executeCommand("chthonic.entropyRefresh");return}if(i)k.start(i);n.window.showInformationMessage("ANNO project detection triggered")}),n.commands.registerCommand("chthonic.reactorSediment",async()=>{if(!T.ready){b.rescanNow(),b.requestGraph(260),n.window.showInformationMessage(`Reactor sediment lane unavailable (${T.reason}); refreshed workspace-health graph instead.`);return}try{let a=await k.requestSediment(10,500);n.window.showInformationMessage(`Sediment computed: ${a.file_count} files, ${a.layer_count} layers (${a.backend}, ${a.compute_time_ms}ms)`)}catch(a){n.window.showErrorMessage(`Sediment computation failed: ${a}`)}}),n.commands.registerCommand("chthonic.openWebCockpit",async()=>{let a=Te(o);if(!await Be(a,1200)){let m=await n.window.showWarningMessage(`Web cockpit is not responding at ${a}.`,"Start and Open","Open Anyway","Cancel");if(!m||m==="Cancel")return;if(m==="Start and Open")await n.commands.executeCommand("chthonic.startWebCockpit"),await Fi(1800)}Gi(t,e,a)}),n.commands.registerCommand("chthonic.startWebCockpit",()=>{if(!i){n.window.showWarningMessage("Workspace root is required to start the Bun/Next cockpit.");return}let a=n.window.createTerminal({name:"Chthonic Web Cockpit",cwd:i});a.show(),a.sendText("bun run web:dev"),e.appendLine("[web-cockpit] started via bun run web:dev")}),n.commands.registerCommand("chthonic.openBunTrainingDocs",async()=>{let a=[{label:"Bun React Guide",description:"Build a React app with Bun",url:"https://bun.com/docs/guides/ecosystem/react#build-a-react-app-with-bun"},{label:"Bun Next.js Guide",description:"Build an app with Next.js and Bun",url:"https://bun.com/docs/guides/ecosystem/nextjs#build-an-app-with-next-js-and-bun"},{label:"Bun Tailwind (Fullstack Bundler)",description:"Tailwind plugin lane for Bun fullstack",url:"https://bun.com/docs/bundler/fullstack#tailwindcss-plugin"},{label:"Bun SQLite API",description:"Typed SQL training lane (bun:sqlite)",url:"https://bun.com/docs/api/sqlite"}],u=await n.window.showQuickPick(a,{placeHolder:"Open Bun training docs"});if(!u)return;n.env.openExternal(n.Uri.parse(u.url))}),n.commands.registerCommand("chthonic.postRestartVerify",()=>{if(!i){n.window.showWarningMessage("Workspace root is required to run post-restart verification.");return}let a=n.window.createTerminal({name:"Chthonic Post-Restart Verify",cwd:i});a.show(),a.sendText("bun run --cwd extensions/chthonic-archive insiders:post-restart:verify"),e.appendLine("[insiders] post-restart verification lane started")}),n.commands.registerCommand("chthonic.restartGate",()=>{if(!i){n.window.showWarningMessage("Workspace root is required to run restart gate checks.");return}let a=n.window.createTerminal({name:"Chthonic Restart Gate",cwd:i});a.show(),a.sendText("bun run --cwd extensions/chthonic-archive insiders:restart:gate"),e.appendLine("[insiders] restart gate check lane started")}),n.commands.registerCommand("chthonic.runtimeStatus",async()=>{let a=Te(o),u=await Be(a,900),m=new Set(await n.commands.getCommands(!0)),y=n.extensions.getExtension("chthonic-archive.chthonic-mandala"),M=n.extensions.getExtension("chthonic-archive.chthonic-statusbar"),G=wi({workspaceRoot:i,entropyEnabled:$,entropyPolyglotEnabled:P,entropyLedgerMode:U,reactorReadiness:T,slabSelfHealingEnabled:Rt,allowNativeSidecars:g,extensionPath:t.extensionPath,webCockpitUrl:a,webCockpitReachable:u,commandCatalog:m,bridgeMandala:{installed:Boolean(y),active:y?.isActive??!1,commandAvailable:m.has("chthonic.mandalaBridge.switchTheme")},bridgeStatusbar:{installed:Boolean(M),active:M?.isActive??!1,commandAvailable:m.has("chthonic.bridge.verifySSOT")}});e.show(!0),e.appendLine("[runtime] component status");for(let z of G)e.appendLine(`[runtime] ${z}`);let ze=G.some((z)=>z.includes("DISABLED")||z.includes("PARKED")||z.includes("UNAVAILABLE")||z.includes("MISSING")||z.includes("UNREACHABLE"));n.window.showInformationMessage(ze?"Some runtime lanes are parked. See Chthonic Archive output for details.":"All runtime lanes are operational.")})),i&&o.get("webCockpit.autoStart",!1))n.commands.executeCommand("chthonic.startWebCockpit");let xt=new je;n.window.registerTreeDataProvider("chthonic.themeView",xt),t.subscriptions.push(n.commands.registerCommand("chthonic.switchTheme",async(a)=>{let u=[{label:"$(paintcan) Flesh & Earth",description:"The Decorator · Warm earth · WCAG AA",id:"Chthonic — Flesh & Earth (The Decorator)"},{label:"$(zap) ROGBIV",description:"Spectra Chroma · Spectral canon · FA¹⁻⁵",id:"Chthonic — ROGBIV (Spectra Chroma)"},{label:"$(symbol-color) Geological Core",description:"Sister Ferrum Scoriae · Forge strata · WCAG AA",id:"Chthonic — Geological Core (Sister Ferrum Scoriae)"},{label:"$(flame) The Decorator",description:"The Decorator · Tier 0.5 · Ornamental precision",id:"Chthonic — The Decorator"}],m=a;if(!m){let y=n.workspace.getConfiguration("workbench").get("colorTheme"),M=await n.window.showQuickPick(u.map((G)=>({...G,picked:y===G.id})),{placeHolder:`Current: ${y}`});if(!M)return;m=M.id}await n.workspace.getConfiguration("workbench").update("colorTheme",m,n.ConfigurationTarget.Workspace),n.window.showInformationMessage(`Theme: ${m}`),xt.refresh()}));let se=o;if(se.get("showSSOTHash",!0)){let a=n.window.createStatusBarItem(n.StatusBarAlignment.Left,50);a.command="chthonic.verifySSOT",a.tooltip="Policy fingerprint — click to verify integrity",t.subscriptions.push(a),De(a),t.subscriptions.push(n.workspace.onDidSaveTextDocument((u)=>{if(u.fileName.includes("copilot-instructions"))De(a)}))}if(se.get("showLineage",!1)){let a=n.window.createStatusBarItem(n.StatusBarAlignment.Left,49);a.command="workbench.view.scm";let u=()=>{let y=Ki(i);a.text=`$(git-branch) ${y.label}`,a.tooltip=y.tooltip,a.show()};u();let m=setInterval(u,6000);t.subscriptions.push({dispose:()=>clearInterval(m)}),a.show(),t.subscriptions.push(a)}let $t=new Ae;$t.configure({entropyEnabled:$,allowSidecars:g,reactorReady:T.ready,selfHealing:Rt}),n.window.registerTreeDataProvider("chthonic.statusView",$t),t.subscriptions.push(n.commands.registerCommand("chthonic.verifySSOT",async()=>{let a=wt();if(a)n.window.showInformationMessage(`Policy fingerprint: ${a.substring(0,16)}…`);else n.window.showWarningMessage("Policy file not found")})),t.subscriptions.push(n.commands.registerCommand("chthonic.refreshStatus",()=>{if($t.refresh(),xt.refresh(),$)b.rescanNow(),b.requestGraph(260),Y.requestManualScan();w("refresh-status")})),Ni(t,l,e)}function Oi(){}async function Ni(t,e,i){let s=String(t.extension.packageJSON.version??"unknown");if(t.workspaceState.get("chthonic.layoutPlacementAppliedVersion")===s)return;try{await e.activate(),await t.workspaceState.update("chthonic.layoutPlacementAppliedVersion",s),i.appendLine(`[layout] default Chthonic placement applied for ${s}`)}catch(l){i.appendLine(`[layout] placement bootstrap failed: ${stringifyError(l)}`)}}function wt(){let t=n.workspace.workspaceFolders?.[0];if(!t)return null;let e=n.workspace.getConfiguration("chthonic").get("ssotPath",".github/copilot-instructions.md"),i=x.join(t.uri.fsPath,e);if(!R.existsSync(i))return null;let s=R.readFileSync(i,"utf-8").replace(/\r\n/g,`
`).replace(/\r/g,`
`).split(`
`).map((r)=>r.trimEnd()).join(`
`).trim();return St.createHash("sha256").update(s,"utf-8").digest("hex")}function De(t){let e=wt();if(e)t.text=`$(shield) ${e.substring(0,8)}`,t.show();else t.text="$(shield) Policy ??",t.show()}function gt(t){let e=t.trim();return e.length>0?e:void 0}function Ui(t){let e=t.trim().replace(/\/+$/,"");if(e.endsWith("/api"))return`${e}/v1`;return e.length>0?e:"https://endoflife.date/api/v1"}function Te(t){let e=t.get("webCockpit.url","http://127.0.0.1:3000").trim();if(/^https?:\/\//i.test(e))return e;return`http://${e}`}function Gi(t,e,i){let o=n.window.createWebviewPanel("chthonicWebCockpit","Chthonic Web Cockpit",n.ViewColumn.Active,{enableScripts:!0,retainContextWhenHidden:!0});o.webview.html=zi(i),o.webview.onDidReceiveMessage(async(s)=>{if(!s||typeof s!=="object")return;let r=s;if(r.type==="start"){await n.commands.executeCommand("chthonic.startWebCockpit");return}if(r.type==="openExternal"&&r.url){await n.env.openExternal(n.Uri.parse(r.url));return}e.appendLine(`[webview] Received message: ${JSON.stringify(s)}`)},void 0,t.subscriptions),o.onDidDispose(()=>{e.appendLine("[webview] Chthonic Web Cockpit panel disposed")},void 0,t.subscriptions)}function zi(t){let e=Vi(),i=Ii(t);return`<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
