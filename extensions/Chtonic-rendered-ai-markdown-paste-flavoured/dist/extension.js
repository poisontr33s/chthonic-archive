...
var require_style_parser = __commonJS((exports2) => {
  ...
  exports2.hyphenate = exports2.parse = undefined;
...
var require_turndown_plugin_gfm_cjs = __commonJS((exports2) => {
  Object.defineProperty(exports2, "__esModule", { value: true });
...
var fence = "```";
function languageFromCodeElement(node) {
  const className = node.getAttribute?.("class") ?? "";
  const match = className.match(/(?:^|\s)(?:language|lang)-([A-Za-z0-9_+.#-]+)/);
  return match?.[1] ?? "";
}
function fenceForCode(text) {
...
`).split(`
`).map((line) => line.replace(/^(\s*[-+*]) {2,}/u, "$1 ").replace(/^(\s*\d+\.) {2,}/u, "$1 ").replace(/[ \t]+$/g, "")).join(`
`).replace(/\n{3,}/g, `
...
// src/extension.ts
var htmlMime = "text/html";
var diagnostic = "Clipboard did not include text/html; pasted source may only provide plain text.";
function activate(context) {
  const kind = vscode.DocumentDropOrPasteEditKind.Text.append("markdown", "renderedAiHtml");
  const htmlProvider = vscode.languages.registerDocumentPasteEditProvider({ language: "markdown" }, {
    async provideDocumentPasteEdits(_document, _ranges, dataTransfer, _context, token) {
      const htmlItem = dataTransfer.get(htmlMime);
      if (!htmlItem) {
        vscode.window.showInformationMessage(diagnostic);
        return [];
      }
      ...
      return [
        new vscode.DocumentPasteEdit(markdown, "Paste Rendered AI HTML as Markdown", kind)
      ];
    }
  }, {
  ...
  const plainTextDiagnosticProvider = vscode.languages.registerDocumentPasteEditProvider({ language: "markdown" }, {
  ...
  const pasteCommand = vscode.commands.registerCommand("renderedAiMarkdownPaste.pasteAsMarkdown", () => vscode.commands.executeCommand("editor.action.clipboardPasteAction"));
