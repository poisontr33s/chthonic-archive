...
fn detect_supreme_matriarchs(data: &GameData) {
    for entity in &data.entities {
        if entity.tier <= SUPREME_MATRIARCH_TIER {
            info!("═══════════════════════════════════════════════════════════════");
            info!("🔥 SUPREME MATRIARCH DETECTED: {0}", entity.name);
            info!("   Archetype: {0} | Tier: {1:.1}", entity.archetype, entity.tier);
            info!("   WHR: {0:.3} | Power: {1}", entity.physics.whr, entity.stats.power);
...
fn log_entity_summary(data: &GameData) {
    ...
    info!("   World Layers: {0}", data.world.layers.len());
}
