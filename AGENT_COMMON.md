...
- **Ruby:** use `rv` for runtime and gem/tool isolation.
  ...
  - **ZJIT+YJIT via WSL2 (BUILT ✅):** Ruby 4.0's JIT requires POSIX (`madvise`/`mprotect`/`PROT_NONE`) — builds on Linux only. Solution: WSL2 distro `podman-machine-default` (Fedora 40, root, WSL2 v2), gcc 14.2.1, Linux rustc 1.95.0. Binary at `/opt/ruby-zjit/bin/ruby` (`ruby 4.0.3 +PRISM [x86_64-linux]`, ZJIT OK, YJIT OK). Windows shim: `pwsh -File build/ruby-zjit/ruby-zjit.ps1 --zjit script.rb` (forwards all args via `wsl -d podman-machine-default -u root`). Build scripts: `build/ruby-zjit/build-wsl2.sh` (Linux build), `build/ruby-zjit/build.sh` (Windows MSYS2 attempt — blocked by POSIX guard). Native Windows rv prebuilt (no JIT) remains the default for dev iteration; use WSL2 binary for JIT benchmarking/profiling.
