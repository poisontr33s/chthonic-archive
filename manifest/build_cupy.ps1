...
try {
    if ($DryRun) {
        ...
        uv run python -c "import cython; print(f'  Cython {cython.__version__}')" 2>&1
...
if (-not $SkipTests) {
    ...
    uv run python -m pytest "$cupyDir\tests\cupy_tests\core_tests" -x -q --timeout=60 2>&1 | Select-Object -First 20
