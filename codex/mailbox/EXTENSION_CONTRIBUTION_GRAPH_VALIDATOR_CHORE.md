...
## Stage 4 — Repeatable WPTG Framework (Begin Anew Baseline)

This stage converts the originally linear Phase 0 -> Part 4 delivery into a reusable, learning-driven cycle.

### Stage 4A — Framework Artifact

- `scripts/wptg_repeatable_cycle.py`
  - Replays Phase 0 -> Part 4 in order.
  - Uses `uv` for Python scripts, `cargo` for ankh-forge Rust subcommands, and `bun` for TypeScript lane pulse.
  - Persists cycle memory and deltas for next-run adaptation.

### Stage 4B — Default View and Learning Memory

Outputs generated by each cycle:
...
- `codex/mailbox/WPTG_REPEATABLE_FRAMEWORK.md`

### Stage 4C — Execution Mode

To start a new baseline cycle:

```powershell
uv run scripts/wptg_repeatable_cycle.py --begin-anew
```

The resulting default view becomes the new WPTG starting posture for subsequent loops.

### Stage 4D — Execution Evidence (Completed)
...
Cycle verdict and score snapshot:

- Verdict: `WARN` (expected lane-exclusion warnings + census fail-grade anomalies)
- Boon: `11.5`
- Penalty: `1.3`
- Net: `10.2`
