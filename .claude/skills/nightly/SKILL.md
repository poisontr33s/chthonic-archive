...
A plan awaiting approval, or a question awaiting an answer, is functionally a hard stop when nobody's there to respond — indistinguishable from silently failing for the entire absence window. This skill never calls `EnterPlanMode` and never calls `AskUserQuestion` during a run. If a task feels like it needs upfront sign-off before executing, that feeling is itself the signal: it's an atlas-§2/§3-sized item (externally blocked, or genuinely foundational — see §1's own gloss of those two atlas categories) that doesn't belong in a nightly at all. Stop short, name it in the landing doc's "recommended next moves" table (§4) — the same way this skill's own §2 already handles forks — rather than reaching for a tool that waits for a reply that isn't coming. Applies identically regardless of who's driving the invocation.
...
No passive clock exists between those two checks — elapsed time is only known at the moments this section explicitly asks for it, not continuously, and there is no mechanism to feel time passing in between. That's sufficient here: this skill was never meant to pace itself against a countdown or fill the absence window. It does one bounded task and stops, whatever real time that honestly takes — a run that finishes in twenty minutes and a run that finishes in three hours are both correct, as long as the thing that shipped is real and verified. Padding work to consume more of the window would violate §2's "one well-verified thing beats several padded ones," not satisfy it.
...
If a real gap surfaced: name it precisely, fix it in THIS SAME `SKILL.md`, and record it in two places — the landing doc's tone note (prose, for a human reading that one run) and the ledger record's `self_improvement` field (§5, structured, for scanning across every run at once). If nothing surfaced: say so explicitly in both places rather than silently skipping the step. "Reviewed, nothing found this run" is a real, valid outcome, not a failure to find something — don't invent a weakness to report just to have filled in this section.

**Re-verifying an edit to this file means actually reading the edited content, not running `skill_audit.py` and trusting its score.** Caught 2026-07-09: `skill_audit.py`'s Claude-flavor check only confirms `SKILL.md` exists and its frontmatter has `name`/`description`/known `allowed-tools` — four shallow checks, zero bytes of the body ever read. A "100%" from it says nothing about whether the edit is internally consistent, whether a cross-reference (`§N`) still points at the right section after a renumbering, or whether the prose even makes sense — it is not a substitute for reading what was actually written, the same "no self-verification circularity" lesson this skill already names elsewhere (§1's verify-before-assume), recurred with a different tool because the lesson hadn't generalized past the first one it was learned from. After any edit to this file: manually trace every `§N` cross-reference against the current header list and confirm each one still resolves to what it claims to.
...
**A third question, added 2026-07-09 after the user asked for cost/value grilling against real usage-panel data:** classify this run's own commits as `feature` (delivers the actual requested task output) or `meta` (this skill's own design, scaffolding, or self-correction — not itself the task's deliverable), and check the ratio honestly. The night this question was added, 2 of 9 same-arc commits were feature work (zombie B2/B3, zombie C1) and 7 were meta — building the ledger, the quality bar, the modes, the plan-mode guard, then a next-morning commit correcting two real errors in that meta-work. That ratio isn't automatically bad — first-time scaffolding costs more than steady-state reuse — but it must be named, not buried under "2 features shipped." Record the split in the ledger record's `commits` field (§5) every run, not just when someone asks from outside.

**After finishing that classification, check it against `CLAUDEBASE/nightlies/MATURITY.md`** — the user's own scorecard for this skill (currently 7/10), with explicit, run-based requirements for each higher rung. Reassess the score there if this run's evidence moves it; do not add new mechanism to this file just to chase a higher number faster than real runs earn it — the ladder's own 8/10 rung requires several runs with *no* new major skill-design in between.
