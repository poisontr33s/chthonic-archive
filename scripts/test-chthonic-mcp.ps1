...
$request2 = '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"chthonic_status","arguments":{}},"id":2}'
...
$request3 = '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"meta_cli","arguments":{"domain":"ide","action":"detect"}},"id":3}'
...
$request4 = '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"meta_cli","arguments":{"domain":"mcp","action":"status"}},"id":4}'
...
$request5 = '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"meta_cli","arguments":{"domain":"detect"}},"id":5}'
...
# Test 6: Direct brush tool
Write-Host "🦀 Test 6: brush --version (via MCP)" -ForegroundColor Yellow
$request6 = '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"brush","arguments":{"args":["--version"]}},"id":6}'
$output = echo $request6 | bun run $MCP_SERVER 2>$null | ConvertFrom-Json
if ($output.result) {
    $output.result.content[0].text -split "`n" | Select-Object -First 5
}
Write-Host ""

# Test 7: chthonic shell probe through meta_cli
Write-Host "🧭 Test 7: chthonic shell probe (via MCP meta_cli)" -ForegroundColor Yellow
$request7 = '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"meta_cli","arguments":{"domain":"shell","action":"probe","args":["--json"]}},"id":7}'
$output = echo $request7 | bun run $MCP_SERVER 2>$null | ConvertFrom-Json
if ($output.result) {
    $output.result.content[0].text -split "`n" | Select-Object -First 10
}
Write-Host ""
