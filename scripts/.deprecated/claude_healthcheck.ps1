...
function ConvertFrom-JsonRelaxed {
  ...
  try {
    ...
    # Some Claude commands intermittently prepend progress text before JSON.
    # Recover by slicing from the first JSON token to the last matching closer.
    foreach ($pair in @(@("[", "]"), @("{", "}"))) {
      $open = [string]$pair[0]
      $close = [string]$pair[1]
      $start = $raw.IndexOf($open)
      $end = $raw.LastIndexOf($close)
      if ($start -ge 0 -and $end -gt $start) {
        $candidate = $raw.Substring($start, ($end - $start + 1))
        try {
          return ($candidate | ConvertFrom-Json -ErrorAction Stop)
        } catch {
          # keep trying other bracket forms
        }
      }
    }
...
try {
  ...
  $checks += Try-Run -Label "claude plugin list --json" -Body {
    ...
    $s = if ($txt -is [Array]) { $txt -join "`n" } else { [string]$txt }
    $obj = ConvertFrom-JsonRelaxed -Text $s
    # Prefer array payload; tolerate wrapper objects for forward compatibility.
    $plugins = @()
    if ($obj -is [Array]) {
      $plugins = @($obj)
    } elseif ($obj.plugins) {
      ...
      throw "Unexpected plugin list JSON shape."
    }
    $ids = @()
