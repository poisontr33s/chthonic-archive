...
CREATE VIEW IF NOT EXISTS session_timeline AS
  SELECT
    ...
    COUNT(DISTINCT fe.id) AS editCount,
    COUNT(DISTINCT tc.id) AS cmdCount,
    COUNT(DISTINCT cb.id) AS codeBlockCount,
    COUNT(DISTINCT ms.id) AS memoryFileCount,
    COUNT(DISTINCT cr.sha) AS commitCount
  FROM sessions s
  LEFT JOIN file_edits      fe ON fe.sessionId = s.sessionId
  LEFT JOIN terminal_cmds   tc ON tc.sessionId = s.sessionId
  LEFT JOIN code_blocks     cb ON cb.sessionId = s.sessionId
  LEFT JOIN memory_snapshots ms ON ms.sessionId = s.sessionId
  LEFT JOIN commit_refs     cr ON cr.sessionId = s.sessionId
...
function ingestSession(db: Database, sessionId: string): boolean {
  ...
  const insFileEdit  = db.prepare("INSERT INTO file_edits (sessionId, filePath, tool, turn) VALUES (?, ?, ?, ?)");
  const insTermCmd   = db.prepare("INSERT INTO terminal_cmds (sessionId, command, goal, explanation, turn) VALUES (?, ?, ?, ?, ?)");
  const insCodeBlock = db.prepare("INSERT INTO code_blocks (sessionId, lang, lines, preview, turn) VALUES (?, ?, ?, ?, ?)");
  const insToolCall  = db.prepare("INSERT INTO tool_calls (sessionId, toolName, callId, turn, success) VALUES (?, ?, ?, ?, NULL) RETURNING id");
  const updSuccess   = db.prepare("UPDATE tool_calls SET success = ? WHERE id = ?");
      ...
      if (type === "user.message") {
        ...
        if (content) {
          for (const cb of extractCodeBlocks(content, turnIdx))
            insCodeBlock.run(sessionId, cb.lang, cb.lines, cb.preview, cb.turn);
        ...
        const row = insToolCall.get(sessionId, toolName, callId ?? null, turnIdx) as { id: number } | undefined;
        ...
        if (toolName === "replace_string_in_file" || toolName === "create_file") {
          ...
          if (fp) insFileEdit.run(sessionId, fp, toolName, turnIdx);
          ...
          if (Array.isArray(reps)) {
            for (const r of reps) {
              ...
              if (fp && !seen.has(fp)) { seen.add(fp); insFileEdit.run(sessionId, fp, toolName, turnIdx); }
            }
          }
        }
        ...
        if (rowId !== undefined && success !== null) updSuccess.run(success, rowId);
      }
    }
  });
  ingest();
...
if (rebuildMode) {
  ...
  for (const t of ["memory_snapshots", "commit_refs", "tool_calls", "code_blocks", "terminal_cmds", "file_edits", "session_restarts", "sessions"])
...
if (statsMode) {
  console.log("\n📊 CORPUS STATS\n");
  for (const t of ["sessions", "session_restarts", "file_edits", "terminal_cmds", "code_blocks", "tool_calls", "commit_refs", "memory_snapshots"]) {
