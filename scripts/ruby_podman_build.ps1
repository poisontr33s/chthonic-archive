...
#
# Usage:
#   pwsh -NoProfile -File scripts/ruby_podman_build.ps1 [-Build] [-Run] [-Verify] [-Export <path>]
#
# Flags:
...
#
# Companion files:
#   build/ruby-zjit-podman/Containerfile  — the build definition
#   build/ruby-zjit-podman/build-verify.sh — post-build verification
#
...
$CONTAINERFILE = Join-Path $PSScriptRoot ".." "build" "ruby-zjit-podman" "Containerfile"
$VERIFY_SCRIPT = Join-Path $PSScriptRoot ".." "build" "ruby-zjit-podman" "build-verify.sh"
...
if ($Build) {
    ...
    $ctxWsl = Get-WslPath (Resolve-Path (Join-Path $PSScriptRoot ".." "build" "ruby-zjit-podman")).Path
...
Quick start:
  pwsh -File scripts/ruby_podman_build.ps1 -Build
  pwsh -File scripts/ruby_podman_build.ps1 -Verify
