...
"""
bun_compliance_audit.py — Bun Package Manager Compliance Scanner

@SID:           TOOL_BUN_COMPLIANCE_AUDIT
@Shabti:        CLI Script
@Heka-Ayni:     CONCEPT_BUN_COMPLIANCE_AUDIT
@Purpose:       Scans repository for non-Bun package manager usage (npm, npx,
                yarn, pnpm) and reports violations with severity levels per
                SSOT §XIV.2 mandate.
                Usage:
                    uv run python scripts/bun_compliance_audit.py
                    uv run python scripts/bun_compliance_audit.py --fix
